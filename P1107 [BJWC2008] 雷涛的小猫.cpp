...
namespace TANGYIXIAO {
const int MAXH = 2005;
int n, h, d, cnt[MAXH][MAXH], dp[MAXH][MAXH], f[MAXH];
...
inline void solve(int Task_Id) {
    ...
    for (int i = 1; i <= n; ++i) {
        dp[i][h] = cnt[i][h];
        f[h] = std::max(f[h], dp[i][h]);
    }
    for (int height = h - 1; height >= 1; --height) {
        for (int i = 1; i <= n; ++i) {
            int best = dp[i][height + 1];
            if (height + d <= h) {
                best = std::max(best, f[height + d]);
            }
            dp[i][height] = best + cnt[i][height];
            f[height] = std::max(f[height], dp[i][height]);
        }
    }
    int ans = 0;
    for (int i = 1; i <= n; ++i) {
