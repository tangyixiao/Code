...
namespace TANGYIXIAO {
const int N = 1e3 + 5, TN = 4e6 + 5, dr[4] = {-1, 1, 0, 0}, dc[4] = {0, 0, -1, 1};
const char dirChar[4] = {'U', 'D', 'L', 'R'};
int h, w, sr = -1, sc = -1, gr = -1, gc = -1, tn, fa[TN];
string mp[N];
char vis[TN];
...
inline void solve(int Task_Id) {
    ...
    for (int d = 0; d < 4; ++d) {
        ...
        if (nr >= 0 && nr < h && nc >= 0 && nc < w && mp[nr][nc] != '#') {
            int idx = ((nr * w + nc) << 2) | d;
            vis[idx] = 1;
            fa[idx] = -1;
            q.push_back(idx);
    ...
    while (hd < (int)q.size()) {
        int idx = q[hd++], d = idx & 3, pos = idx >> 2, r = pos / w, c = pos % w;
        if (r == gr && c == gc) {
            string path;
            int cur = idx;
            while (cur != -1) {
                path.push_back(dirChar[cur & 3]);
                cur = fa[cur];
            }
