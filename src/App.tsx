  ...
  return <main className="app-shell">
    ...
    {error ? <section className="error-card"><p>{error}</p><button onClick={() => void load()}>重试加载</button></section> : <div className="workspace">
      <aside className="sidebar" aria-label="文件列表">
        ...
        <div className="file-list">{files.map((file) => <button key={file.path} className={`file-row ${file.path === selectedPath ? 'selected' : ''}`} onClick={() => select(file.path)}><span className={file.type}>{file.type === 'cpp' ? 'C++' : 'MD'}</span>{file.name}</button>)}</div>
      </aside>
