...
#pragma GCC optimize("inline","fast-mth","unroll-loops","no-stack-protector")
...
#pragma GCC optimize("tree-veize")     // 对应 -ftree-veize：启用自动向量化
*/
...
#include <cmth>    // 数学函数 (sin, sqrt, pow)
...
#include <ctgmth>    // 泛型数学 (tgmth.h) [C++17 废弃]
#endif
#endif
...
#include <ext/pb_ds/listupdate_policy.hpp> // 列表更新策略（用于哈希表冲突处理）
...
inline void solve(int Task_Id) {
    ...
    ve<int> z(n + 1);
    function<void(int, int)> dfz = [&](int u, int p) {
        z[u] = 1;
        for (auto &[v, w] : g[u]) {
            if (v == p)
                continue;
    ...
    for (int u = 1; u <= n; ++u) {
        ...
        for (int v = u; v <= n; ++v) {
            if (u != v) {
                ve<int> pth;
                function<bool(int, int)> find_pth = [&](int x, int p) {
                    pth.push_back(x);
                    if (x == v) {
                        return true;
                    }
