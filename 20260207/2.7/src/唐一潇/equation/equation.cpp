...
bool SUBTASK_EQU, SUBTASK_512;
int T, n, a[N], p, loga[N], tmp, cnt, MIN_, MAX_;
inline void SOLVE_EQU() {
    tmp = ((a[1] * loga[1] * loga[1]) << 2) + ((a[1] * loga[1]) << 1) + a[1];
    if (tmp == p) {
        cnt = n * (n - 1) * (n - 2) / 6;
        cout << cnt << "\n";
    } else {
...
inline void solve() {
    ...
    int lgmi = 1 << (highbit(MIN_)), lgma = 1 << (highbit(MAX_));
    int resmi = MIN_ * lgmi * lgmi * 4 + MIN_ * lgmi * 2 + MIN_;
    int resma = MAX_ * lgma * lgma * 4 + MAX_ * lgma * 2 + MAX_;
    if (resmi > p || resma < p) {
        cout << "0\n";
        return;
    }
    for (int i = 1; i <= n; i++) {
        for (int j = i + 1; j <= n; j++) {
            for (int k = j + 1; k <= n; k++) {
                tmp = ((a[i] * loga[j] * loga[k]) << 2) + ((a[j] * loga[k]) << 1) + a[k];
                if (tmp == p) {
                    cnt++;
                }
            }
