...
inline void solve(int Task_Id) {
    ...
    for (int i = 1; i <= n; ++i)
        dp[i][0] = dp[i - 1][0] + score[idx(s1[i - 1])][idx('-')];
    for (int j = 1; j <= m; ++j)
        dp[0][j] = dp[0][j - 1] + score[idx('-')][idx(s2[j - 1])];
    ...
    for (int i = 1; i <= n; ++i) {
        ...
        for (int j = 1; j <= m; ++j) {
            ...
            dp[i][j] = dp[i - 1][j - 1] + score[c1][c2];

            dp[i][j] = max(dp[i][j], dp[i - 1][j] + score[c1][idx('-')]);

            dp[i][j] = max(dp[i][j], dp[i][j - 1] + score[idx('-')][c2]);
        }
    }
