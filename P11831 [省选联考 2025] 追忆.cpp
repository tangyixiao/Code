...
vector<int> b_blocks[128];
int pos_in_b_block[MAXN + 1];
int b_block_of[MAXN + 1];

int a_val[MAXN + 1], b_val[MAXN + 1];
int posA[MAXN + 1], posB[MAXN + 1];

void update_A_leaf(int blk, int node_idx, bool set_bit) {
    int idx = K2_A + blk;
    if (set_bit)
        tree_A[idx].set(node_idx);
    else
        tree_A[idx].reset(node_idx);
    for (idx >>= 1; idx; idx >>= 1) {
        if (tree_A[2 * idx][node_idx] | tree_A[2 * idx + 1][node_idx])
            tree_A[idx].set(node_idx);
        else
            tree_A[idx].reset(node_idx);
    }
...
void query_A(Bitset &res, int l, int r) {
    if (l > r)
        return;
    l += K2_A;
    r += K2_A;
...
void update_B_leaf(int blk, int node_idx, bool set_bit) {
    int idx = K2_B + blk;
    if (set_bit)
        tree_B[idx].set(node_idx);
    else
        tree_B[idx].reset(node_idx);
    for (idx >>= 1; idx; idx >>= 1) {
        if (tree_B[2 * idx][node_idx] | tree_B[2 * idx + 1][node_idx])
            tree_B[idx].set(node_idx);
        else
            tree_B[idx].reset(node_idx);
    }
...
int find_max_b_block(const Bitset &Q) {
    int idx = 1;
    while (idx < K2_B) {
        if ((Q & tree_B[2 * idx + 1]).any())
            idx = 2 * idx + 1;
        else
            idx = 2 * idx;
    }
...
inline void solve(int Task_Id) {
    ...
    for (int i = 0; i < 2 * K2_A; ++i)
        tree_A[i].reset();
    for (int i = 1; i <= n; ++i) {
        int blk = (a_val[i] - 1) / B_A;
        tree_A[K2_A + blk].set(i - 1);
    }
    for (int i = K2_A - 1; i; --i)
        tree_A[i] = tree_A[2 * i] | tree_A[2 * i + 1];

    for (int i = 0; i < 2 * K2_B; ++i)
        tree_B[i].reset();
    for (int i = 1; i <= n; ++i) {
        int blk = (b_val[i] - 1) / B_B;
        tree_B[K2_B + blk].set(i - 1);
    }
    for (int i = K2_B - 1; i; --i)
        tree_B[i] = tree_B[2 * i] | tree_B[2 * i + 1];
    ...
    for (int i = 1; i <= n; ++i) {
        int blk = (b_val[i] - 1) / B_B;
        b_block_of[i] = blk;
        pos_in_b_block[i] = b_blocks[blk].size();
        b_blocks[blk].push_back(i);
    }
    ...
    while (q--) {
        ...
        if (op == 1) {
            ...
            if (x == y)
                continue;
            int ax = a_val[x], ay = a_val[y];
            int i_old_x = (ax - 1) / B_A;
            int i_old_y = (ay - 1) / B_A;
            swap(a_val[x], a_val[y]);
            posA[ax] = y;
            posA[ay] = x;
            if (i_old_x != i_old_y) {
                update_A_leaf(i_old_x, x - 1, false);
                update_A_leaf(i_old_y, x - 1, true);
                update_A_leaf(i_old_y, y - 1, false);
                update_A_leaf(i_old_x, y - 1, true);
            }
        } else if (op == 2) {
            int x, y;
            cin >> x >> y;
            if (x == y)
                continue;
            int bx = b_val[x], by = b_val[y];
            int j_old_x = (bx - 1) / B_B;
            int j_old_y = (by - 1) / B_B;
            swap(b_val[x], b_val[y]);
            posB[bx] = y;
            posB[by] = x;
            if (j_old_x != j_old_y) {
                update_B_leaf(j_old_x, x - 1, false);
                update_B_leaf(j_old_y, x - 1, true);
                update_B_leaf(j_old_y, y - 1, false);
                update_B_leaf(j_old_x, y - 1, true);

                int px = pos_in_b_block[x];
                int last = b_blocks[j_old_x].back();
                b_blocks[j_old_x][px] = last;
                pos_in_b_block[last] = px;
                b_blocks[j_old_x].pop_back();

                b_block_of[x] = j_old_y;
                pos_in_b_block[x] = b_blocks[j_old_y].size();
                b_blocks[j_old_y].push_back(x);

                int py = pos_in_b_block[y];
                last = b_blocks[j_old_y].back();
                b_blocks[j_old_y][py] = last;
                pos_in_b_block[last] = py;
                b_blocks[j_old_y].pop_back();

                b_block_of[y] = j_old_x;
                pos_in_b_block[y] = b_blocks[j_old_x].size();
                b_blocks[j_old_x].push_back(y);
            }
        } else {
            int x, l, r;
            cin >> x >> l >> r;
            ...
            int i_l = (l - 1) / B_A;
            int i_r = (r - 1) / B_A;
            int left_full = (l == i_l * B_A + 1) ? i_l : i_l + 1;
            int right_full = (r == (i_r + 1) * B_A) ? i_r : i_r - 1;
            if (left_full <= right_full)
                query_A(M_range, left_full, right_full);

            if (left_full > i_l) {
                int R = (i_l + 1) * B_A;
                for (int v = l; v <= min(r, R); ++v)
                    M_range.set(posA[v] - 1);
            }

            if (right_full < i_r && i_r > i_l) {
                int L = i_r * B_A + 1;
                for (int v = max(l, L); v <= r; ++v)
                    M_range.set(posA[v] - 1);
            }

            Bitset Q = S[x] & M_range;
            if (Q.none()) {
                cout << "0\n";
                continue;
            }

            int j = find_max_b_block(Q);
            int ans = 0;
            for (int y : b_blocks[j]) {
                if (a_val[y] >= l && a_val[y] <= r && S[x][y - 1]) {
                    if (b_val[y] > ans)
                        ans = b_val[y];
                }
            }
