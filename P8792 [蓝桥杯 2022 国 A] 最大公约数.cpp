//  Author: Tangyixiao
//  Time: 2026-08-13 11:04:11
//  Problem: P8792 [蓝桥杯 2022 国 A] 最大公约数
//  Contest: Luogu
//  URL: https://www.luogu.com.cn/problem/P8792
//  Memory Limit: 512 MB
//  Time Limit: 1000 ms
//  Interactive: false
//  Test Type: single
//  Batch ID: 7f78a66e-ed77-4df4-917e-bf1b2e264322
//
// Algorithm:
// Complexity: O()
// Note:
...
inline void solve(int Task_Id) {
    ...
    for (int i = 1; i <= n; i++) {
        ...
        for (auto [g, l] : p) {
            ...
            if (q.back().first == x)
                q.back().second = min(q.back().second, y);
            else
                q.push_back({x, y});
        }
