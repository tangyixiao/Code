...
inline void solve(int Task_Id) {
    ...
    int d = *min_element(k.begin(), k.end());
    int k_val = -1;
    for (int x : k) {
        if (gcd(x, d) == 1) {
            k_val = x;
    ...
    vector<int> start_idx(d, -1), end_idx(d, -1);
    start_idx[order[0]] = 0;

    for (int i = 0; i < d - 1; ++i) {
        int prev = order[i], curr = order[i + 1];
        int sz_prev = groups[prev].size(), sz_curr = groups[curr].size();
        bool found = false;

        for (int e = 0; e < min(3, sz_prev); ++e) {
            if (e == start_idx[prev])
                continue;
            for (int s = 0; s < min(3, sz_curr); ++s) {
                int diff = abs(groups[curr][s] - groups[prev][e]);
                bool ok = false;
                for (int kj : k)
                    if (diff % kj == 0) {
                        ok = true;
        ...
        if (!found) {
            for (int e = 0; e < sz_prev; ++e) {
                if (e == start_idx[prev])
                    continue;
                for (int s = 0; s < sz_curr; ++s) {
                    int diff = abs(groups[curr][s] - groups[prev][e]);
                    bool ok = false;
                    for (int kj : k)
                        if (diff % kj == 0) {
    ...
    int last = order.back();
    end_idx[last] = (start_idx[last] + 1) % groups[last].size();

    vector<int> ans;
    for (int r : order) {
        int s = start_idx[r], e = end_idx[r];
        int sz = groups[r].size();
        vector<int> other;
        for (int i = 0; i < sz; ++i)
            if (i != s && i != e)
                other.push_back(i);
        ans.push_back(groups[r][s]);
        for (int i : other)
            ans.push_back(groups[r][i]);
        if (s != e)
            ans.push_back(groups[r][e]);
    }
