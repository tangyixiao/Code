...
//  Interactive: false
//  Test Type: single
//  Batch ID: 93382300-f0f2-4467-b327-34ac0334ce41
//
// Algorithm:
// Complexity: O()
// Note:
...
int n, a[N], l[N], r[N], lg[N], s[N], mn[K][N], mx[K][N];
stack<int> st;
inline int q(int l, int r, string s) {
    return s == "max" ? (max(mx[lg[r - l + 1]][l], mx[lg[r - l + 1]][r - (1 << lg[r - l + 1]) + 1])) : (min(mn[lg[r - l + 1]][l], mn[lg[r - l + 1]][r - (1 << lg[r - l + 1]) + 1]));
}
inline void solve(int Task_Id) {
    cin >> n;
    for (int i = 1; i <= n; i++) {
    ...
    for (int i = 1; i <= n; i++) {
        if ((q(i, r[i] - 1, "max") - q(l[i], i - 1, "min")) > a[i]) {
            cout << "NO\n";
            return;
        }
    }
