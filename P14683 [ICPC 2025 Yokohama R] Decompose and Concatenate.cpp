...
inline void solve(int Task_Id) {
    ...
    for (int La = 1; La <= 18; ++La) {
        for (int Lb = 1; Lb <= 18; ++Lb) {
            ll min_b = pow10[Lb - 1];
            ll max_b = pow10[Lb] - 1;

            ll max_a_by_b = N - min_b;
            if (max_a_by_b < 1)
                continue;
            ll min_a_by_b = max(1LL, N - max_b);

            ll min_a = max(pow10[La - 1], min_a_by_b);
            ll max_a = min(pow10[La] - 1, max_a_by_b);
            ...
            if (min_a <= max_a) {
                ...
                int len = S.length();
                if (len > max_len || (len == max_len && S > best)) {
                    max_len = len;
                    best = S;
                }
            }
