...
//  Interactive: false
//  Test Type: single
//  Batch ID: 0b7970b4-06f5-4b45-a83b-e18ebab553eb
//
// Algorithm:
// Complexity: O()
// Note:
...
int n, a[N], p[N], q[N], r[N], s[N], t[N], c[N << 5][2], z[N << 5], o, ans;
pair<int, int> b[N];
...
int ask(int x, int l, int rr) {
    int u = r[l - 1], v = r[rr], w = 0;
    for (int i = 30; i >= 0; i--) {
        int k = x >> i & 1;
        if (z[c[v][k ^ 1]] - z[c[u][k ^ 1]])
            w |= 1 << i, u = c[u][k ^ 1], v = c[v][k ^ 1];
        else
            u = c[u][k], v = c[v][k];
    }
    return w;
...
inline void solve(int Task_Id) {
    ...
    for (int i = 1; i <= n; i++) {
        int x = b[i].second, l = p[x], rr = q[x];
        q[l] = rr;
        p[rr] = l;

        if (l)
            ans = max(ans, ask(a[x], p[l] + 1, rr - 1));
        if (rr <= n)
            ans = max(ans, ask(a[x], l + 1, q[rr] - 1));
    }
