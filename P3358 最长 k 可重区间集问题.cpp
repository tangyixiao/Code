...
inline void solve(int Task_Id) {
    ...
    int S = m + 1, T = m + 2;
    vector<tuple<int, int, int, int>> edges; // from,to,cap,cost
    auto add_edge = [&](int u, int v, int cap, int cost) {
        edges.emplace_back(u, v, cap, cost);
        edges.emplace_back(v, u, 0, -cost);
    };
    for (int i = 0; i < m - 1; ++i) {
        add_edge(i + 1, i + 2, k, 0);
    }
    for (int i = 0; i < n; ++i) {
        int u = get_id(l[i]);
        int v = get_id(r[i]);
    ...
    int ecnt = 0;
    for (auto &e : edges) {
        int u = get<0>(e), v = get<1>(e), cap = get<2>(e), cost = get<3>(e);
        G[u].push_back(ecnt++);
        G[v].push_back(ecnt++);
    }
    vector<int> to(ecnt), cap(ecnt), cost(ecnt), rev(ecnt);
    ecnt = 0;
    for (auto &e : edges) {
        int u = get<0>(e), v = get<1>(e), c = get<2>(e), w = get<3>(e);
        to[ecnt] = v;
        cap[ecnt] = c;
        cost[ecnt] = w;
        rev[ecnt] = ecnt + 1;
    ...
    while (true) {
        ...
        while (!q.empty()) {
            ...
            inq[u] = false;
            for (int idx : G[u]) {
                if (cap[idx] > 0 && dist[to[idx]] < dist[u] + cost[idx]) {
                    dist[to[idx]] = dist[u] + cost[idx];
                    prevv[to[idx]] = u;
                    preve[to[idx]] = idx;
                    if (!inq[to[idx]]) {
                        q.push(to[idx]);
