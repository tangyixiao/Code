...
int n, m, a[E][E], p[E][E], pr[E][E], pn[E][E], rk[E][E], rn[E][E], mt[E], mn[E], q[Q], ptr[E];
...
inline void solve(int Task_Id) {
    ...
    for (int i = 1; i <= n; ++i) {
        ...
        sort(pr[i] + 1, pr[i] + n + 1, [&](int x, int y) { return p[i][x] < p[i][y]; });
        pr[i][n + 1] = 0;
    }
    for (int v = 1; v <= n; ++v) {
        iota(pn[v] + 1, pn[v] + n + 1, 1);
        sort(pn[v] + 1, pn[v] + n + 1, [&](int i, int j) { return p[i][v] > p[j][v]; });
        for (int k = 1; k <= n; ++k)
            rn[v][pn[v][k]] = k;
    }
    fill(mt + 1, mt + n + 1, 0);
