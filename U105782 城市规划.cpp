//  Author: Tangyixiao
// Time: 2026-08-29 11:58:41
//  Problem: U105782 城市规划
//  Contest: Luogu - SXYZ 训练赛②
//  URL: https://www.luogu.com.cn/problem/U105782?contestId=353385
//  Memory Limit: 125 MB
//  Time Limit: 2000 ms
//  Interactive: false
//  Test Type: single
//  Batch ID: 9fcfc1a5-f9a6-4baa-8087-2329b3b33737
//
// Algorithm:
// Complexity: O()
// Note:
...
#define FILE_INDEX 1 // the index of the file id the local file system
...
namespace FILE_IO {
const std::string Insuffix = ".id", Outsuffix = ".od", Anssuffix = ".ans";
...
namespace TANGYIXIAO {
const int N = 2e4 + 5;
int n, m, tot, top, tim, dfn[N], low[N], stk[N], c[N], id[N], od[N], a, b;
bool vis[N];
vector<int> g[N];
...
inline void solve(int Task_Id) {
    ...
    for (int i = 1; i <= n; i++) {
        g[i].clear();
        a = b = dfn[i] = low[i] = c[i] = id[i] = od[i] = vis[i] = tot = top = tim = 0;
    }
