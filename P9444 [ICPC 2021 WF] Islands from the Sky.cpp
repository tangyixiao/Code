//  Author: Tangyixiao
// Time: 2026-08-17 23:01:16
//  Problem: P9444 [ICPC 2021 WF] Islands from the Sky
//  Contest: Luogu
//  URL: https://www.luogu.com.cn/problem/P9444
//  Memory Limit: 2048 MB
//  Time Limit: 2000 ms
//  Interactive: false
//  Test Type: single
//  Batch ID: eab4758c-09c3-4eb4-be66-83bb73278406
//
// Algorithm:
// Complexity: O()
// Note:
...
inline void solve(int Task_Id) {
    ...
    for (int i = 0; i < m; i++)
        cin >> l[i].x1 >> l[i].y1 >> l[i].z1 >> l[i].x2 >> l[i].y2 >> l[i].z2;
    ...
    for (int i = 0; i < n; i++) {
        ...
        for (int j = 0; j < m; j++) {
            long long dx = l[j].x2 - l[j].x1;
            long long dy = l[j].y2 - l[j].y1;
            long long q = dx * dx + dy * dy;

            long double r = 0;
            bool ok = 1;

            for (int k = 0; k < s[i]; k++) {
                long long x = p[i][k].x - l[j].x1;
                long long y = p[i][k].y - l[j].y1;

                long long d = x * dx + y * dy;
                ...
                long double t = (long double)d / q;
                long double h = l[j].z1 +
                                (l[j].z2 - l[j].z1) * t;

                long long cr = x * dy - y * dx;
                long double dis = fabsl((long double)cr) / sqrtl((long double)q);

                r = max(r, dis / h);
            }

            if (ok)
                best = min(best, atanl(r));
        }
