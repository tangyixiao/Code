//  Author: Tangyixiao
// Time: 2026-08-29 09:04:04
//  Problem: U105734 搭建温泉
//  Contest: Luogu - SXYZ 训练赛②
//  URL: https://www.luogu.com.cn/problem/U105734?contestId=353385
//  Memory Limit: 125 MB
//  Time Limit: 1000 ms
//  Interactive: false
//  Test Type: single
//  Batch ID: ea28b881-7c48-44ee-a528-028f64dd6eb6
//
// Algorithm:
// Complexity: O()
// Note:
...
inline void solve(int Task_Id) {
    ...
    ans = 2.0 * pi * r;
    for (int i = 1; i <= n; ++i) {
        ans += sqrt((x[i] - x[i == n ? 1 : i + 1]) * (x[i] - x[i == n ? 1 : i + 1]) + (y[i] - y[i == n ? 1 : i + 1]) * (y[i] - y[i == n ? 1 : i + 1]));
    }
