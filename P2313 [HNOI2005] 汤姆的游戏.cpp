//  Author: Tangyixiao
// Time: 2026-08-15 23:25:04
//  Problem: P2313 [HNOI2005] 汤姆的游戏
//  Contest: Luogu
//  URL: https://www.luogu.com.cn/problem/P2313
//  Memory Limit: 125 MB
//  Time Limit: 1000 ms
//  Interactive: false
//  Test Type: single
//  Batch ID: 8c511559-0b9a-4d92-b52e-a7ebbb46ea4f
//
// Algorithm:
// Complexity: O()
// Note:
...
inline void solve(int Task_Id) {
    ...
    for (; m--;) {
        ...
        for (j = 0; j < n; j++) {
            if (t[j] == 'r')
                ans += x > min(a[j], c[j]) && x < max(a[j], c[j]) && y > min(b[j], d[j]) && y < max(b[j], d[j]);
            else
                ans += (x - a[j]) * (x - a[j]) + (y - b[j]) * (y - b[j]) < c[j] * c[j];
        }
        cout << ans << '\n';
    }
    return;
