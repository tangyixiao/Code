//  Author: Tangyixiao
//  Time: 2026-08-14 12:05:07
//  Problem: P4197 [ONTAK2010] Peaks
//  Contest: Luogu
...
//  Interactive: false
//  Test Type: single
//  Batch ID: b149c964-b14b-4da3-b9ee-b905e4dabfde
//
// Algorithm:
// Complexity: O()
// Note:
...
int n, m, q, tot, ct, df;
int h[N], b[N], f[N], w[N], c[N][2];
int fa[N][19], l[N], r[N], a[N], st[N];
int rt[N], ls[N * 5], rs[N * 5], s[N * 5];
...
int ask(int x, int y, int l, int r, int k) {
    if (l == r)
        return l;
    int m = (l + r) >> 1, t = s[rs[y]] - s[rs[x]];
    if (k <= t)
        return ask(rs[x], rs[y], m + 1, r, k);
    return ask(ls[x], ls[y], l, m, k - t);
}
inline void solve(int Task_Id) {
    ...
    for (int i = 1; i <= n; i++) {
        int x = lower_bound(b + 1, b + n + 1, h[a[i]]) - b;
        rt[i] = add(rt[i - 1], 1, n, x);
    }
    ...
    while (q--) {
        ...
        int p = ask(rt[l[v] - 1], rt[r[v]], 1, n, k);
        cout << b[p] << "\n";
    }
