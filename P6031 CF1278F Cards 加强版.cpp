//  Author: Tangyixiao
// Time: 2026-07-10 07:53:54
//  Problem: P6031 CF1278F Cards 加强版
//  Contest: Luogu
//  URL: https://www.luogu.com.cn/problem/P6031
//  Memory Limit: 256 MB
//  Time Limit: 2000 ms
//  Interactive: false
//  Test Type: single
//  Batch ID: 9d184830-4a66-498a-9764-cc46bff77e6f
//
// Algorithm:
// Complexity: O()
// Note:
...
const int M = 998244353, SZ = 10000005;
int f[SZ], d[SZ], iv[SZ];
char vs[SZ];
int prm[SZ / 10];
ll pw(ll a, ll b) {
...
inline void solve(int Task_Id) {
    ...
    for (int i = 2; i <= L; ++i) {
        ...
        for (int j = 0; j < pc && (ll)i * prm[j] <= L; ++j) {
            int p = prm[j], ip = i * p;
            vs[ip] = 1;
            f[ip] = (ll)f[i] * f[p] % M;
            if (i % p == 0)
    ...
    for (int j = L; j >= 1; --j) {
        as = (as + s * f[j] % M * d[j] % M * pp % M * ifv) % M;
        ll ad = cc * ur % M;
        s = (q * s + ad) % M;
        pp = pp * ipb % M;
        ur = ur * u % M;
