...
inline void solve(int Task_Id) {
    ...
    for (int i = M - 1; i >= 0; --i) {
        int L = posters[i].first, R = posters[i].second;
        int covered_len = 0;

        for (auto &seg : covered) {
            int inter_l = max(L, seg.first);
            int inter_r = min(R, seg.second);
            if (inter_l <= inter_r)
                covered_len += inter_r - inter_l + 1;
        }

        if (covered_len < R - L + 1) {
            ++visible;
