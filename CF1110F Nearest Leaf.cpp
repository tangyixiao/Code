...
struct SegTree {
    int n;
    vector<ll> minv, lazy;
    SegTree(int _n) : n(_n), minv(4 * _n), lazy(4 * _n, 0) {}
    ...
    void push(int node) {
        if (lazy[node] != 0) {
            ...
            minv[node * 2] += lazy[node];
            lazy[node * 2 + 1] += lazy[node];
            minv[node * 2 + 1] += lazy[node];
            lazy[node] = 0;
        }
    }
...
inline void solve(int Task_Id) {
    ...
    vector<ll> init(n + 1);
    for (int i = 1; i <= n; ++i) {
        init[i] = is_leaf[i] ? dep[i] : INF;
    }
    SegTree seg(n);
    seg.build(1, 1, n, init);

    vector<vector<Query>> queries_at_node(n + 1);
    for (int i = 0; i < q; ++i) {
        int v, l, r;
        cin >> v >> l >> r;
        queries_at_node[v].push_back({l, r, i});
    }

    vector<ll> ans(q);

    vector<int> stk_u, stk_idx;
    stk_u.push_back(1);
    stk_idx.push_back(0);

    while (!stk_u.empty()) {
        int u = stk_u.back();
        int idx = stk_idx.back();

        if (idx == 0) {
            for (auto &qr : queries_at_node[u]) {
                ans[qr.id] = seg.query(1, 1, n, qr.l, qr.r);
            }
        }
        ...
        if (idx < (int)children[u].size()) {
            ...
            seg.update(1, 1, n, L, R, -weight);
            if (L > 1)
                seg.update(1, 1, n, 1, L - 1, weight);
            if (R < n)
                seg.update(1, 1, n, R + 1, n, weight);
