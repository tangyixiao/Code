//  Author: Tangyixiao
//  Time: 2026-08-14 12:03:31
//  Problem: P5920 [IOI 2005] gar
//  Contest: Luogu
//  URL: https://www.luogu.com.cn/problem/P5920
//  Memory Limit: 125 MB
//  Time Limit: 1000 ms
//  Interactive: false
//  Test Type: single
//  Batch ID: 12a58dc7-aa56-44aa-b8b2-d40e9e3d05f0
//
// Algorithm:
// Complexity: O()
// Note:
...
inline void solve(int Task_Id) {
    ...
    for (int i = 1; i < l; i++)
        if (u[i] < 1e9 && d[i + 1] < 1e9)
            ans = min(ans, u[i] + d[i + 1]);

    for (int i = 1; i < w; i++)
        if (x[i] < 1e9 && y[i + 1] < 1e9)
