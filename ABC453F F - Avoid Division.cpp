...
inline void solve(int Task_Id) {
    ...
    sort(cg2.begin(), cg2.end(), [&](int a, int b) {
        return c[a] > c[b];
    });
    int rem = l;
    for (int i : cg2) {
    ...
    vector<int> leo(n + 1, 0);
    for (int leaf : lo) {
        while (!q.empty()) {
            auto [cnt, col] = q.front();
            q.pop();
            if (cnt > 0) {
                leo[leaf] = col;
                cnt--;
    ...
    for (int i = 1; i <= n; ++i) {
        if (!ilf[i]) {
            while (ptr < (int)ac.size() && rc[ac[ptr]] == 0) {
                ptr++;
            }
            if (ptr == (int)ac.size()) {
                ptr = 0;
