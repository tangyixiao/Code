...
inline void solve(int Task_Id) {
    ...
    vector<long long> suf_max(n + 2, -INF);
    for (int i = n; i >= 1; --i) {
        suf_max[i] = max(suf_max[i + 1], pre[n] - pre[i - 1]);
    }

    for (int j = 1; j < n; ++j) {
        long long left = pre[j];
        long long right = suf_max[j + 1];
        if (right != -INF) {
    ...
    vector<long long> lm(n), rm(n);
    lmax[0] = a[0];
    lm[0] = a[0];
    for (int i = 1; i < n; ++i) {
        lmax[i] = max((long long)a[i], lmax[i - 1] + a[i]);
        lm[i] = max(lm[i - 1], lmax[i]);
    }
    rmax[n - 1] = a[n - 1];
    rm[n - 1] = a[n - 1];
    for (int i = n - 2; i >= 0; --i) {
        rmax[i] = max((long long)a[i], rmax[i + 1] + a[i]);
        rm[i] = max(rm[i + 1], rmax[i]);
    }
