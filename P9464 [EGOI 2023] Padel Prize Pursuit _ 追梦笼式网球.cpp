...
struct SegTree {
    ...
    void build(int node, int l, int r) {
        ...
        color[node] = min(color[node * 2], color[node * 2 + 1]);
    }
    void update(int node, int l, int r, int idx, int val) {
        ...
        if (max_val[left] > max_val[right]) {
            max_val[node] = max_val[left];
            color[node] = color[left];
        } else if (max_val[right] > max_val[left]) {
            max_val[node] = max_val[right];
            color[node] = color[right];
        } else {
            max_val[node] = max_val[left];
            color[node] = min(color[left], color[right]);
        }
    }
...
inline void solve(int Task_Id) {
    ...
    vector<int> parent(M, -1), owner(M), w(M);
    vector<int> root(N, -1);
