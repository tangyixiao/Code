...
//  Interactive: false
//  Test Type: single
//  Batch ID: 37348bda-dc99-4d4a-81f0-a783f0dd1c72
//
// Algorithm:
// Complexity: O()
// Note:
...
const int N = 1005, M = 1000005, MOD1 = 1000000007, MOD2 = 1000000009;
int n, m, tot = 1;
int h[N], to[M], w[M], nx[M], eid[M];
int d1[N], d2[N], pr[N], pre[N];
bool ban[M];
...
inline void solve(int Task_Id) {
    ...
    static int ord[N], cnt1[N], cnt2[N], cnt1_2[N], cnt2_2[N];
    for (int i = 1; i <= n; ++i)
        ord[i] = i;
    sort(ord + 1, ord + n + 1, [](int x, int y) { return d1[x] < d1[y]; });
    ...
    for (int i = 1; i <= n; ++i) {
        ...
        for (int j = h[u]; j; j = nx[j]) {
            if (ban[j])
                continue;
            int v = to[j], ww = w[j];
            if (d1[u] + ww == d1[v] && d1[u] + ww + d2[v] == L) {
    ...
    for (int i = n; i >= 1; --i) {
        ...
        for (int j = h[v]; j; j = nx[j]) {
            ...
            int u = to[j], ww = w[j];
            if (d1[u] + ww == d1[v] && d1[u] + ww + d2[v] == L) {
                cnt2[u] = (cnt2[u] + cnt2[v]) % MOD1;
                cnt2_2[u] = (cnt2_2[u] + cnt2_2[v]) % MOD2;
            }
        }
    }

    static bool crit[M / 2 + 5] = {false};
    for (int i = 2; i <= tot; i += 2) {
        int id = eid[i];
        int u = to[i ^ 1], v = to[i], ww = w[i];
        bool ok = false;
        if (d1[u] + ww + d2[v] == L) {
            if ((1LL * cnt1[u] * cnt2[v]) % MOD1 == cnt1[n] &&
                (1LL * cnt1_2[u] * cnt2_2[v]) % MOD2 == cnt1_2[n])
                ok = true;
        }
        if (!ok && d1[v] + ww + d2[u] == L) {
            if ((1LL * cnt1[v] * cnt2[u]) % MOD1 == cnt1[n] &&
                (1LL * cnt1_2[v] * cnt2_2[u]) % MOD2 == cnt1_2[n])
                ok = true;
        }
        if (ok)
            crit[id] = true;
