...
int f[MAXN], second_f[MAXN];
int pos[MAXN];
int nodes_by_dfn[MAXN];

// Segment tree for MEX during DFS (non-persistent)
int zero_cnt[4 * (MAXN + 2)];

void build_mex(int rt, int l, int r) {
    zero_cnt[rt] = r - l + 1;
    if (l == r)
        return;
    int mid = (l + r) / 2;
    build_mex(rt * 2, l, mid);
    build_mex(rt * 2 + 1, mid + 1, r);
}

void update_mex(int rt, int l, int r, int pos, int val) {
    if (l == r) {
        zero_cnt[rt] = (val == 1 ? 0 : 1);
        return;
    }
    int mid = (l + r) / 2;
    if (pos <= mid)
        update_mex(rt * 2, l, mid, pos, val);
    else
        update_mex(rt * 2 + 1, mid + 1, r, pos, val);
    zero_cnt[rt] = zero_cnt[rt * 2] + zero_cnt[rt * 2 + 1];
}

int query_kth_mex(int rt, int l, int r, int k) {
    if (l == r)
        return l;
    int mid = (l + r) / 2;
    if (zero_cnt[rt * 2] >= k)
        return query_kth_mex(rt * 2, l, mid, k);
    else
        return query_kth_mex(rt * 2 + 1, mid + 1, r, k - zero_cnt[rt * 2]);
}

// Persistent segment tree node pool
struct Node {
    int lc, rc, cnt;
    long long sum;
};
...
int new_node(int lc = 0, int rc = 0, int cnt = 0, long long sum = 0) {
    pool.push_back({lc, rc, cnt, sum});
    return pool.size() - 1;
}

void init_pool() {
    pool.clear();
    new_node(); // node 0 is empty
}

// Tree1: domain [0, n], stores cnt and sum of f
int insert1(int pre, int l, int r, int pos, int cnt_val, long long sum_val) {
    int cur = new_node(pool[pre].lc, pool[pre].rc, pool[pre].cnt + cnt_val, pool[pre].sum + sum_val);
    if (l == r)
        return cur;
    int mid = (l + r) >> 1;
    if (pos <= mid)
        pool[cur].lc = insert1(pool[pre].lc, l, mid, pos, cnt_val, sum_val);
    else
        pool[cur].rc = insert1(pool[pre].rc, mid + 1, r, pos, cnt_val, sum_val);
    return cur;
}

pair<int, long long> query1(int root, int l, int r, int ql, int qr) {
    if (!root || ql > r || qr < l)
        return {0, 0};
    if (ql <= l && r <= qr)
        return {pool[root].cnt, pool[root].sum};
    int mid = (l + r) >> 1;
    auto left = query1(pool[root].lc, l, mid, ql, qr);
    auto right = query1(pool[root].rc, mid + 1, r, ql, qr);
    return {left.first + right.first, left.second + right.second};
}

pair<int, long long> range_query1(int rootL, int rootR, int l, int r, int ql, int qr) {
    auto resR = query1(rootR, l, r, ql, qr);
    auto resL = query1(rootL, l, r, ql, qr);
    return {resR.first - resL.first, resR.second - resL.second};
}

// Tree2: domain [0, n+1], stores cnt and sum of second_f
int insert2(int pre, int l, int r, int pos, int cnt_val, long long sum_val) {
    int cur = new_node(pool[pre].lc, pool[pre].rc, pool[pre].cnt + cnt_val, pool[pre].sum + sum_val);
    if (l == r)
        return cur;
    int mid = (l + r) >> 1;
    if (pos <= mid)
        pool[cur].lc = insert2(pool[pre].lc, l, mid, pos, cnt_val, sum_val);
    else
        pool[cur].rc = insert2(pool[pre].rc, mid + 1, r, pos, cnt_val, sum_val);
    return cur;
}

pair<int, long long> query2(int root, int l, int r, int ql, int qr) {
    if (!root || ql > r || qr < l)
        return {0, 0};
    if (ql <= l && r <= qr)
        return {pool[root].cnt, pool[root].sum};
    int mid = (l + r) >> 1;
    auto left = query2(pool[root].lc, l, mid, ql, qr);
    auto right = query2(pool[root].rc, mid + 1, r, ql, qr);
    return {left.first + right.first, left.second + right.second};
}

int root1[MAXN];
vector<int> bucket[MAXN];
vector<int> roots_x[MAXN];
...
inline void solve(int Task_Id) {
    ...
    function<void(int, int)> dfs = [&](int u, int fa) {
        dfn[u] = ++timer;
        nodes_by_dfn[timer] = u;
        sz[u] = 1;
        update_mex(1, 0, n + 1, p[u], 1);
        f[u] = query_kth_mex(1, 0, n + 1, 1);
        if (zero_cnt[1] >= 2)
            second_f[u] = query_kth_mex(1, 0, n + 1, 2);
        else
            second_f[u] = n + 1;
        for (int v : adj[u]) {
            if (v == fa)
    ...
    for (int i = 1; i <= n; ++i) {
        int u = nodes_by_dfn[i];
        bucket[f[u]].push_back(u);
    }
    ...
    for (int i = 1; i <= n; ++i) {
        int u = nodes_by_dfn[i];
        root1[i] = insert1(root1[i - 1], 0, n, f[u], 1, f[u]);
    }

    for (int x = 0; x <= n; ++x) {
        if (bucket[x].empty())
            continue;
        roots_x[x].resize(bucket[x].size() + 1);
        roots_x[x][0] = 0;
        for (size_t j = 0; j < bucket[x].size(); ++j) {
            int u = bucket[x][j];
            roots_x[x][j + 1] = insert2(roots_x[x][j], 0, n + 1, second_f[u], 1, second_f[u]);
        }
    }
    ...
    for (int v = 1; v <= n; ++v) {
        int x = f[v], pv = p[v];
        int V_l = dfn[v], V_r = dfn[v] + sz[v] - 1;
        int posx = (x < n ? pos[x] : 0);
        bool inV = false;
        int R1_l = 0, R1_r = -1;
        if (posx != 0 && dfn[v] <= dfn[posx] && dfn[posx] <= V_r) {
            inV = true;
            R1_l = dfn[posx];
            R1_r = dfn[posx] + sz[posx] - 1;
        }

        long long delta = 0;
        if (inV) {
            auto res1 = range_query1(root1[R1_l - 1], root1[R1_r], 0, n, pv + 1, n);
            int cnt1 = res1.first;
            long long sum1 = res1.second;
            delta += 1LL * pv * cnt1 - sum1;
        }

        vector<pair<int, int>> R2_segs;
        if (inV) {
            if (V_l <= R1_l - 1)
                R2_segs.push_back({V_l, R1_l - 1});
            if (R1_r + 1 <= V_r)
                R2_segs.push_back({R1_r + 1, V_r});
        } else {
            R2_segs.push_back({V_l, V_r});
        }

        if (!bucket[x].empty()) {
            for (auto &seg : R2_segs) {
                int L = seg.first, R = seg.second;
                if (L > R)
                    continue;
                auto it1 = lower_bound(bucket[x].begin(), bucket[x].end(), L,
                                       [&](int u, int val) { return dfn[u] < val; });
                auto it2 = upper_bound(bucket[x].begin(), bucket[x].end(), R,
                                       [&](int val, int u) { return val < dfn[u]; });
                int idx_l = it1 - bucket[x].begin();
                int idx_r = it2 - bucket[x].begin();
                if (idx_l >= idx_r)
                    continue;
                int ver_l = (idx_l > 0 ? roots_x[x][idx_l] : 0);
                int ver_r = roots_x[x][idx_r];
                int cnt2 = pool[ver_r].cnt - pool[ver_l].cnt;
                if (cnt2 == 0)
                    continue;
                if (pv < x) {
                    delta += 1LL * cnt2 * (pv - x);
                } else {
                    auto resR = query2(ver_r, 0, n + 1, 0, pv);
                    auto resL = query2(ver_l, 0, n + 1, 0, pv);
                    int cnt_le = resR.first - resL.first;
                    long long sum_le = resR.second - resL.second;
                    int cnt_gt = cnt2 - cnt_le;
                    delta += sum_le + 1LL * pv * cnt_gt - 1LL * cnt2 * x;
                }
            }
        }
        if (delta > max_delta)
