//  Author: Tangyixiao
//  Time: 2026-08-16 07:48:52
//  Problem: P2900 [USACO08MAR] Land Acquisition G
//  Contest: Luogu
//  URL: https://www.luogu.com.cn/problem/P2900
//  Memory Limit: 128 MB
//  Time Limit: 500 ms
//  Interactive: false
//  Test Type: single
//  Batch ID: d22664fd-98b5-4a31-99a4-dd528789ac70
//
// Algorithm:
// Complexity: O()
// Note:
...
ll d[N];
int q[N], h, t;
bool cmp(R x, R y) { return x.w < y.w || (x.w == y.w && x.l > y.l); }
bool le(int j, int k, ll w) {
    return d[k] - d[j] <= w * (b[j + 1].l - b[k + 1].l);
}
bool ge(int j, int k, int i) {
    return (d[k] - d[j]) * (b[k + 1].l - b[i + 1].l) >= (d[i] - d[k]) * (b[j + 1].l - b[k + 1].l);
}
inline void solve(int Task_Id) {
    scanf("%d", &n);
    for (int i = 0; i < n; ++i)
    ...
    q[0] = 0;
    for (int i = 1; i <= m; ++i) {
        while (h < t && le(q[h], q[h + 1], b[i].w))
            ++h;
        int j = q[h];
        d[i] = d[j] + (ll)b[i].w * b[j + 1].l;
        if (i < m) {
            while (h < t && ge(q[t - 1], q[t], i))
                --t;
            q[++t] = i;
        }
    }
