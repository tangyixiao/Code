...
namespace TANGYIXIAO {
const int N = 1e5 + 5;
int n, m, d, head[N], nxt[N], l[N], r[N], pre[N];
int te[N], tv[N], p[N], pe[N], pos[N], cut[N];
ll w[N], f[N], g[N], tr[N << 2], rep[N];
map<pair<int, int>, int> id;

inline void add(int x) {
    nxt[x] = head[l[x]], head[l[x]] = x;
    return;
}
...
inline void collect(int o, int L, int R, ll v) {
    chmax(v, tr[o]);
    if (L == R) {
        rep[L] = v;
        return;
    ...
    int mid = (L + R) >> 1;
    collect(o << 1, L, mid, v), collect(o << 1 | 1, mid + 1, R, v);
    return;
}
...
inline void solve(int Task_Id) {
    ...
    for (int j = cnt; j >= 1; j--) {
        int k = cnt - j + 1;
        pe[k] = te[j], p[k + 1] = tv[j], cut[p[k]] = k;
        if (pe[k]) {
            pos[pe[k]] = k;
        }
    }
    ...
    for (int e = 1; e <= m; e++) {
        ...
        int ql = lower_bound(p + 1, p + cnt + 1, l[e]) - p;
        int qr = lower_bound(p + 1, p + cnt + 1, r[e]) - p - 1;
        if (ql <= qr) {
            modify(1, 1, cnt, ql, qr, f[l[e]] + w[e] + g[r[e]]);
        }
    }

    for (int i = 1; i < n; i++) {
        if (cut[i]) {
            modify(1, 1, cnt, cut[i], cut[i], f[i] + g[i + 1]);
        }
    }
    collect(1, 1, cnt, 0);
    ...
    for (int i = 1; i <= d; i++) {
        ...
        cin >> x >> y >> v;
        auto it = id.find({x, y});
        if (it != id.end() && pos[it->second]) {
            ans = rep[pos[it->second]];
        }
        chmax(ans, f[x] + v + g[y]);
        cout << ans << "\n";
    }
