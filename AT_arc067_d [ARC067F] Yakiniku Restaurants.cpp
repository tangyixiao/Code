...
void ad(int p, int l, int r, int ql, int qr, ll v) {
    if (ql <= l && r <= qr) {
        mx[p] += v;
        tg[p] += v;
        return;
    ...
    if (ql <= mid)
        ad(p << 1, l, mid, ql, qr, v);
    if (qr > mid)
        ad(p << 1 | 1, mid + 1, r, ql, qr, v);
    mx[p] = max(mx[p << 1], mx[p << 1 | 1]);
}
ll qr(int p, int l, int r, int ql, int qrr) {
    if (ql <= l && r <= qrr)
        return mx[p];
    int mid = (l + r) / 2;
    if (tg[p]) {
    ...
    ll res = -1e18;
    if (ql <= mid)
        res = max(res, qr(p << 1, l, mid, ql, qrr));
    if (qrr > mid)
        res = max(res, qr(p << 1 | 1, mid + 1, r, ql, qrr));
    return res;
}
inline void solve(int Task_Id) {
    cin >> n >> m;
    ...
    for (int R = 1; R <= n; ++R) {
        for (int j = 1; j <= m; ++j) {
            ...
            int &tp = sp[j];
            int new_l = R;
            while (tp > 0 && sv[j][tp] <= v) {
                int val = sv[j][tp];
                int l = sl[j][tp];
                --tp;
                if (l <= cur_R)
                    ad(1, 1, n, l, cur_R, v - val);
                new_l = l;
                cur_R = l - 1;
            }
            ++tp;
