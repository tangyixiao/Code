//  Author: Tangyixiao
//  Time: 2026-08-13 07:14:59
//  Problem: P15836 [蓝桥杯第一届国际赛] 捕鱼达人
//  Contest: Luogu
//  URL: https://www.luogu.com.cn/problem/P15836
//  Memory Limit: 512 MB
//  Time Limit: 1000 ms
//  Interactive: false
//  Test Type: single
//  Batch ID: 206ca2b8-4576-4835-866e-4da30241418d
//
// Algorithm:
// Complexity: O()
// Note:
...
unordered_map<ull, unsigned char, H> f;
...
int gd(ull s) {
    ...
    while (s) {
        ...
        while (t) {
            ...
            t &= t - 1;
            int d = __builtin_popcountll(a[u] & s);
            if (d > m)
                m = d, p = u;
        }
        s &= ~a[p];
...
int lb(ull s) {
    ...
    int k = __builtin_popcountll(s), m = 1;
    ull t = s;

    while (t) {
        int u = __builtin_ctzll(t);
        t &= t - 1;
        m = max(m, __builtin_popcountll(a[u] & s));
    }
    ...
    while (q) {
        ...
        while (t) {
            int u = __builtin_ctzll(t);
            t &= t - 1;
            int z = __builtin_popcountll(a[u] & s);
            if (z < d)
                d = z, v = u;
        }
...
inline int dfs(ull s) {
    ...
    sort(b, b + bc, [&](int u, int v) {
        return __builtin_popcountll(a[u] & s) >
               __builtin_popcountll(a[v] & s);
    });
