...
void up(int u, int l, int r, int ql, int qr) {
    if (ql <= l && r <= qr) {
        --mn[u];
        ++lz[u];
        return;
    ...
    if (ql <= m)
        up(u << 1, l, m, ql, qr);
    if (qr > m)
        up(u << 1 | 1, m + 1, r, ql, qr);
    mn[u] = min(mn[u << 1], mn[u << 1 | 1]);
}

int qr(int u, int l, int r, int ql, int qrr) {
    if (ql <= l && r <= qrr)
        return mn[u];
    ps(u);
    int m = (l + r) >> 1, res = 2e9;
    if (ql <= m)
        res = min(res, qr(u << 1, l, m, ql, qrr));
    if (qrr > m)
        res = min(res, qr(u << 1 | 1, m + 1, r, ql, qrr));
    return res;
}
...
inline void solve(int Task_Id) {
    ...
    pair<int, int> tmp[M];
    for (int i = 0; i < m; ++i)
        tmp[i].first = rd(), tmp[i].second = rd();
    if (!m) {
        printf("%d\n", n);
        return;
    }
    ...
    tot = 0;
    for (int i = 0; i < m;) {
        int cp = tmp[i].first, cx = tmp[i].second;
        ++i;
        while (i < m && tmp[i].first == cp)
            cx = min(cx, tmp[i].second), ++i;
        p[tot] = cp;
        c[tot] = cx;
        ++tot;
    }
    bd(1, 0, tot - 1);
    sort(s, s + n, [](const S &a, const S &b) { return a.r < b.r; });
    int ans = 0;
    for (int i = 0; i < n; ++i) {
        int L = lower_bound(p, p + tot, s[i].l) - p;
        int R = upper_bound(p, p + tot, s[i].r) - p - 1;
        if (L > R)
            ++ans;
        else if (qr(1, 0, tot - 1, L, R) >= 1) {
            ++ans;
            up(1, 0, tot - 1, L, R);
        }
    }
