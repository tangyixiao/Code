...
void add_edge(int p, int l, int r, int ql, int qr, const Edge &e) {
    if (ql <= l && r <= qr) {
        tree[p].push_back(e);
        return;
    }
    int mid = (l + r) >> 1;
    if (ql <= mid)
        add_edge(p << 1, l, mid, ql, qr, e);
    if (qr > mid)
        add_edge(p << 1 | 1, mid + 1, r, ql, qr, e);
}
...
inline void solve(int Task_Id) {
    ...
    for (int i = 1; i <= m; ++i) {
        ...
        cin >> x >> y >> l >> r;
        if (l + 1 <= r) {
            add_edge(1, 1, k, l + 1, r, {x, y});
        }
    }

    dsu.init();
    fill(ans + 1, ans + k + 1, false);
    dfs(1, 1, k);
