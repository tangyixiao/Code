...
struct State {
    ...
    bool operator==(const State &o) const {
        return step == o.step && cur == o.cur && last_player == o.last_player;
    }
};

struct StateHash {
    size_t operator()(const State &s) const {
        return ((size_t)s.step << 40) ^ ((size_t)s.cur << 20) ^ (size_t)s.last_player;
    }
};
...
bool dfs(int step, int cur, int last_player) {
    if (step == 1) {
        for (auto &[p, idx] : pos[cur]) {
            if (p == last_player)
                continue;
            int L = max(0, idx - k + 1);
            for (int j = L; j < idx; ++j) {
...
inline void solve(int Task_Id) {
    ...
    // 构建每个数字作为结尾时的出现位置（开头至少有一个元素，故从下标 1 开始）
    pos.assign(max_val + 1, vector<pair<int, int>>());
    for (int i = 0; i < n; ++i) {
