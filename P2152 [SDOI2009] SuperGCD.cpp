//  Author: Tangyixiao
//  Time: 2026-08-13 11:10:16
//  Problem: P2152 [SDOI2009] SuperGCD
//  Contest: Luogu
//  URL: https://www.luogu.com.cn/problem/P2152
//  Memory Limit: 128 MB
//  Time Limit: 1000 ms
//  Interactive: false
//  Test Type: single
//  Batch ID: 214388d6-d1ba-40d8-a7f3-a50d6670aafb
//
// Algorithm:
// Complexity: O()
// Note:
...
struct B {
    ...
    void shr(int k) {
        ...
        if (!r) {
            ...
            for (int i = 0; i < m; i++) {
                uint64_t v = (uint64_t)a[i + q] >> r;
                if (i + q + 1 < n)
                    v |= (uint64_t)a[i + q + 1] << (32 - r);
                a[i] = (uint32_t)v;
            }
        }
    ...
    void sub(const B &b) {
        ...
        for (int i = 0; i < n; i++) {
            uint64_t x = a[i];
            uint64_t y = (i < b.n ? b.a[i] : 0) + c;

            if (x < y) {
                a[i] = (uint32_t)((1ULL << 32) + x - y);
                c = 1;
            } else {
                a[i] = (uint32_t)(x - y);
                c = 0;
    ...
    void out() const {
        ...
        static uint32_t s[1200];
        int m = 0;

        while (!x.z())
            s[m++] = x.div(1000000000);
