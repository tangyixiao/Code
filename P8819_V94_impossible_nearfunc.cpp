...
namespace TANGYIXIAO {
    ...
    constexpr uint64_t EM=(1ULL<<19)-1;
    constexpr uint64_t IM=(1ULL<<12)-1;
    constexpr int OS=19,IS=38,BS=50;
    constexpr uint64_t BB=1ULL<<BS;

    inline int vhead(int v) { return (int)(se[v]&EM); }
    inline int vbeg(int v) { return (int)((se[v]>>OS)&EM); }
    inline int vid(int v) { return (int)((se[v]>>IS)&IM); }
    inline int vbase(int v) { return (int)((se[v]>>BS)&1); }

    inline uint64_t *elist() {
        return sq;
    }

    inline void add_ex(int e,int u,int v) {
        uint64_t z=se[v];
        int p=(int)(z&EM)+1;
        int b=(int)((z>>OS)&EM);
        elist()[b+p-1]=((uint64_t)u<<19)|(uint64_t)e;
        tmp[e]=p;
        se[v]=z+1;
    }

    inline void del_ex(int e,int v) {
        uint64_t z=se[v];
        int p=tmp[e];
        int c=(int)(z&EM);
        int b=(int)((z>>OS)&EM);
    ...
    inline void direct_add(int u,int d) {
        int x=val[u];
    ...
    inline void pair_add(int u,int d) {
        int x=val[u];
    ...
    __attribute__((noinline))
    void run_reduced(int NC) {
        E=m;
        P=0;

        memset(val,0,1LL*(n+1)*sizeof(int));
    ...
    void solve() {
        ...
        bool func=false;
        bool near=(n>=400000&&m>n&&m-n<=32768);
        int fw=0,NC=0,cov=0;
        ...
        if (m<=n) {
            ...
            for (int i=1;i<=m;i++) {
                int u=io.read();
                int v=io.read();
                sq[i]=((uint64_t)u<<19)|(uint64_t)v;
                ind[v]++;

                if (func) {
                    uint64_t b=1ULL<<(u&63);
                    uint64_t &z=se[u>>6];
        ...
        if (func) {
            q=io.read();
            E=m;
            memcpy(val,ind,1LL*(n+1)*sizeof(int));
        ...
        if (fw) {
            memset(se,0,1LL*(fw+1)*sizeof(uint64_t));
        }
        ...
        bool dpath=(m<=10000&&q<=1000)||!Q2||!Q4;
        bool rpath=false;

        if (!dpath&&Q2&&Q4) {
            long long sc=0;
            long long sc2=near?0:m+1LL;

            for (int i=1;i<=q&&(sc<=m||sc2<=m);i++) {
                uint64_t x=qop[i];
                int op=x&3;
