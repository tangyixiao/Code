...
inline void solve(int Task_Id) {
    ...
    auto KM = [&](int w[55][55]) -> long long {
        const long long INF = 1e18;
        long long lx[55], ly[55];
        int matchR[55];
            ...
            while (true) {
                ...
                std::function<bool(int)> dfs = [&](int x) -> bool {
                    ...
                    for (int y = 1; y <= n; ++y) {
                        if (visR[y])
                            continue;
                        long long gap = lx[x] + ly[y] - w[x][y];
                        if (gap == 0) {
                            visR[y] = true;
                            if (matchR[y] == 0 || dfs(matchR[y])) {
                                matchR[y] = x;
                                return true;
                            }
                        } else if (gap < slack[y])
    ...
    long long min_ans = -KM(w1);
    long long max_ans = KM(w2);
    std::cout << min_ans << '\n'
              << max_ans << '\n';
}
} // namespace TANGYIXIAO