//  Author: Tangyixiao
//  Time: 2026-08-27 07:15:48
//  Problem: P10088 [ROIR 2022] n 巧板 (Day 1)
//  Contest: Luogu
//  URL: https://www.luogu.com.cn/problem/P10088
//  Memory Limit: 128 MB
//  Time Limit: 1000 ms
//  Interactive: false
//  Test Type: single
//  Batch ID: 1c37c739-6ce0-418d-b418-43b5796fd20e
//
// Algorithm:
// Complexity: O()
// Note:
...
bool ck(int c, int x, int y, int z) {
    ...
    for (int i = 0; i < 3; i++) {
        ...
        for (int j = 0; j < 3; j++) {
            ...
            if (fabsl(a[c][1] + a[y][j] + a[x][i2] - PI) > E) {
                continue;
            }
            ...
            for (int k = 0; k < 3; k++) {
                ...
                if (fabsl(a[c][0] + a[x][i] + a[z][k2] - PI) > E) {
                    continue;
                }

                if (fabsl(a[c][2] + a[z][k] + a[y][j2] - PI) > E) {
                    continue;
                }
...
inline void solve(int Task_Id) {
    ...
    for (int i = 0; i < n; i++) {
        ...
        for (int j = 0; j < 3; j++) {
            for (int k = 0; k < 3; k++) {
                long long x = p[i][j].x - p[i][k].x;
                long long y = p[i][j].y - p[i][k].y;
                d[i][j][k] = x * x + y * y;
            }
        }
        for (int j = 0; j < 3; j++) {
            ...
            long long x1 = p[i][x].x - p[i][j].x;
            long long y1 = p[i][x].y - p[i][j].y;
            long long x2 = p[i][y].x - p[i][j].x;
            long long y2 = p[i][y].y - p[i][j].y;

            long double cr = (long double)x1 * y2 - (long double)y1 * x2;
            long double dt = (long double)x1 * x2 + (long double)y1 * y2;

            a[i][j] = atan2l(fabsl(cr), dt);
        }
    }
