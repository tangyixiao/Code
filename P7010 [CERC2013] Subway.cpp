...
namespace TANGYIXIAO {
const int K = 2e6 + 5, P = 3e5 + 5;
int nt[K], st[K], hd[P], sn[K], bt[K];
bool vs[K];
int q[K], up[P];
int nv[P], cnt[P];
int ls[100005], ll[100005], as[1000005];
inline void solve(int Task_Id) {
    int T;
    cin >> T;
    ...
    while (T--) {
        ...
        for (int i = 0; i < L; ++i) {
            while (getline(cin, line) && line.empty())
                ;
            size_t p = line.find("route:");
            stringstream ss(line.substr(p + 6));
            string t;
            ls[i] = ta;
            while (getline(ss, t, ',')) {
                size_t b = t.find_first_not_of(' ');
        ...
        string jline, mline;
        while (getline(cin, jline) && jline.empty())
            ;
        while (getline(cin, mline) && mline.empty())
            ;
        auto gs = [](const string &s) -> string {
            size_t p = s.find(" at ");
            return s.substr(p + 4);
        };
        string sstart = gs(jline), send = gs(mline);
        int start = id[sstart], target = id[send];
        ...
        memset(vs, 0, sizeof(bool) * V);
        memset(nv, -1, sizeof(int) * N);
        int ans_trans = -1, ans_time = -1, qh = 0, qt = 0, up_cnt = 0;
        for (int u = hd[start]; u != -1; u = sn[u])
            if (!vs[u]) {
                vs[u] = 1;
                bt[u] = 0;
                q[qt++] = u;
        ...
        while (1) {
            while (qh < qt) {
                int u = q[qh++], t = bt[u], s = st[u];
                if (s == target) {
                    if (ans_trans == -1)
                        ans_trans = cur_trans;
                    if (cur_trans == ans_trans)
            ...
            for (int i = 0; i < up_cnt; ++i) {
                int s = up[i], btime = nv[s];
                for (int u = hd[s]; u != -1; u = sn[u])
                    if (!vs[u]) {
                        vs[u] = 1;
                        bt[u] = btime;
                        q[qt++] = u;
        ...
        int lines = ans_trans + 1;
        cout << "optimal travel from " << sstart << " to " << send << ": " << lines << " line" << (lines != 1 ? "s" : "") << ", " << ans_time << " minute" << (ans_time != 1 ? "s" : "") << '\n';
    }
    return;
}
} // namespace TANGYIXIAO
