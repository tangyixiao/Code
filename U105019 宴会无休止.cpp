//  Author: Tangyixiao
//  Time: 2026-08-24 07:30:27
//  Problem: U105019 宴会无休止
//  Contest: Luogu - SXYZ 训练赛①
//  URL: https://www.luogu.com.cn/problem/U105019?contestId=352491
//  Memory Limit: 125 MB
//  Time Limit: 1000 ms
//  Interactive: false
//  Test Type: single
//  Batch ID: 16868e19-47b7-46ab-a713-45bdaf418cdd
//
// Algorithm:
// Complexity: O()
// Note:
...
namespace TANGYIXIAO {
const int N = 1e6 + 5, mon[] = {0, 31, 60, 91, 121, 152, 182, 213, 244, 274, 305, 335, 366}, ss = 86400;
struct node {
    int m, d;
...
inline void solve(int Task_Id) {
    ...
    sort(a + 1, a + 1 + n);
    s[1] = mon[a[1].m - 1] + a[1].d - 1, s[n + 1] = mon[12] - mon[a[n].m - 1] - a[n].d;
    for (int i = 2; i <= n; i++) {
        s[i] = mon[a[i].m - 1] - mon[a[i - 1].m - 1] + a[i].d - a[i - 1].d - 1;
    }
    sort(s + 1, s + 2 + n);
    cout << round(s[n + 1] / 366.0 * ss);
    return;
}
} // namespace TANGYIXIAO
