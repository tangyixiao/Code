...
void bit_add(int pos, int val, int n) {
...
inline void solve(int Task_Id) {
    ...
    startR[1] = 0;
    for (int r = 2; r <= N; ++r) {
        startR[r] = startR[r - 1] + cntR[r - 1];
    }
    startR[N + 1] = startR[N] + cntR[N];
    ...
    for (int r = 1; r <= N; ++r) {
        ...
        int len = cntR[r];
        if (len > 0) {
            sort(lefts + st, lefts + st + len);
        }
    }

    sort(ine, ine + M,
         [](const ikun &a, const ikun &b) { return a.R < b.R; });
    ...
    for (int qi = 0; qi < Q; ++qi) {
        ...
        if (rMax == 0) {
            ...
            auto it = lower_bound(lefts + st, lefts + st + len, S);
            if (it == lefts + st + len) {
                ans[idx] = false;
            } else {
                int minL = *it;
