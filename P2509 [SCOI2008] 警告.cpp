...
vector<string> lines;
vector<StmtType> lineType;
int n;
vector<int> elseOfIf, endOfIf;
...
Value parseValue(const string &tok) {
    Value v;
    if (isalpha(tok[0])) {
        v.isVar = true;
        v.var = tok[0];
...
void addUseVarsFromValue(set<char> &use, const Value &v) {
    if (v.isVar)
        use.insert(v.var);
}
inline void solve(int Task_Id) {
    ...
    lineType.resize(n + 1);
    elseOfIf.assign(n + 1, -1);
    endOfIf.assign(n + 1, -1);
    ...
    for (int i = 1; i <= n; ++i) {
        if (lineType[i] == IF) {
            ifStack.push(i);
        } else if (lineType[i] == ELSE) {
            int ifLine = ifStack.top();
            elseOfIf[ifLine] = i;
        } else if (lineType[i] == ENDIF) {
            int ifLine = ifStack.top();
            ifStack.pop();
            endOfIf[ifLine] = i;
        }
    ...
    vector<ExecNode> nodes;
    map<int, int> lineToIdx;

    set<char> paramVars;
    {
        vector<string> toks = split(lines[1]);
        for (size_t j = 1; j < toks.size(); ++j) {
            paramVars.insert(toks[j][0]);
        }
        ExecNode node;
        node.line = 1;
        node.type = HEAD;
    ...
    for (int i = 2; i <= n; ++i) {
        if (lineType[i] == ASSIGN || lineType[i] == IF || lineType[i] == RETURN) {
            ExecNode node;
            node.line = i;
            node.type = lineType[i];
            node.def = 0;
            string s = lines[i];
            vector<string> toks = split(s);

            if (lineType[i] == ASSIGN) {
                node.def = toks[0][0];
                Value v1 = parseValue(toks[2]);
                addUseVarsFromValue(node.use, v1);
                if (toks.size() > 3) {
                    Value v2 = parseValue(toks[4]);
                    addUseVarsFromValue(node.use, v2);
                }
            } else if (lineType[i] == IF) {
                // IF A < 5 THEN
                node.use.insert(toks[1][0]);
                addUseVarsFromValue(node.use, parseValue(toks[3]));
            } else if (lineType[i] == RETURN) {
                addUseVarsFromValue(node.use, parseValue(toks[1]));
            }

            nodes.push_back(node);
            lineToIdx[i] = nodes.size() - 1;
        }
    }

    auto nextExecLine = [&](int startLine) -> int {
    ...
    for (int idx = 0; idx < nodeCnt; ++idx) {
        ExecNode &node = nodes[idx];
        int ln = node.line;
        if (node.type == RETURN)
            continue;
        if (node.type == HEAD) {
            int nxt = nextExecLine(1);
            if (nxt != -1) {
                int nxtIdx = lineToIdx[nxt];
                node.succ.push_back(nxtIdx);
            }
            continue;
        }
        if (node.type == ASSIGN) {
            int nxtExec = nextExecLine(ln);
            if (nxtExec == -1)
                continue;
            bool jumpOver = false;
            for (int k = ln + 1; k < nxtExec; ++k) {
                if (lineType[k] == ELSE || lineType[k] == ENDIF) {
                    jumpOver = true;
                    break;
                }
            }
            if (!jumpOver) {
                int nxtIdx = lineToIdx[nxtExec];
                node.succ.push_back(nxtIdx);
            } else {
                int targetLine = -1;
                for (int ifLine = ln - 1; ifLine >= 1; --ifLine) {
                    if (lineType[ifLine] == IF) {
                        int endLn = endOfIf[ifLine];
                        if (endLn > ln) {
                            targetLine = endLn;
                            break;
                        }
                ...
                if (targetLine != -1) {
                    int afterEnd = nextExecLine(targetLine);
                    if (afterEnd != -1) {
                        int afterIdx = lineToIdx[afterEnd];
                        node.succ.push_back(afterIdx);
                    }
                }
            }
            continue;
        ...
        if (node.type == IF) {
            int ifLine = ln;
            int elseLn = elseOfIf[ifLine];
            int endLn = endOfIf[ifLine];

            int thenStart = -1;
            for (int k = ifLine + 1; k < (elseLn != -1 ? elseLn : endLn); ++k) {
                if (lineType[k] == ASSIGN || lineType[k] == IF || lineType[k] == RETURN) {
                    thenStart = k;
                    break;
                }
            }
            if (thenStart != -1) {
                node.succ.push_back(lineToIdx[thenStart]);
            } else {
                int afterEnd = nextExecLine(endLn);
                if (afterEnd != -1)
                    node.succ.push_back(lineToIdx[afterEnd]);
            }

            if (elseLn != -1) {
                int elseStart = -1;
                for (int k = elseLn + 1; k < endLn; ++k) {
                    if (lineType[k] == ASSIGN || lineType[k] == IF || lineType[k] == RETURN) {
                        elseStart = k;
                        break;
                    }
                ...
                if (elseStart != -1) {
                    node.succ.push_back(lineToIdx[elseStart]);
                } else {
                    int afterEnd = nextExecLine(endLn);
                    if (afterEnd != -1)
                        node.succ.push_back(lineToIdx[afterEnd]);
    ...
    for (int i = 0; i < nodeCnt; ++i) {
        for (int succ : nodes[i].succ) {
            nodes[succ].pred.push_back(i);
        }
    }

    vector<bool> reachable(nodeCnt, false);
    queue<int> q;
    reachable[0] = true;
    q.push(0);
    while (!q.empty()) {
    ...
    auto varMask = [](char c) -> unsigned {
        return 1u << (c - 'A');
    };
    unsigned paramMask = 0;
    for (char c : paramVars)
        paramMask |= varMask(c);

    vector<unsigned> IN(nodeCnt, 0), OUT(nodeCnt, 0);
    IN[0] = paramMask;
    OUT[0] = paramMask;
    ...
    vector<Warning> warns;

    for (int idx = 1; idx < nodeCnt; ++idx) {
        ExecNode &node = nodes[idx];
        int ln = node.line;
        if (!reachable[idx]) {
            warns.push_back({ln, "unreachable code"});
        } else {
            set<char> uninitVars;
            unsigned inMask = IN[idx];
            for (char var : node.use) {
                if (!(inMask & varMask(var))) {
                    uninitVars.insert(var);
                }
            }
            for (char var : uninitVars) {
                warns.push_back({ln, "variable " + string(1, var) + " might not have been initialized"});
            }
        }
    }
