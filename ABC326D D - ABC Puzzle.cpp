...
inline void solve(int Task_Id) {
    ...
    string R, C;
    cin >> N >> R >> C;
    vector<int> row_req(N), col_req(N);
    for (int i = 0; i < N; ++i) {
        row_req[i] = (R[i] == 'A' ? 1 : (R[i] == 'B' ? 2 : 3));
        col_req[i] = (C[i] == 'A' ? 1 : (C[i] == 'B' ? 2 : 3));
    }
    vector<vector<vector<int>>> row_ways(N);
    for (int i = 0; i < N; ++i) {
        vector<int> pos;
        function<void(int, int)> dfs_pos = [&](int start, int cnt) {
            if (cnt == 3) {
                vector<int> chars = {1, 2, 3};
                do {
                    vector<int> row(N, 0);
                    for (int k = 0; k < 3; ++k)
                        row[pos[k]] = chars[k];
                    ...
                    if (row[leftmost] == row_req[i])
                        row_ways[i].push_back(row);
    ...
    vector<int> col_cnt[4];
    for (int ch = 1; ch <= 3; ++ch)
        col_cnt[ch].assign(N, 0);
    vector<bool> col_first_set(N, false);
    bool found = false;
        ...
        for (auto &way : row_ways[r]) {
            ...
            for (int c = 0; c < N; ++c) {
                ...
                if (ch == 0)
                    continue;
                col_cnt[ch][c]--;
                if (col_cnt[ch][c] == 0 && col_first_set[c]) {
                    bool any = false;
                    for (int cc = 1; cc <= 3; ++cc)
                        if (col_cnt[cc][c] > 0)
                            any = true;
                    if (!any)
                        col_first_set[c] = false;
                }
                grid[r][c] = 0;
            }
        }
