//  Author: Tangyixiao
//  Time: 2026-08-18 15:15:06
//  Problem: P4400 [JSOI2008] Blue Mary的旅行
//  Contest: Luogu
//  URL: https://www.luogu.com.cn/problem/P4400
//  Memory Limit: 125 MB
//  Time Limit: 1000 ms
//  Interactive: false
//  Test Type: single
//  Batch ID: c6b0bef9-5f0c-472c-8c46-0370fe1b376c
//
// Algorithm:
// Complexity: O()
// Note:
...
namespace TANGYIXIAO {
const int N = 6005, M = 600005, INF = 1e9;
int n, m, k, S, T, ec = 1, h[N], to[M], nx[M], w[M];
int d[N], cur[N], q[N];
int x[2505], y[2505], z[2505];

inline int id(int t, int x) {
    return t * n + x;
}
...
inline int dfs(int x, int f) {
    ...
    int res = 0;
    for (int &i = cur[x]; i && f; i = nx[i]) {
        int y = to[i];
        if (w[i] && d[y] == d[x] + 1) {
            int v = dfs(y, min(f, w[i]));
            if (v) {
                w[i] -= v;
                w[i ^ 1] += v;
                f -= v;
...
inline void solve(int Task_Id) {
    ...
    for (int t = 1; fl < k; t++) {
        for (int i = 1; i <= n; i++) {
            add(id(t - 1, i), id(t, i), INF);
        }

        for (int i = 1; i <= m; i++) {
            add(id(t - 1, x[i]), id(t, y[i]), z[i]);
        }
