...
inline void put_tag(int x, long long v) {
    mx[x] += v;
    lz[x] += v;
}
...
void modify(int x, int l, int r, int Ll, int Rr, long long k) {
    if (Ll <= l && r <= Rr) {
        put_tag(x, k);
        return;
    }
...
inline void solve(int Task_Id) {
    ...
    for (int i = 1; i <= m; i++) {
        int l_idx = lower_bound(b + 1, b + ln + 1, al[i]) - b;
        int r_idx = lower_bound(b + 1, b + ln + 1, ar[i]) - b;
        nx[++ec] = hd[r_idx];
        tl[ec] = l_idx;
        tv[ec] = av[i];
        hd[r_idx] = ec;
    ...
    for (int i = 1; i <= ln; i++) {
        for (int e = hd[i]; e; e = nx[e]) {
            modify(1, 1, ln, 1, tl[e], tv[e]);
        }

        int j = lower_bound(b + 1, b + ln + 1, b[i] - k + 1) - b;

        dp[i] = max(0LL, query(1, 1, ln, j, i - 1)) - 1LL * b[i] * d - d;

        int pos = i - 2;
        if (b[i - 1] != b[i] - 1)
            pos = i - 1;

        dp[i] = max(dp[i], query(1, 1, ln, i, i) + got(pos) - d);
        dp[i] = max(dp[i], dp[i - 1]);

        modify(1, 1, ln, i, i, got(pos) + 1LL * b[i] * d);
    }
