...
//  Interactive: false
//  Test Type: single
//  Batch ID: 5fe658ef-f0e0-4516-81d9-14ca127b7c66
//
// Algorithm:
// Complexity: O()
// Note:
...
inline void solve(int Task_Id) {
    ...
    if (k) {
        if (k <= n - 2) {
            ...
            a[0] = 0, b[0] = (k ^ (n - 1)), a[min(k, (k ^ (n - 1)))] = k, b[min(k, (k ^ (n - 1)))] = n - 1;
        }
    } else {
        for (int i = 0; i < n >> 1; i++) {
            a[i] = i, b[i] = (i ^ (n - 1));
    ...
    if (k == n - 1) {
        a[0] = n - 2, b[0] = n - 1, a[1] = 1, b[1] = n - 3, a[2] = 0, b[2] = 2;
        for (int i = 3; i < n >> 1; i++) {
            a[i] = i, b[i] = (i ^ (n - 1));
        }
    }
