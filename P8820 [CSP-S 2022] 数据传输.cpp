...
//  Interactive: false
//  Test Type: single
//  Batch ID: a3768d5c-26e7-490f-84b7-144540005bb9
//
// Algorithm:
// Complexity: O()
// Note:
...
Vec operator*(const Vec &A, const Mat &B) {
    ...
    for (int i = 0; i < K; i++)
        for (int j = 0; j < K; j++)
            if (A.a[i] < INF && B.a[i][j] < INF)
                C.a[j] = min(C.a[j], A.a[i] + B.a[i][j]);
    return C;
}
...
void build(int p, int l, int r) {
    ...
    sf[p] = sf[p << 1] * sf[p << 1 | 1];
    sr[p] = sr[p << 1 | 1] * sr[p << 1];
}

Mat qf(int p, int l, int r, int L, int R) {
    if (L <= l && r <= R)
        return sf[p];
    ...
    if (R <= mid)
        return qf(p << 1, l, mid, L, R);

    if (L > mid)
        return qf(p << 1 | 1, mid + 1, r, L, R);

    return qf(p << 1, l, mid, L, R) *
           qf(p << 1 | 1, mid + 1, r, L, R);
}

Mat qr(int p, int l, int r, int L, int R) {
    if (L <= l && r <= R)
        return sr[p];
    ...
    if (R <= mid)
        return qr(p << 1, l, mid, L, R);

    if (L > mid)
        return qr(p << 1 | 1, mid + 1, r, L, R);
...
void append(Path &x, const Mat &A) {
    if (x.empty) {
        x.a = A;
        x.empty = 0;
    } else
...
Mat getUp(int u, int anc, bool inc) {
    Mat res = ID();

    while (top[u] != top[anc]) {
        res = res * qr(1, 1, n, dfn[top[u]], dfn[u]);
        u = fa[top[u]];
    }
    ...
    if (L <= R)
        res = res * qr(1, 1, n, L, R);

    return res;
}

Mat getDown(int anc, int u, bool inc) {
    Mat res = ID();
    Mat tmp[25];
    int cnt = 0;

    while (top[u] != top[anc]) {
        tmp[++cnt] = qf(1, 1, n, dfn[top[u]], dfn[u]);
        u = fa[top[u]];
    }
...
Vec calcUp(int u, int z) {
    ...
    Mat A = getUp(fa[u], z, 1);

    return f * A;
}

Vec calcDown(int u, int z) {
    Vec f;
    f.a[0] = w[u];
...
ll Solve(int s, int t) {
    ...
    if (s != z) {
        Mat X = getUp(fa[s], z, 1);
        A = A * X;
    }

    if (t != z) {
        Mat X = getUp(fa[t], z, 1);
        B = B * X;
    }
    ...
    for (int i = 0; i < K; i++)
        for (int j = 0; j < K; j++) {
            if (A.a[i] >= INF || B.a[j] >= INF)
                continue;

            if (i == 0 && j == 0) {
                ans = min(ans, A.a[i] + B.a[j] - w[z]);
            } else if (i + j <= K) {
                ans = min(ans, A.a[i] + B.a[j]);
            }
        }

    if (K == 3) {
        if (A.a[2] < INF && B.a[2] < INF)
            ans = min(ans, A.a[2] + B.a[2] + mn[z]);
    }
