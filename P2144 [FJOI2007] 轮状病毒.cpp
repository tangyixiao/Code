...
namespace TANGYIXIAO {
const int N = 105, L = 60;
int f[N][L], len[N];

void add(int *r, int &lr, int *a, int la, int *b, int lb) {
    int mx = max(la, lb), carry = 0;
    for (int i = 0; i < mx; ++i) {
        int s = carry + (i < la ? a[i] : 0) + (i < lb ? b[i] : 0);
        r[i] = s % 10;
        carry = s / 10;
    }
    lr = mx;
...
void sub(int *r, int &lr, int *a, int la, int *b, int lb) {
    int borrow = 0;
    for (int i = 0; i < la; ++i) {
        int s = a[i] - borrow - (i < lb ? b[i] : 0);
        if (s < 0) {
            s += 10;
            borrow = 1;
        } else
...
void mul3(int *r, int &lr, int *a, int la) {
    int carry = 0;
    for (int i = 0; i < la; ++i) {
        int s = a[i] * 3 + carry;
        r[i] = s % 10;
...
inline void solve(int Task_Id) {
    ...
    int tmp[L], ltmp;
    for (int i = 3; i <= n; ++i) {
        mul3(tmp, ltmp, f[i - 1], len[i - 1]);              // tmp = 3*f[i-1]
        sub(f[i], len[i], tmp, ltmp, f[i - 2], len[i - 2]); // - f[i-2]
        int two[1] = {2};
        add(tmp, ltmp, f[i], len[i], two, 1); // + 2
        memcpy(f[i], tmp, ltmp * sizeof(int));
        len[i] = ltmp;
    }
