//  Author: Tangyixiao
//  Time: 2026-08-13 10:06:08
//  Problem: P4135 作诗
//  Contest: Luogu
//  URL: https://www.luogu.com.cn/problem/P4135
//  Memory Limit: 512 MB
//  Time Limit: 2500 ms
//  Interactive: false
//  Test Type: single
//  Batch ID: 5dd65ffc-c80a-4874-b864-d29d77ed4325
//
// Algorithm:
// Complexity: O()
// Note:
...
namespace TANGYIXIAO {
const int N = 100005, B = 320, K = 320;

int n, c, m, nb, a[N], bl[N], L[K], R[K];
int s[K][N], f[K][K], t[N], v[N], T;

int ask(int l, int r) {
    int x = bl[l], y = bl[r], p = x + 1, q = y - 1, ans = 0;
    if (p <= q)
        ans = f[p][q];
...
inline void solve(int Task_Id) {
    ...
    for (int i = 1; i <= nb; i++) {
        memcpy(s[i], s[i - 1], (c + 1) * sizeof(int));
        for (int j = L[i]; j <= R[i]; j++)
            ++s[i][a[j]];
    }
