...
int a[262144], b[262144], r[262144], w[100005], iv[100005], f[100005], g[100005];
int pw(int x, int y) {
    int s = 1;
    while (y) {
        if (y & 1)
...
void ntt(int *a, int n, int fg) {
    ...
    for (int l = 2; l <= n; l <<= 1) {
        ...
        for (int i = 0; i < n; i += l) {
            int w = 1;
            for (int j = 0; j < l / 2; ++j) {
                int u = a[i + j], v = 1LL * w * a[i + j + l / 2] % Q;
                a[i + j] = (u + v) % Q;
                a[i + j + l / 2] = (u - v + Q) % Q;
                w = 1LL * w * g % Q;
            }
...
inline void solve(int Task_Id) {
    ...
    iv[1] = 1;
    for (int i = 2; i <= M; ++i)
        iv[i] = 1LL * (Q - Q / i) * iv[Q % i] % Q;
    g[0] = 1;
    for (int i = 1; i <= M; ++i)
        g[i] = 1LL * g[i - 1] * iv[i] % Q;
    for (int i = 0; i <= M; ++i) {
        a[i] = 1LL * w[i] * g[i] % Q;
        b[i] = g[i];
        if (i & 1)
    ...
    for (int i = 2; i <= S; ++i)
        fs = 1LL * fs * i % Q;
    int ifs = pw(fs, Q - 2);
    int C = 1, P = 1, ps = 1, ips = 1;
    long long ans = 0;
    ans = (ans + 1LL * pw(M, N) * w[0]) % Q;
    for (int i = 1; i <= lim; ++i) {
        C = 1LL * C * (M - i + 1) % Q * iv[i] % Q;
        int bs = N - (i - 1) * S;
        long long ml = 1;
        for (int j = 0; j < S; ++j)
            ml = ml * (bs - j) % Q;
        ...
        ps = 1LL * ps * fs % Q;
        ips = 1LL * ips * ifs % Q;
        int pwv = pw(M - i, N - i * S);
        int fi = 1LL * C * P % Q * ips % Q * pwv % Q;
        ans = (ans + 1LL * fi * w[i]) % Q;
    }
    printf("%lld\n", ans);
    return;
}
