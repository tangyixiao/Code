//  Author: Tangyixiao
//  Time: 2026-08-12 10:33:20
//  Problem: P13272 [NOI2025] 序列变换
//  Contest: Luogu
//  URL: https://www.luogu.com.cn/problem/P13272
//  Memory Limit: 1024 MB
//  Time Limit: 6000 ms
//  Interactive: false
//  Test Type: single
//  Batch ID: 6ecbd425-e7e3-454c-94de-948f9aaaf0f3
//
// Algorithm:
// Complexity: O()
// Note:
...
int n, a[N], b[N], c[N], l[N], r[N], pc[N], ip[N], lg[N], st[2][13][N], si[2][N];
long long sa[N], sb[N], f[N], g[N];
...
long long mn(int p, int l, int r) {
    int k = lg[r - l + 1];
    return min(st[p][k][l], st[p][k][r - (1 << k) + 1]);
}
inline void solve(int Task_Id) {
    cin >> n;
    ...
    for (int i = 1; i <= n; i++) {
        ...
        long long x = pw(c[i], 1000000005);

        pc[i] = 1LL * pc[i - 1] * c[i] % 1000000007;
        ip[i] = 1LL * ip[i - 1] * x % 1000000007;

        si[0][i] = si[0][i - 1];
        si[1][i] = si[1][i - 1];
        si[i & 1][i] = (si[i & 1][i] + x) % 1000000007;

        st[0][0][i] = st[1][0][i] = 1000000000;
        st[i & 1][0][i] = b[i];
    }
    ...
    for (int p = 0; p < 2; p++)
        for (int k = 1; k < 13; k++)
            for (int i = 1; i + (1 << k) - 1 <= n; i++)
                st[p][k][i] = min(st[p][k - 1][i], st[p][k - 1][i + (1 << (k - 1))]);
    ...
    for (int i = 0; i < n; i++) {
        for (int j = i + 1; j <= n; j++) {
            int x = max(i + 1, r[j]), y = min(j, l[i + 1]);

            if (x > y)
                continue;

            long long z = g[i] * pc[j] % 1000000007 * ip[i] % 1000000007;
            long long q = sa[j] - sa[i];

            if (!q) {
                f[j] = max(f[j], f[i] + sb[j] - sb[i]);
                g[j] = (g[j] + z) % 1000000007;
            } else if (q > 0) {
                f[j] = max(f[j], f[i] + sb[j] - sb[i] - mn(0, x, y));
                long long w = si[0][y] - si[0][x - 1];
                if (w < 0)
                    w += 1000000007;
                g[j] = (g[j] + z * w) % 1000000007;
            } else {
                f[j] = max(f[j], f[i] + sb[j] - sb[i] - mn(1, x, y));
                long long w = si[1][y] - si[1][x - 1];
                if (w < 0)
                    w += 1000000007;
                g[j] = (g[j] + z * w) % 1000000007;
            }
