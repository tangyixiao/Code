//  Author: Tangyixiao
//  Time: 2026-09-20 08:17:25
//  Problem: U110069 最大加权矩形
//  Contest: Luogu - SXYZ 训练赛④
//  URL: https://www.luogu.com.cn/problem/U110069?contestId=357226
//  Memory Limit: 125 MB
//  Time Limit: 1000 ms
//  Interactive: false
//  Test Type: single
//  Batch ID: d4b517af-13aa-4371-bc91-7ee14a0acc9f
//
// Algorithm:
// Complexity: O()
// Note:
...
namespace TANGYIXIAO {
const int N = 1e2 + 5;
int n, a[N][N], s[N][N], ans;
inline void solve(int Task_Id) {
    cin >> n;
    for (int i = 1; i <= n; i++) {
        for (int j = 1; j <= n; j++) {
            cin >> a[i][j], s[i][j] = s[i - 1][j] + s[i][j - 1] + a[i][j] - s[i - 1][j - 1];
        }
    }
    ...
    for (int i = 1; i <= n; i++) {
        for (int j = 1; j <= n; j++) {
            for (int r = i; r <= n; r++) {
                for (int c = j; c <= n; c++) {
                    ans = max(s[r][c] - s[r][j - 1] - s[i - 1][c] + s[i - 1][j - 1], ans);
                }
            }
        }
    }
