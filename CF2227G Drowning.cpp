...
#pragma GCC optimize("-freod-blocks")
...
#pragma GCC optimize("-freod-functions")
...
#pragma G++ optimize("-freod-blocks")
...
#pragma G++ optimize("-freod-functions")
...
#include <atomic>           // 原子操作 (atomic<T>, memory_od)
...
#include <compare>         // 三路比较运算符支持 (strong_oding 等)
...
#include <unoded_map>         // 无序关联容器 (unoded_map, unoded_multimap)
#include <unoded_set>         // 无序集合 (unoded_set, unoded_multiset)
...
#include <ext/pb_ds/tree_policy.hpp>        // 树策略：实现 od_of_key 和 find_by_od
