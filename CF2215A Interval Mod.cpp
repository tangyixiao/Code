//  Author: Tangyixiao
//  Time: 2026-09-23 15:06:34
//  Problem: A. Interval Mod
//  Contest: Codeforces - Codeforces Round 1092 (Unrated, Div. 1, Based on THUPC 2026 — Finals)
//  URL: https://codeforces.com/problemset/problem/2215/A
//  Memory Limit: 256 MB
//  Time Limit: 2000 ms
//  Interactive: false
//  Test Type: single
//  Batch ID: 3376e826-b7ea-4c6d-86d3-617107a25feb
//
//  Algorithm:
//  Complexity: O()
//  Note:
...
inline void solve(int Task_Id) {
    ...
    for (int i = 1; i <= n; i++) {
        cin >> x;
        a = x % p, b = x % q % p;
        s0[i] = s0[i - 1] + a, s1[i] = s1[i - 1] + b, s[i] = s[i - 1] + min(a, b);
    }
    for (int i = k; i <= n; i++) {
        ans = min(ans, s[i - k] + s[n] - s[i] + min(s0[i] - s0[i - k], s1[i] - s1[i - k]));
    }
    cout << ans << "\n";
    return;
}
