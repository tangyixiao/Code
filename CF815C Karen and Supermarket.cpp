...
int sz[5005], sc[5005];
long long f0[12510000], f1[12510000];
long long *p0[5005], *p1[5005];
long long t0[5005], t1[5005];
...
inline void solve(int Task_Id) {
    ...
    for (int i = n; i >= 2; --i) {
        ...
        for (int j = 0; j <= sc[p]; ++j) {
            for (int k = 0; k <= sz[i]; ++k) {
                t0[j + k] = min(t0[j + k], p0[p][j] + p0[i][k]);
                t1[j + k] = min(t1[j + k], p1[p][j] + min(p0[i][k], p1[i][k]));
            }
        }
        sc[p] = tot;
        for (int j = 0; j <= tot; ++j) {
