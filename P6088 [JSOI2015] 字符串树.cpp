//  Author: Tangyixiao
//  Time: 2026-08-12 07:58:30
//  Problem: P6088 [JSOI2015] 字符串树
//  Contest: Luogu
//  URL: https://www.luogu.com.cn/problem/P6088
//  Memory Limit: 500 MB
//  Time Limit: 2000 ms
//  Interactive: false
//  Test Type: single
//  Batch ID: f4d07a78-7b95-4d1f-a812-4f2ae4e2f809
//
// Algorithm:
// Complexity: O()
// Note:
...
int n, q, h[N], to[N << 1], nx[N << 1], id[N << 1], ec;
char w[N][11];
int fa[18][N], d[N], rt[N], o[N], tp;
struct T {
    int c[26], s;
} tr[N * 11];
int tc;
...
void ae(int u, int v, int z) {
    to[++ec] = v, id[ec] = z, nx[ec] = h[u], h[u] = ec;
}
...
inline void solve(int Task_Id) {
    ...
    while (q--) {
        ...
        cin >> u >> v >> s;
        int x = lca(u, v);
        cout << qr(rt[u], s) + qr(rt[v], s) - 2 * qr(rt[x], s) << '\n';
    }
    return;
}
} // namespace TANGYIXIAO
