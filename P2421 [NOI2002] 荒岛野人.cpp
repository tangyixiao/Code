...
int chk(int M) {
    int x, y;
    for (int i = 0; i < vc; ++i) {
        int a = v[i].a, c0 = v[i].c0, lim = v[i].lim;
        if (a == 0) {
            if (c0 % M == 0)
                return 0;
            continue;
        }
        int aa = abs(a), g = exgcd(aa, M, x, y);
        if (c0 % g != 0)
            continue;
        int mod = M / g;
        int x0 = (1LL * x * (c0 / g) % mod + mod) % mod;
        if (a < 0)
            x0 = (mod - x0) % mod;
        if (x0 <= lim)
            return 0;
...
inline void solve(int Task_Id) {
    ...
    for (int i = 0; i < n; ++i)
        for (int j = i + 1; j < n; ++j) {
            v[vc].a = p[i] - p[j];
            v[vc].c0 = ((c[j] - c[i]) % 1000000000 + 1000000000) % 1000000000;
            v[vc].lim = min(l[i], l[j]);
            ++vc;
        }
    for (int M = mx;; ++M) {
        if (chk(M)) {
