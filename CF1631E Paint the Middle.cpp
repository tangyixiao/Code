//  Author: Tangyixiao
//  Time: 2026-09-09 14:49:05
//  Problem: E. Paint the Middle
//  Contest: Codeforces - Codeforces Round 768 (Div. 2)
//  URL: https://codeforces.com/contest/1631/problem/E
//  Memory Limit: 256 MB
//  Time Limit: 2000 ms
//  Interactive: false
//  Test Type: single
//  Batch ID: 2bddc47b-ebf9-4919-856d-5cf806371201
//
// Algorithm:
// Complexity: O()
// Note:
...
inline void solve(int Task_Id) {
    cin >> n;
    for (int i = 1; i <= n; i++) {
        cin >> a[i], f[a[i]] = f[a[i]] ? f[a[i]] : i, e[a[i]] = i;
    }
    for (int v = 1; v <= n; v++) {
        if (f[v] && f[v] < e[v]) {
            s[++m] = {f[v], e[v]};
    ...
    sort(s + 1, s + 1 + m);
    for (int p = 1, L, R, q, d, i, k; p <= m;) {
        L = s[p].l, R = s[p].r, q = i = p, d = L, k = 0;
        for (; q + 1 <= m && s[q + 1].l <= R; q++, R = max(R, s[q].r))
            ;
        for (int h = d; d < R; k++, d = h) {
            for (; i <= q && s[i].l <= d; h = max(h, s[i].r), i++)
                ;
        }
        ans += R - L + 1 - (k + 1), p = q + 1;
    }
