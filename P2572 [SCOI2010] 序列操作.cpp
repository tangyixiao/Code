...
void push_up(int rt) {
    ...
    for (int k = 0; k < 2; ++k) {
        tree[rt].pre[k] = tree[l].pre[k];
        if (tree[l].pre[k] == tree[l].len)
            tree[rt].pre[k] += tree[r].pre[k];

        tree[rt].suf[k] = tree[r].suf[k];
        if (tree[r].suf[k] == tree[r].len)
            tree[rt].suf[k] += tree[l].suf[k];

        tree[rt].mx[k] = max({tree[l].mx[k],
                              tree[r].mx[k],
                              tree[l].suf[k] + tree[r].pre[k]});
    }
}

void apply_set(int rt, int val) {
    tree[rt].set = val;
    tree[rt].rev = false;
    tree[rt].sum = val ? tree[rt].len : 0;
    tree[rt].pre[val] = tree[rt].suf[val] = tree[rt].mx[val] = tree[rt].len;
    tree[rt].pre[val ^ 1] = tree[rt].suf[val ^ 1] = tree[rt].mx[val ^ 1] = 0;
}

void apply_rev(int rt) {
    tree[rt].sum = tree[rt].len - tree[rt].sum;
    swap(tree[rt].pre[0], tree[rt].pre[1]);
    swap(tree[rt].suf[0], tree[rt].suf[1]);
    swap(tree[rt].mx[0], tree[rt].mx[1]);
    if (tree[rt].set != -1) {
        tree[rt].set ^= 1;
    } else {
        tree[rt].rev ^= 1;
...
void push_down(int rt) {
    int l = rt << 1, r = rt << 1 | 1;
    if (tree[rt].set != -1) {
        apply_set(l, tree[rt].set);
        apply_set(r, tree[rt].set);
        tree[rt].set = -1;
    }
    if (tree[rt].rev) {
        apply_rev(l);
...
void build(int rt, int l, int r) {
    ...
    if (l == r) {
        ...
        tree[rt].len = 1;
        tree[rt].sum = val;
        tree[rt].pre[val] = tree[rt].suf[val] = tree[rt].mx[val] = 1;
        tree[rt].pre[val ^ 1] = tree[rt].suf[val ^ 1] = tree[rt].mx[val ^ 1] = 0;
        return;
    }
    int mid = (l + r) >> 1;
...
void update_set(int rt, int l, int r, int ql, int qr, int val) {
    if (ql <= l && r <= qr) {
        apply_set(rt, val);
        return;
    }
    ...
    int mid = (l + r) >> 1;
    if (ql <= mid)
        update_set(rt << 1, l, mid, ql, qr, val);
    if (qr > mid)
        update_set(rt << 1 | 1, mid + 1, r, ql, qr, val);
    push_up(rt);
}

void update_rev(int rt, int l, int r, int ql, int qr) {
    if (ql <= l && r <= qr) {
        apply_rev(rt);
        return;
    }
    ...
    int mid = (l + r) >> 1;
    if (ql <= mid)
        update_rev(rt << 1, l, mid, ql, qr);
    if (qr > mid)
        update_rev(rt << 1 | 1, mid + 1, r, ql, qr);
    push_up(rt);
}
...
int query_sum(int rt, int l, int r, int ql, int qr) {
    ...
    int mid = (l + r) >> 1, res = 0;
    if (ql <= mid)
        res += query_sum(rt << 1, l, mid, ql, qr);
    if (qr > mid)
        res += query_sum(rt << 1 | 1, mid + 1, r, ql, qr);
    return res;
}

Node query_max(int rt, int l, int r, int ql, int qr) {
    if (ql <= l && r <= qr)
        return tree[rt];
    push_down(rt);
    int mid = (l + r) >> 1;
    if (qr <= mid)
        return query_max(rt << 1, l, mid, ql, qr);
    if (ql > mid)
        return query_max(rt << 1 | 1, mid + 1, r, ql, qr);

    Node left = query_max(rt << 1, l, mid, ql, qr);
    Node right = query_max(rt << 1 | 1, mid + 1, r, ql, qr);
    Node res;
    res.sum = left.sum + right.sum;
    res.len = left.len + right.len;

    for (int k = 0; k < 2; ++k) {
        res.pre[k] = left.pre[k];
        if (left.pre[k] == left.len)
            res.pre[k] += right.pre[k];

        res.suf[k] = right.suf[k];
        if (right.suf[k] == right.len)
            res.suf[k] += left.suf[k];

        res.mx[k] = max({left.mx[k], right.mx[k], left.suf[k] + right.pre[k]});
    }
    return res;
}
inline void solve(int Task_Id) {
