...
void update_expired(long long t) {
    while (!pq.empty() && pq.top().first <= t) {
        auto [exp, i] = pq.top();
        pq.pop();
        // 只有当过期时刻未发生变化且状态为待分配或占用时才处理
        if (pool[i].expire == exp && (pool[i].state == 1 || pool[i].state == 2)) {
            if (pool[i].state == 1) {
                pool[i].state = 0;
                pool[i].owner = "";
            } else { // state == 2
...
// 查找占用者为 sender 的 IP（任意状态）
int find_owner(const string &sender) {
    for (int i = 1; i <= N; ++i) {
        if (pool[i].owner == sender)
            return i;
    }
    return -1;
}
...
long long calc_expire(long long t, long long req_exp) {
    if (req_exp == 0) {
        ...
        long long earliest = t + T_min;
        long long latest = t + T_max;
        if (req_exp < earliest)
            return earliest;
        if (req_exp > latest)
            return latest;
...
// 处理 Discover 报文
void process_discover(long long t, const string &sender, long long req_exp) {
    int ip = find_owner(sender);
    if (ip == -1)
        ip = find_free();
    if (ip == -1)
        ip = find_expired();
    ...
    pool[ip].state = 1;
    pool[ip].owner = sender;
    pool[ip].expire = calc_expire(t, req_exp);
    pq.push({pool[ip].expire, ip});
    cout << H << " " << sender << " OFR " << ip << " " << pool[ip].expire << "\n";
}

// 处理 Request 报文
void process_request(long long t, const string &sender, const string &recver, int ip, long long req_exp) {
    if (recver != H) {
        // 释放该客户端所有待分配状态的地址
        for (int i = 1; i <= N; ++i) {
            if (pool[i].owner == sender && pool[i].state == 1) {
                pool[i].state = 0;
                pool[i].owner = "";
                pool[i].expire = 0;
            }
    ...
    pool[ip].expire = calc_expire(t, req_exp);
    pq.push({pool[ip].expire, ip});
    cout << H << " " << sender << " ACK " << ip << " " << pool[ip].expire << "\n";
}
...
inline void solve(int Task_Id) {
    ...
    for (int i = 0; i < n; ++i) {
        ...
        int ip;
        long long exp;
        cin >> t >> sender >> recver >> type >> ip >> exp;
        ...
        if (type == "DIS") {
            process_discover(t, sender, exp);
        } else { // REQ
            process_request(t, sender, recver, ip, exp);
        }
    }
    return;
}
