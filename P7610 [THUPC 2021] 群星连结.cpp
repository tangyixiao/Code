...
enum class TalentType : int {
    None = 0,           // 我自闭了
    FleshPuppet = 1,    // 血肉皮囊
    StarProjection = 2, // 星河力量投射
    MindOverMatter = 3, // 心胜于物
    Transcendence = 4,  // 超凡入圣
...
enum class SkillType : int {
    ...
    IntercontinentalMissile = 2, // 日昇之雨
    SkyscraperBreak = 3,         // 天楼霸断剑
    Showtime = 4,                // 演出开始
    SiriusBite = 5,              // 天狼噬斩
    EarthBlueFlash = 6,          // 地球蓝色闪电
    AuroraBloom = 7,             // 极光绽放
    Meteor = 8,                  // 流星
    FairyShelter = 9,            // 精灵庇护
...
inline void solve(int Task_Id) {
    ...
    while (true) {
        ...
        for (auto &ev : delayed_events[current_turn]) {
            ...
            Character *target_chars = player[ev.target_player].characters;
            for (int i = 1; i <= num_characters; ++i) {
                if (ev.type == EventType::AttackBonus)
                    target_chars[i].attack_bonus += ev.value;
                else if (ev.type == EventType::DefenseBonus)
                    target_chars[i].defense_bonus += ev.value;
                else if (ev.type == EventType::EnergyBonus)
