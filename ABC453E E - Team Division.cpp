...
namespace TANGYIXIAO {
const int N = 1e6 + 5;
int n, dfA[N], dfB[N], db[N], L[N], R[N], fac[N], ifac[N], cntA[N], cntB[N], cntBo[N];
inline void solve(int Task_Id) {
    cin >> n;
    for (int i = 0; i < n; i++) {
        cin >> L[i] >> R[i];
    ...
    for (int i = 0; i < n; i++) {
        ...
        int lba = max(l, n - r), rba = min(r, n - l);
        if (lba <= rba) {
            db[lba] += 1;
            db[rba + 1] -= 1;
        }
    ...
    for (int k = 1; k <= n - 1; ++k) {
        cntA[k] = cntA[k - 1] + dfA[k];
        cntB[k] = cntB[k - 1] + dfB[k];
        cntBo[k] = cntBo[k - 1] + db[k];
    }
    ...
    int ans = 0;
    for (int k = 1; k <= n - 1; ++k) {
        int ca = cntA[k], cb = cntB[k], cbo = cntBo[k];
        int cntA = ca - cbo, cntB = cb - cbo;
        if (n - (ca + cb - cbo) > 0) {
            continue;
        }
        int nd = k - cntA;
