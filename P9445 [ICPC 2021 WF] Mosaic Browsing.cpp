//  Author: Tangyixiao
// Time: 2026-08-17 23:03:56
//  Problem: P9445 [ICPC 2021 WF] Mosaic Browsing
//  Contest: Luogu
//  URL: https://www.luogu.com.cn/problem/P9445
//  Memory Limit: 256 MB
//  Time Limit: 6000 ms
//  Interactive: false
//  Test Type: single
//  Batch ID: 4ce7e97c-7ff5-414a-a315-88f707a75589
//
// Algorithm:
// Complexity: O()
// Note:
...
C a[M], b[M], rt[101];
unsigned char p[1000][1000];
pair<int, int> an[1000005];

inline C add(C a, C b) {
    return {a.x + b.x, a.y + b.y};
}

inline C sub(C a, C b) {
    return {a.x - b.x, a.y - b.y};
}
...
inline void solve(int Task_Id) {
    ...
    for (int i = 1; i <= 100; i++) {
        double q = 2 * P * i / 101.0;
        rt[i] = {cos(q), sin(q)};
    }

    int w = cp + cq - 1, n = 1;

    long long z = 1LL * (rp + rq - 1) * w;
    ...
    for (int i = 0; i + rp <= rq; i++)
        for (int j = 0; j + cp <= cq; j++) {
            long long id = 1LL * (i + rp - 1) * w + j + cp - 1;
