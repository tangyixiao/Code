...
int n, q;
vector<int> g[MAXN];
int parent[MAXN], depth[MAXN], dfn[MAXN];
int timer = 0;
int up[LOGN][MAXN];
...
struct Node {
    ...
    Node() : min1(0), min2(0), max1(0), max2(0) {}
};
...
Node merge(const Node &a, const Node &b) {
    ...
    if (a.min1)
        v.emplace_back(dfn_global[a.min1], a.min1);
    if (a.min2)
        v.emplace_back(dfn_global[a.min2], a.min2);
    if (b.min1)
        v.emplace_back(dfn_global[b.min1], b.min1);
    if (b.min2)
        v.emplace_back(dfn_global[b.min2], b.min2);
    sort(v.begin(), v.end());
    ...
    v.clear();
    if (a.max1)
        v.emplace_back(dfn_global[a.max1], a.max1);
    if (a.max2)
        v.emplace_back(dfn_global[a.max2], a.max2);
    if (b.max1)
        v.emplace_back(dfn_global[b.max1], b.max1);
    if (b.max2)
        v.emplace_back(dfn_global[b.max2], b.max2);
...
void build(int node, int l, int r) {
    if (l == r) {
        seg[node].min1 = seg[node].max1 = l;
        seg[node].min2 = seg[node].max2 = 0;
        return;
    }
    int mid = (l + r) / 2;
    build(node * 2, l, mid);
...
inline void solve(int Task_Id) {
    ...
    while (q--) {
        ...
        int u_min = res.min1, u_sec_min = res.min2;
        int u_max = res.max1, u_sec_max = res.max2;

        int cand1 = lca(u_sec_min, u_max);
        int d1 = depth[cand1];

        int cand2 = lca(u_min, u_sec_max);
        int d2 = depth[cand2];
