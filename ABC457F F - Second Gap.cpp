...
void range_mul(int node, int l, int r, int ql, int qr, long long val) {
    if (ql <= l && r <= qr) {
        apply(node, val);
        return;
    }
...
void point_add(int node, int l, int r, int idx, long long val) {
    if (l == r) {
        sum[node] = (sum[node] + val) % MOD;
        return;
    }
    ...
    int mid = (l + r) / 2;
    if (idx <= mid)
        point_add(node * 2, l, mid, idx, val);
    else
        point_add(node * 2 + 1, mid + 1, r, idx, val);
    sum[node] = (sum[node * 2] + sum[node * 2 + 1]) % MOD;
}
...
inline void solve(int Task_Id) {
    ...
    for (int j = N - 1; j >= 1; --j) {
        if (j == N - 1) {
            long long w = point_query(1, 1, N, N);
            range_mul(1, 1, N, 1, N, 0);
            point_add(1, 1, N, j, w);
            point_add(1, 1, N, N, w);
        } else {
            if (D[j] != D[j + 1]) {
                int S = j + D[j];
                long long w = point_query(1, 1, N, S);
                range_mul(1, 1, N, 1, N, 0);
                point_add(1, 1, N, S, w);
                point_add(1, 1, N, j, w);
            } else {
