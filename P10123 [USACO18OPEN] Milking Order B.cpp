...
inline void solve(int Task_Id) {
    ...
    cin >> n >> m >> k;
    vector<int> order(m);
    vector<int> ord_idx(n + 1, -1);
    for (int i = 0; i < m; ++i) {
        cin >> order[i];
        ord_idx[order[i]] = i;
    }
    vector<int> fix_pos(n + 1, 0);
    for (int i = 0; i < k; ++i) {
        int c, p;
        cin >> c >> p;
        fix_pos[c] = p;
        ...
        for (int pos = 1; pos <= n; ++pos) {
            ...
            for (int i = 1; i <= n; ++i) {
                if (slot[i] != 0) {
                    int c = slot[i];
                    if (ord_idx[c] != -1) {
                        if (ord_idx[c] <= last_ord_idx) {
                            ok = false;
                            break;
                        }
                        last_ord_idx = ord_idx[c];
                        all_order_pos[ord_idx[c]] = i;
                    }
                }
            }
            if (!ok)
            ...
            int prev = 0;
            for (int i = 0; i < m; ++i) {
                if (all_order_pos[i] != -1) {
                    prev = all_order_pos[i];
                } else {
                    int next_fixed = n + 1;
                    for (int j = i + 1; j < m; ++j) {
                        if (all_order_pos[j] != -1) {
                            next_fixed = all_order_pos[j];
                            break;
                        }
                    }
                    bool placed = false;
                    for (int p = prev + 1; p < next_fixed; ++p) {
                        if (slot[p] == 0) {
                            slot[p] = order[i];
                            all_order_pos[i] = p;
                            prev = p;
