//  Author: Tangyixiao
// Time: 2026-08-17 22:59:41
//  Problem: P9443 [ICPC 2021 WF] Hand of the Free Marked
//  Contest: Luogu
//  URL: https://www.luogu.com.cn/problem/P9443
//  Memory Limit: 256 MB
//  Time Limit: 2000 ms
//  Interactive: false
//  Test Type: single
//  Batch ID: 407312ec-2ca9-4628-b107-4d1916e2403a
//
// Algorithm:
// Complexity: O()
// Note:
...
int k, m, x[12];
long long a[12], n;
long double c[12][12], fac, ans;

long double C(long long n, int r) {
    if (r < 0 || n < r)
        return 0;
    long double s = 1;
    for (int i = 1; i <= r; i++)
...
void dfs(int p, int r) {
    if (p == m) {
        ...
        for (int i = 0; i < m; i++)
            if (x[i])
                t += (long double)x[i] / (a[i] - x[i] + 1);

        ans += u * min((long double)1, fac * t);
        return;
    }

    for (int i = 0; i <= r && i <= a[p]; i++) {
        x[p] = i;
        dfs(p + 1, r - i);
    }
}
