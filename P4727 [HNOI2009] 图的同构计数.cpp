...
//  Interactive: false
//  Test Type: single
//  Batch ID: 59d720cf-d8f1-47a9-ac13-7e13b58e2382
//
// Algorithm: polya
// Complexity: O()
// Note:
...
void dfs(int rem, int max_val, int len) {
    if (rem == 0) {
        ...
        for (int a = 0; a < K; ++a) {
            ...
            for (int b = a + 1; b < K; ++b) {
                int j = nz_i[b];
                c += mi * nz_m[b] * gcd_tab[i][j];
            }
            c += (mi * (mi - 1LL) / 2) * i;
        }
        long long term = pow2[c];
        for (int a = 0; a < K; ++a) {
            int i = nz_i[a];
            int mi = nz_m[a];
            int inv_i = inv[i];
