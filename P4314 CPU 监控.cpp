...
struct Tag {
    int add, set;
    Tag(int a = 0, int s = INF) : add(a), set(s) {}
    bool isSet() const { return set != INF; }
    int apply(int x) const {
        return isSet() ? set : x + add;
    }
    Tag merge(const Tag &t) const {
        if (t.isSet())
            return Tag(0, t.set);
        if (isSet())
            return Tag(0, set + t.add);
        return Tag(add + t.add, INF);
    }
    Tag maxTag(const Tag &t, int x = 0) const {
        int val1 = apply(x);
        int val2 = t.apply(x);
        if (val1 > val2)
            return *this;
...
void push_up(int rt) {
    ...
    tree[rt].hmx = max(tree[rt << 1].hmx, tree[rt << 1 | 1].hmx);
}

void apply(int rt, const Tag &curTag, const Tag &hisTag) {
    tree[rt].hmx = max(tree[rt].hmx, hisTag.apply(tree[rt].mx));
    tree[rt].mx = curTag.apply(tree[rt].mx);
    tree[rt].htag = tree[rt].htag.maxTag(tree[rt].tag.merge(hisTag), 0);
    tree[rt].tag = tree[rt].tag.merge(curTag);
}

void push_down(int rt) {
    apply(rt << 1, tree[rt].tag, tree[rt].htag);
    apply(rt << 1 | 1, tree[rt].tag, tree[rt].htag);
    tree[rt].tag = Tag();
    tree[rt].htag = Tag();
}

void build(int rt, int l, int r) {
    tree[rt].tag = tree[rt].htag = Tag();
    if (l == r) {
        tree[rt].mx = tree[rt].hmx = a[l];
        return;
    }
    int mid = (l + r) >> 1;
    build(rt << 1, l, mid);
...
void update(int rt, int l, int r, int ql, int qr, const Tag &t) {
    if (ql <= l && r <= qr) {
        apply(rt, t, t);
        return;
    }
    ...
    int mid = (l + r) >> 1;
    if (ql <= mid)
        update(rt << 1, l, mid, ql, qr, t);
    if (qr > mid)
        update(rt << 1 | 1, mid + 1, r, ql, qr, t);
    push_up(rt);
}
...
int query_max(int rt, int l, int r, int ql, int qr) {
    ...
    int res = -INF;
    if (ql <= mid)
        res = max(res, query_max(rt << 1, l, mid, ql, qr));
    if (qr > mid)
        res = max(res, query_max(rt << 1 | 1, mid + 1, r, ql, qr));
    return res;
}

int query_hmax(int rt, int l, int r, int ql, int qr) {
    if (ql <= l && r <= qr)
        return tree[rt].hmx;
    push_down(rt);
    int mid = (l + r) >> 1;
    int res = -INF;
    if (ql <= mid)
        res = max(res, query_hmax(rt << 1, l, mid, ql, qr));
    if (qr > mid)
        res = max(res, query_hmax(rt << 1 | 1, mid + 1, r, ql, qr));
    return res;
}
...
inline void solve(int Task_Id) {
    ...
    while (E--) {
        ...
        cin >> op >> x >> y;
        if (op[0] == 'Q') {
            cout << query_max(1, 1, T, x, y) << '\n';
        } else if (op[0] == 'A') {
            cout << query_hmax(1, 1, T, x, y) << '\n';
        } else if (op[0] == 'P') {
            cin >> z;
            update(1, 1, T, x, y, Tag(z, INF));
        } else if (op[0] == 'C') {
            cin >> z;
            update(1, 1, T, x, y, Tag(0, z));
        }
    }
    return;
}
