//  Author: Tangyixiao
// Time: 2026-07-03 09:43:10
//  Problem: P7251 [JSOI2014] 强连通图
//  Contest: Luogu
//  URL: https://www.luogu.com.cn/problem/P7251
//  Memory Limit: 128 MB
//  Time Limit: 1000 ms
//  Interactive: false
//  Test Type: single
//  Batch ID: 2538eb66-8c17-4bab-8b97-189a3ad530d6
//
// Algorithm:
// Complexity: O()
// Note:
...
int n, m, hd[N], to[M], nx[M], tot;
int df[N], lo[N], st[N], tp, ins[N], tm, sc, id[N], sz[N];
void ad(int u, int v) {
    to[++tot] = v;
    nx[tot] = hd[u];
    hd[u] = tot;
...
inline void solve(int Task_Id) {
    ...
    static int in[N] = {0}, ou[N] = {0};
    static pair<int, int> ed[M];
    int ec = 0;
    for (int u = 1; u <= n; ++u)
        for (int i = hd[u]; i; i = nx[i]) {
            int v = to[i];
