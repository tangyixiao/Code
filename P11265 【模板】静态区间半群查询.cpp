...
struct output {
    ...
    void setres(const mat &res) {
        int tmp = 0;
        for (int i : {0, 1})
            for (int j : {0, 1})
                tmp += res.a[i][j] ^ kv[i][j];
        ans ^= tmp;
    }
} out;
...
int n, m;
mat a[N];
int ql[N], qr[N], qid[N], tmp[N];
mat Lprod[N], Rprod[N];

void df(int l, int r, int qL, int qR) {
    if (qL > qR)
        return;
    if (l == r) {
        for (int i = qL; i <= qR; ++i)
    ...
    for (int i = mid - 1; i >= l; --i)
        Lprod[i] = mul(a[i], Lprod[i + 1]);

    Rprod[mid + 1] = a[mid + 1];
    for (int i = mid + 2; i <= r; ++i)
        Rprod[i] = mul(Rprod[i - 1], a[i]);

    int left = qL, right = qR;
    for (int i = qL; i <= qR; ++i) {
        int id = qid[i];
        if (ql[id] <= mid && qr[id] > mid) {
            out.setres(mul(Lprod[ql[id]], Rprod[qr[id]]));
        } else if (qr[id] <= mid) {
            tmp[left++] = id;
        } else {
            tmp[right--] = id;
