...
inline void solve(int Task_Id) {
    ...
    vector<pair<int, int>> friendEdges(p), enemyEdges(q);
    ...
    int len = S.size();
    vector<vector<char>> isFriend(len, vector<char>(len, 0));
    vector<vector<char>> isEnemy(len, vector<char>(len, 0));
    vector<vector<int>> friendIdx(len), enemyIdx(len);

    for (auto &e : friendEdges) {
        int u = e.first, v = e.second;
        int i = pos[u], j = pos[v];
        isFriend[i][j] = isFriend[j][i] = 1;
        friendIdx[i].push_back(j);
        friendIdx[j].push_back(i);
    }
    for (auto &e : enemyEdges) {
        int u = e.first, v = e.second;
        int i = pos[u], j = pos[v];
        isEnemy[i][j] = isEnemy[j][i] = 1;
        enemyIdx[i].push_back(j);
        enemyIdx[j].push_back(i);
    }

    long long ans = 0;
    long long isolated = n - len;
    ans += isolated * (isolated - 1) / 2;
    ans += isolated * len;
