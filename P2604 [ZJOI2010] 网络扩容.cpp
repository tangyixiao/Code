...
inline void solve(int Task_Id) {
    ...
    vector<tuple<int, int, ll, ll>> orig;
    for (int i = 0; i < m; ++i) {
        int u, v;
        ll c, w;
        cin >> u >> v >> c >> w;
        add(u, v, c, 0);
        orig.emplace_back(u, v, c, w);
    }

    vector<int> cur(n + 1), level(n + 1);
    ...
    function<ll(int, int, ll)> dfs = [&](int u, int t, ll f) -> ll {
        if (u == t || !f)
            return f;
    ...
    for (auto &[u, v, c, w] : orig) {
        add(u, v, INF, w);
    }

    ll mincost = 0;
    vector<ll> dist(n + 1);
    vector<int> pre(n + 1), incf_idx(n + 1);
    vector<bool> inq(n + 1);
    ...
    while (flow < k && spfa()) {
        ...
        int v = t;
        while (v != s) {
            aug = min(aug, edges[incf_idx[v]].cap);
            v = pre[v];
        }
        v = t;
        while (v != s) {
            Edge &e = edges[incf_idx[v]];
            e.cap -= aug;
            edges[incf_idx[v] ^ 1].cap += aug;
            v = pre[v];
        }
        flow += (int)aug;
        mincost += aug * dist[t];
