...
set<Point> black, white;

bool is_black(const Point &p) { return black.count(p); }
bool is_white(const Point &p) { return white.count(p); }
bool is_empty(const Point &p) { return !is_black(p) && !is_white(p); }

bool check_white_win() {
    for (const auto &b : black) {
        for (const auto &d : dirs) {
            Point p1 = b + d;
            Point p2 = b + d * 2;
            Point p3 = b + d * 3;
            if (is_white(p1) && is_white(p2) && is_white(p3))
                return true;
        }
    }
    return false;
...
int count_threats() {
    ...
    for (const auto &b : black) {
        for (const auto &d : dirs) {
            ...
            if (is_black(p1) || is_black(p2) || is_black(p3))
                continue;
            int w = is_white(p1) + is_white(p2) + is_white(p3);
            if (w == 2)
                cnt++;
        }
    }
...
int evaluate(const Point &p) {
    white.insert(p);
    if (check_white_win()) {
        white.erase(p);
        return 10000;
    }
    int score = count_threats();
    white.erase(p);
    return score;
}

vector<Point> get_candidates() {
    set<Point> cand_set;
    ...
    return vector<Point>(cand_set.begin(), cand_set.end());
}
inline void solve(int Task_Id) {
    black.clear();
    white.clear();
    black.insert(Point(0, 0));
    ...
    while (step <= 13) {
        ...
        if (step == 1) {
            ...
            vector<Point> cand = get_candidates();
            int best_score = -1;
