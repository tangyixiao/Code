...
inline void solve(int Task_Id) {
    ...
    mp[1] = 0;
    ll total_zero = n;
    auto get_len = [&](map<ll, int>::iterator it) -> ll {
        auto nxt = next(it);
        if (nxt == mp.end())
            return n - it->first + 1;
        else
            return nxt->first - it->first;
    };

    auto split = [&](ll pos) -> map<ll, int>::iterator {
        if (pos > n)
            return mp.end();
    ...
    while (q--) {
        ...
        auto itl = split(l);
        auto itr = (r + 1 <= n ? split(r + 1) : mp.end());
        ...
        if (itr != mp.end()) {
            auto left = prev(itr);
            if (left->second == itr->second) {
                mp.erase(itr);
            }
        }

        cout << modpow(3, total_zero) << '\n';
    }
