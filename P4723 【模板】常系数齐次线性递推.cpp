...
vector<int> poly_inv(const vector<int> &a, int n) {
    vector<int> inv(1, modpow(a[0], MOD - 2));
    int cur = 1;
    while (cur < n) {
        cur <<= 1;
        vector<int> tmp(a.begin(), a.begin() + min((int)a.size(), cur));
        vector<int> fa = inv, fb = tmp;
        int sz = cur * 2;
        fa.resize(sz, 0);
        fb.resize(sz, 0);
        ntt(fa, false);
        ntt(fb, false);
        for (int i = 0; i < sz; ++i) {
            fa[i] = 1LL * fa[i] * (2 - 1LL * fa[i] * fb[i] % MOD + MOD) % MOD;
        }
        ntt(fa, true);
        inv.resize(cur);
        for (int i = 0; i < cur; ++i)
...
vector<int> poly_mod(const vector<int> &A, const vector<int> &C, const vector<int> &invC_rev) {
    int n = (int)A.size() - 1, m = (int)C.size() - 1;
    if (n < m)
        return A;
    vector<int> A_rev = A;
    reverse(A_rev.begin(), A_rev.end());
    int lenQ = n - m + 1;
    vector<int> Q_rev = mul(A_rev, invC_rev);
    Q_rev.resize(lenQ);
    reverse(Q_rev.begin(), Q_rev.end());
    vector<int> QC = mul(Q_rev, C);
    vector<int> R(m);
    for (int i = 0; i < m; ++i) {
        int val = (i < (int)A.size() ? A[i] : 0);
        int sub = (i < (int)QC.size() ? QC[i] : 0);
        R[i] = (val - sub + MOD) % MOD;
    }
...
inline void solve(int Task_Id) {
    ...
    vector<int> invC_rev = poly_inv(C_rev, k);
    vector<int> res = {1};
    vector<int> base = {0, 1};
    long long e = n;
    while (e) {
        if (e & 1) {
            res = mul(res, base);
            res = poly_mod(res, C, invC_rev);
        }
        base = mul(base, base);
        base = poly_mod(base, C, invC_rev);
        e >>= 1;
    }
    int ans = 0;
    for (int i = 0; i < k; ++i) {
