#pragma G++ optimize(3, "Ofast", "inline", "unroll-loops")
#include <bits/stdc++.h>
using namespace std;
...
class FFTLimitExceededError : public std::exception {
	public:
		const char* what() const throw() {
			return "FFT limit exceeded";
		}
};
class NegativeRadicandError : public std::exception {
	public:
		const char* what() const throw() {
			return "Radicand is negative";
		}
};
...
class BigInteger {
	protected:
		...
		BigInteger(const digit_t* in, int len) : digits(nullptr), flag(true) {
			...
			std::memset(digits, 0, sizeof(digit_t) * (len + 1));
			for (int i = 0; i < len; i++)
				digits[i + 1] = in[i];
		}
...
signed main() {
	long long k, w;
	cin >> k >> w;
	for(long long i=2ll; i<=w/k; i++) {
		ans1=ans1+BigInteger::factorial((1ll<<k)-1ll)/(BigInteger::factorial(i)*BigInteger::factorial*(((1ll<<k)-1ll)-i));
	}
	if(w%k) {
		for(long long i=1ll; i<=(1ll<<(w%k)); i++) {
			ans2=ans2+BigInteger::factorial((1ll<<k)-1ll-i)/(BigInteger::factorial(w/k)*BigInteger::factorial*((1ll<<k)-1ll-i-(w/k));
		}
	}
	cout<<ans1+ans2;
