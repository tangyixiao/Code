//  Author: Tangyixiao
//  Time: 2026-08-13 10:10:59
//  Problem: P4149 [IOI 2011] Race
//  Contest: Luogu
//  URL: https://www.luogu.com.cn/problem/P4149
//  Memory Limit: 256 MB
//  Time Limit: 3000 ms
//  Interactive: false
//  Test Type: single
//  Batch ID: d8574b3a-d0d5-4e5d-832a-2a0d3d7230f4
//
// Algorithm:
// Complexity: O()
// Note:
...
const int N = 200005, M = 400005, K = 1000005, I = 0x3f3f3f3f;

struct Q {
    int u, p, d, e;
} q[N];

int n, k, h[N], to[M], nx[M], w[M], ec;
int sz[N], pa[N], o[N], st[N];
int d[K], use[K], uc, ds[N], de[N];
bool ban[N];
int ans = I;

void ae(int u, int v, int z) {
    to[++ec] = v;
    w[ec] = z;
    nx[ec] = h[u];
    h[u] = ec;
