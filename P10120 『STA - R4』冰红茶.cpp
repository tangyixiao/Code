...
int n, q;
ll S;
int curL, curR; // 当前口味为1的区间（上一次操作1的区间），初始为空
...
void apply_set(int u, int l, int r, ll val) {
    tr[u].min_val = val;
    tr[u].lazy_set = val;
}
...
void pushdown(int u, int l, int r) {
    if (tr[u].lazy_set != -1) {
        int mid = (l + r) >> 1;
        apply_set(u << 1, l, mid, tr[u].lazy_set);
        apply_set(u << 1 | 1, mid + 1, r, tr[u].lazy_set);
        tr[u].lazy_set = -1;
    }
}

void range_set(int u, int l, int r, int ql, int qr, ll val) {
    if (ql <= l && r <= qr) {
        if (tr[u].cnt > 0)
            apply_set(u, l, r, val);
        return;
...
void range_del(int u, int l, int r, int ql, int qr, ll thresh) {
    if (tr[u].cnt == 0)
        return;
    if (ql <= l && r <= qr) {
        if (tr[u].min_val > thresh)
            return;
        if (l == r) {
            tr[u].min_val = INF;
            tr[u].cnt = 0;
    ...
    int mid = (l + r) >> 1;
    if (ql <= mid)
        range_del(u << 1, l, mid, ql, qr, thresh);
    if (qr > mid)
        range_del(u << 1 | 1, mid + 1, r, ql, qr, thresh);
    pushup(u);
}
    ...
    while (q--) {
        ...
        if (op == 1) {
            ...
            if (curL > curR) {
                ...
                if (curL < l) {
                    int L = curL, R = min(curR, l - 1);
                    if (L <= R)
                        diffs.push_back({L, R});
                }
                if (curR > r) {
                    int L = max(curL, r + 1), R = curR;
                    if (L <= R)
                        diffs.push_back({L, R});
                }
                if (l < curL) {
                    int L = l, R = min(r, curL - 1);
                    if (L <= R)
                        diffs.push_back({L, R});
                }
                if (r > curR) {
                    int L = max(l, curR + 1), R = r;
                    if (L <= R)
                        diffs.push_back({L, R});
                }
            }
