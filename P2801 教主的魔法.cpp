...
//  Interactive: false
//  Test Type: single
//  Batch ID: 49929dee-17cf-4fb8-9e52-656689a3b971
//
// Algorithm:
// Complexity: O()
// Note:
...
int n, q, a[N], b[N], bl[N], tg[1005], l[1005], r[1005], cnt;
...
int ask(int L, int R, int c) {
    ...
    for (int i = l[y]; i <= R; i++)
        ans += a[i] + tg[y] >= c;
    for (int i = x + 1; i < y; i++) {
        int p = lower_bound(b + l[i], b + r[i] + 1, c - tg[i]) - b;
        ans += r[i] - p + 1;
    }
    return ans;
}
