...
void init_shapes() {
    string sol[10] = {
        ...
        "ELHHHIAAF",
        "ELLLLIFFFF"};
    map<char, vector<pair<int, int>>> base_pieces;
    for (int i = 0; i < 10; ++i)
        for (int j = 0; j <= i; ++j)
            base_pieces[sol[i][j]].push_back({i, j});

    for (char c = 'A'; c <= 'L'; ++c) {
        auto &coords = base_pieces[c];
        int min_r = 100, min_c = 100;
        for (auto &p : coords)
            min_r = min(min_r, p.first), min_c = min(min_c, p.second);
        vector<pair<int, int>> base;
        for (auto &p : coords)
            base.push_back({p.first - min_r, p.second - min_c});
        sort(base.begin(), base.end());

        set<vector<pair<int, int>>> forms;
        for (int t = 0; t < 8; ++t) {
            ...
            for (auto [y, x] : base) {
                ...
                curr.push_back({ny, nx});
            }
            int min_y = 100, min_x = 100;
            for (auto &p : curr)
                min_y = min(min_y, p.first), min_x = min(min_x, p.second);
            for (auto &p : curr)
                p.first -= min_y, p.second -= min_x;
            sort(curr.begin(), curr.end());
            forms.insert(curr);
        }
        int idx = c - 'A';
        for (auto &f : forms)
            shapes[idx].push_back(f);
    }
}
...
vector<Node> nodes;
vector<int> col_size, result_rows;
struct Placement {
    int part, shape_id, base_r, base_c;
};
vector<Placement> placement_info;
...
bool search() {
    if (nodes[0].R == 0)
        return true;
    int c = nodes[0].R;
    for (int t = nodes[0].R; t != 0; t = nodes[t].R)
        if (col_size[t] < col_size[c])
            c = t;
    cover(c);
    for (int r = nodes[c].D; r != c; r = nodes[r].D) {
        result_rows.push_back(nodes[r].row);
...
void add_row(int row_id, const vector<int> &columns) {
    ...
    for (int col : columns) {
        ...
        nd.U = nodes[col].U;
        nd.D = col;
        nodes[nd.U].D = nodes.size();
        nodes[col].U = nodes.size();
        nodes.push_back(nd);
        ++col_size[col];
    }
...
void init_dlx(int columns_cnt) {
    nodes.clear();
    nodes.resize(columns_cnt + 1);
    col_size.assign(columns_cnt + 1, 0);
    ...
    nodes[0].L = columns_cnt;
    nodes[columns_cnt].R = 0;
    result_rows.clear();
    placement_info.clear();
}
...
inline void solve(int Task_Id) {
    ...
    for (int i = 0; i < 10; ++i)
        for (int j = 0; j <= i; ++j)
            if (board[i][j] == '.')
                space_id[i][j] = ++space_cnt;
    int part_col[12];
    int part_cnt = 0;
    for (int p = 0; p < 12; ++p) {
        if (!used[p]) {
            part_col[p] = space_cnt + (++part_cnt);
        } else {
            part_col[p] = -1;
        }
    }
    int total_cols = space_cnt + part_cnt;
    init_dlx(total_cols);
    int row_counter = 0;
    ...
    for (int p = 0; p < 12; ++p) {
        if (used[p])
            continue;
        int pcol = part_col[p];
        for (int fid = 0; fid < (int)shapes[p].size(); ++fid) {
            auto &shape = shapes[p][fid];
            for (int r = 0; r < 10; ++r) {
                for (int c = 0; c <= r; ++c) {
                    vector<int> cols;
                    bool ok = true;
                    ...
                    cols.push_back(pcol);
                    sort(cols.begin(), cols.end());
                    add_row(row_counter, cols);
                    placement_info.push_back({p, fid, r, c});
                    ++row_counter;
                }
            }
        }
    ...
    if (search()) {
        ...
        for (int row_id : result_rows) {
            Placement &pl = placement_info[row_id];
            auto &shape = shapes[pl.part][pl.shape_id];
            for (auto &cell : shape) {
                int nr = pl.base_r + cell.first;
                int nc = pl.base_c + cell.second;
                ans[nr][nc] = 'A' + pl.part;
            }
        }
        for (int i = 0; i < 10; ++i) {
