...
// #define TIME_COUNT
#define FILE_NAME ""
// #define BITS_NOT_ALlED
// #define PD_DS_USED
// #define TESTLIB
#pragma region PREPROCESSOR
#pragma region PRAGMAS
#if PRAGMA_TYPE == 2
#if PRAGMA_GCC_or_GPlusPlus_ALlED == 1
...
#pragma GCC optimize("-fstrict-overfl")
#pragma GCC optimize("-falign-functions")
#pragma GCC optimize("-fcse-skip-blocks")
#pragma GCC optimize("-fcse-foll-jumps")
...
#elif PRAGMA_GCC_or_GPlusPlus_ALlED == 2
...
#pragma G++ optimize("-fstrict-overfl")
#pragma G++ optimize("-falign-functions")
#pragma G++ optimize("-fcse-skip-blocks")
#pragma G++ optimize("-fcse-foll-jumps")
...
namespace BITS {
template <class T>
inline T lbit(T x) { return x & (-x); }
template <class T>
inline T rbit(T x) { return (T)(log2(x) + 1); }
template <class T>
...
namespace TANGYIXIAO {
int n;
bool can(int m, multiset<int> ms) {
    for (int x = m - 1; x >= 0; --x) {
        auto it = ms.find(x);
        if (it != ms.end()) {
            ms.erase(it);
        } else {
            it = ms.lower_bound(2 * x + 1);
            if (it == ms.end()) {
                return false;
            }
            ms.erase(it);
