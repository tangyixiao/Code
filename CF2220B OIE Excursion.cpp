...
ll M; 
inline ll nxt(ll x) { return (x + 1) % M; }
inline ll prv(ll x) { return (x - 1 + M) % M; }

bool in_interval(ll l, ll r, ll p) {
    if (l <= r)
        return l <= p && p <= r;
    else
        return p >= l || p <= r;
}
vector<pair<ll, ll>> remove_pt(ll l, ll r, ll p) {
    vector<pair<ll, ll>> res;
    if (!in_interval(l, r, p)) {
        if (l <= r)
            res.emplace_back(l, r);
        else {
            res.emplace_back(l, M - 1);
    ...
    if (l == p) {
        ...
        if (l <= r) {
            if (nl <= r)
                res.emplace_back(nl, r);
        } else {
            if (nl != 0)
                res.emplace_back(nl, M - 1);
            if (0 <= r)
                res.emplace_back(0, r);
        }
    } else if (r == p) {
        ...
        if (l <= r) {
            if (l <= nr)
                res.emplace_back(l, nr);
        } else {
            res.emplace_back(l, M - 1);
            if (0 <= nr)
                res.emplace_back(0, nr);
        }
    } else {
        ll lp = prv(p);
        ll rp = nxt(p);
        if (l <= r) {
            ...
            if (rp <= r)
                res.emplace_back(rp, r);
        } else {
            if (p >= l) { 
                if (l <= lp)
                    res.emplace_back(l, lp);
                if (rp <= M - 1)
                    res.emplace_back(rp, M - 1);
                if (0 <= r)
                    res.emplace_back(0, r);
            } else { 
                res.emplace_back(l, M - 1);
                if (0 <= lp)
                    res.emplace_back(0, lp);
                if (rp <= r)
                    res.emplace_back(rp, r);
            }
        }
...
vector<pair<ll, ll>> gen_from_normal_interval(ll L, ll R, ll B) {
    vector<pair<ll, ll>> res;
    if (L <= prv(B)) {
        ll start = nxt(L);
        if (start != B) {
            if (start <= prv(B))
                res.emplace_back(start, prv(B));
            else {
                res.emplace_back(start, M - 1);
                res.emplace_back(0, prv(B));
            }
        }
    }
    if (R >= nxt(B)) {
        ll min_x = max(L, nxt(B));
        ll start = nxt(min_x);
        if (start <= prv(B))
            res.emplace_back(start, prv(B));
        else {
...
vector<pair<ll, ll>> gen_y_intervals(ll L, ll R, ll B) {
    vector<pair<ll, ll>> res;
    if (L <= R) {
        auto tmp = gen_from_normal_interval(L, R, B);
        res.insert(res.end(), tmp.begin(), tmp.end());
    } else {
        if (L <= M - 1) {
            auto tmp = gen_from_normal_interval(L, M - 1, B);
            res.insert(res.end(), tmp.begin(), tmp.end());
        }
        if (0 <= R) {
            auto tmp = gen_from_normal_interval(0, R, B);
            res.insert(res.end(), tmp.begin(), tmp.end());
        }
    }
    return res;
...
vector<pair<ll, ll>> merge_intervals(vector<pair<ll, ll>> vec) {
    if (vec.empty())
        return {};
    ...
    }  if (merged.size() > 1 && merged[0].first == 0 && merged.back().second == M - 1) {
  
        merged[0].first = merged.back().first; 
        merged[0].second = merged.back().second;
        merged.pop_back();
    }
    return merged;
}
...
inline void solve(int Task_Id) {
    ...
    if (B[0] == 0) {
        if (M > 1)
            cur.emplace_back(1, M - 1);
    } else if (B[0] == M - 1) {
        cur.emplace_back(0, M - 2);
    } else {
        cur.emplace_back(0, B[0] - 1);
        cur.emplace_back(B[0] + 1, M - 1);
    }
    ...
    for (int i = 0; i < n - 1; ++i) {
        ...
        vector<pair<ll, ll>> nxt_intervals;
        for (auto &[L, R] : cur) {
            auto gen = gen_y_intervals(L, R, Bi);
            for (auto &seg : gen) {
                auto splitted = remove_pt(seg.first, seg.second, Bnext);
                for (auto &s : splitted)
                    nxt_intervals.push_back(s);
            }
        }
        cur = merge_intervals(nxt_intervals);
    }
