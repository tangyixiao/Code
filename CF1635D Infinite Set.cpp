...
//  Interactive: false
//  Test Type: single
//  Batch ID: 4b79bacf-84fa-46ae-ad70-d82d66bdd2ff
//
// Algorithm:
// Complexity: O()
// Note:
...
inline void solve(int Task_Id) {
    ...
    ans = 0, f[0] = f[1] = s[0] = 1, s[1] = 2;
    for (int i = 2; i <= p; i++) {
        f[i] = (f[i - 1] + f[i - 2]) % mod, s[i] = (s[i - 1] + f[i]) % mod;
    }
    for (int i = 1, l = 0, x = a[1]; i <= n; i++, l = flag = 0, x = a[i]) {
        for (; x;) {
            if (x == 1) {
                break;
            }
