...
inline void solve(int Task_Id) {
    ...
    vector<vector<long long>> C(D, vector<long long>(D, 0));
    vector<int> cnt(D, 0);
    for (int x = 1; x <= n; ++x) {
        int a = belong[x];
        for (int b = 0; b < D; ++b)
    ...
    vector<vector<int>> K(D, vector<int>(D, 0));
    for (int a = 0; a < D; ++a) {
        for (int b = 0; b < D; ++b) {
            if (a == b) {
                K[a][a] = inv2;
            } else {
                long long num = C[a][b] % MOD;
                int denom = 1LL * s[a] * s[b] % MOD;
                K[a][b] = num * modpow(denom, MOD - 2) % MOD;
            }
        }
    }
    ...
    for (int i = 1; i <= n; ++i)
        inv[i] = 1LL * inv[i] * fact[i - 1] % MOD;

    vector<int> qi(q), qx(q);
    vector<vector<int>> indices(D);
    for (int k = 0; k < q; ++k) {
        cin >> qi[k] >> qx[k];
        int d = id_of[qy];
        indices[d].push_back(qi[k]);
    }
    for (int d = 0; d < D; ++d) {
        sort(indices[d].begin(), indices[d].end());
        indices[d].erase(unique(indices[d].begin(), indices[d].end()), indices[d].end());
    }

    vector<BIT> bit;
    for (int d = 0; d < D; ++d)
        bit.emplace_back(indices[d].size());

    int U = n;
    vector<int> f(D, 0), r = s;
    long long N = fact[n];
    vector<int> sum_idx(D, 0);
    long long sum_idx_total = 0;
    int f_total = 0;
    long long Y_total = 0;

    vector<vector<int>> X(D, vector<int>(D, 0));
    vector<int> col_sum(D, 0);
    long long E_FF_diff_val = 0;

    vector<int> S(D, 0);
    vector<int> P_fF(D, 0);

    for (int d = 0; d < D; ++d)
        P_fF[d] = 1LL * T[d] * inv[n] % MOD;
    ...
    for (int k = 0; k < q; ++k) {
        ...
        vector<int> F_bef(D, 0), F_aft(D, 0);
        int forced_before_total = 0;
        for (int e = 0; e < D; ++e) {
            if (indices[e].empty())
                continue;

            int rank = lower_bound(indices[e].begin(), indices[e].end(), i) - indices[e].begin();
            F_bef[e] = bit[e].sum(rank);
            F_aft[e] = f[e] - F_bef[e];
            forced_before_total += F_bef[e];
        }

        int U_bef = i - 1 - forced_before_total;
        int U_aft = U - 1 - U_bef;
        ...
        N = N * inv[U] % MOD * r[d] % MOD;
        U--;
        f[d]++;
        r[d]--;
        sum_idx[d] = (sum_idx[d] + i) % MOD;
        sum_idx_total = (sum_idx_total + i) % MOD;
        f_total++;

        Y_total = (sum_idx_total - f_total + MOD - 1LL * f_total * (f_total - 1) / 2 % MOD + MOD) % MOD;
        ...
        for (int e = 0; e < D; ++e) {
            ...
            int add_de = F_aft[e];
            if (add_ed) {
                X[e][d] = (X[e][d] + add_ed) % MOD;
                col_sum[d] = (col_sum[d] + add_ed) % MOD;
                E_FF_diff_val = (E_FF_diff_val + 1LL * add_ed * K[e][d]) % MOD;
            }
            if (add_de) {
                X[d][e] = (X[d][e] + add_de) % MOD;
                col_sum[e] = (col_sum[e] + add_de) % MOD;
                E_FF_diff_val = (E_FF_diff_val + 1LL * add_de * K[d][e]) % MOD;
            }
        }

        if (old_fd) {
            X[d][d] = (X[d][d] + old_fd) % MOD;
            col_sum[d] = (col_sum[d] + old_fd) % MOD;
            E_FF_diff_val = (E_FF_diff_val + 1LL * old_fd * inv2) % MOD;
        }

        int pos_rank = lower_bound(indices[d].begin(), indices[d].end(), i) - indices[d].begin() + 1;
        bit[d].add(pos_rank, 1);

        for (int e = 0; e < D; ++e) {
            S[e] = (S[e] + K[e][d]) % MOD;
        }
        long long sum_f_PfF = 0;
        if (U > 0) {
            int invU = inv[U];
            for (int e = 0; e < D; ++e) {
                P_fF[e] = 1LL * (T[e] - S[e] + MOD) * invU % MOD;
                sum_f_PfF = (sum_f_PfF + 1LL * f[e] * P_fF[e]) % MOD;
            }
        } else {
            fill(P_fF.begin(), P_fF.end(), 0);
            sum_f_PfF = 0;
        }

        long long sum_YP = 0;
        for (int e = 0; e < D; ++e) {
            long long Y_e = (sum_idx[e] - f[e] - col_sum[e]) % MOD;
            if (Y_e < 0)
                Y_e += MOD;
            sum_YP = (sum_YP + Y_e * P_fF[e]) % MOD;
        }

        long long E_Mixed = Y_total + 1LL * U * sum_f_PfF - 2LL * sum_YP;
        E_Mixed %= MOD;
        if (E_Mixed < 0)
            E_Mixed += MOD;

        long long E_FF = 1LL * U * (U - 1) % MOD * inv2 % MOD * inv2 % MOD;
        long long E_FF_same = 0;
        for (int e = 0; e < D; ++e) {
            E_FF_same = (E_FF_same + 1LL * f[e] * (f[e] - 1) % MOD * inv2 % MOD * inv2) % MOD;
        }

        long long E_total = (E_FF + E_FF_same + E_FF_diff_val + E_Mixed) % MOD;
        ans[k] = N * E_total % MOD;
    }
