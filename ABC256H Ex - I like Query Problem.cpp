...
void apply_assign(int u, int l, int r, int val) {
    tr[u].sum = (ll)val * (r - l + 1);
    tr[u].maxv = tr[u].minv = val;
    tr[u].tag = val;
}
...
void pushdown(int u, int l, int r) {
    if (tr[u].tag != -1) {
        int mid = (l + r) >> 1;
        apply_assign(u << 1, l, mid, tr[u].tag);
        apply_assign(u << 1 | 1, mid + 1, r, tr[u].tag);
        tr[u].tag = -1;
    }
}

void update_assign(int u, int l, int r, int L, int R, int val) {
    if (L <= l && r <= R) {
        apply_assign(u, l, r, val);
        return;
    }
    pushdown(u, l, r);
    int mid = (l + r) >> 1;
    if (L <= mid)
        update_assign(u << 1, l, mid, L, R, val);
    if (R > mid)
        update_assign(u << 1 | 1, mid + 1, r, L, R, val);
    pushup(u);
}

void update_div(int u, int l, int r, int L, int R, int x) {
    if (L <= l && r <= R && tr[u].maxv == tr[u].minv) {
        int newv = tr[u].maxv / x;
        apply_assign(u, l, r, newv);
        return;
    }
    pushdown(u, l, r);
    int mid = (l + r) >> 1;
    if (L <= mid)
        update_div(u << 1, l, mid, L, R, x);
    if (R > mid)
        update_div(u << 1 | 1, mid + 1, r, L, R, x);
    pushup(u);
}
...
ll query_sum(int u, int l, int r, int L, int R) {
    ...
    if (R > mid)
        res += query_sum(u << 1 | 1, mid + 1, r, L, R);
    return res;
}
