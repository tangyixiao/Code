...
//  Interactive: false
//  Test Type: single
//  Batch ID: ee1767a0-cefa-4e4a-a331-b927387101bf
//
// Algorithm:
// Complexity: O()
// Note:
...
namespace TANGYIXIAO {
const int N = 8005;
int n, m, wf[4005], hb[4005], tot;
vector<int> g[N];
map<string, int> mp;
int df[N], lo[N], st[N], tp, ins[N], tm, sc, id[N];
...
inline void solve(int Task_Id) {
    ...
    for (int i = 0; i < tot; ++i)
        if (!df[i])
            tj(i);
    for (int i = 1; i <= n; ++i)
        puts(id[wf[i]] == id[hb[i]] ? "Unsafe" : "Safe");
    return;
}
} // namespace TANGYIXIAO
