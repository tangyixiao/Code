...
inline void solve(int Task_Id) {
    ...
    for (int i = 1; i <= n; i++) {
        if (!red[i] && !vis[i]) {
            ...
            vector<double> F0(n + 1, INF), F1(n + 1, INF);
            for (int idx = comp.size() - 1; idx >= 0; idx--) {
                ...
                for (int u : children[v]) {
                    sum_F1 += F1[u];
                    double delta = (F0[u] >= INF / 2) ? INF : F0[u] - F1[u];
                    deltas.push_back(delta);
                }
                sort(deltas.begin(), deltas.end());
                int m = deltas.size();
                vector<double> pref(m + 1, 0.0);
                for (int j = 0; j < m; j++) {
                    pref[j + 1] = pref[j] + deltas[j];
                    if (pref[j + 1] > INF)
                        pref[j + 1] = INF;
                }
                for (int c = 0; c < 2; c++) {
                    ...
                    for (int x = 0; x <= m; x++) {
                        if (r + x == 0)
                            continue;
                        double cost = (double)deg[v] / (r + x) + sum_F1 + pref[x];
                        if (cost < best)
                            best = cost;
                    }
                    if (c == 0)
