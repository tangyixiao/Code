...
inline void solve(int Task_Id) {
    ...
    struct Dinic {
        ...
        int n;
        Dinic(int n_) : n(n_), g(n_), level(n_), iter(n_) {}
    ...
    auto build = [&](int cap1, int capn) {
        Dinic dinic(2 * n + 2);
        int S = 0, T = 2 * n + 1;
        for (int i = 1; i <= n; ++i) {
            int in = i, out = i + n;
            if (i == 1)
                dinic.add_edge(in, out, cap1);
            else if (i == n)
                dinic.add_edge(in, out, capn);
            else
                dinic.add_edge(in, out, 1);
            if (dp[i] == 1) {
                if (i == 1)
                    dinic.add_edge(S, in, cap1);
                else
                    dinic.add_edge(S, in, 1);
            }
            if (dp[i] == s) {
                if (i == n)
                    dinic.add_edge(out, T, capn);
                else
                    dinic.add_edge(out, T, 1);
            }
        }
        for (int i = 1; i <= n; ++i) {
            for (int j = i + 1; j <= n; ++j) {
