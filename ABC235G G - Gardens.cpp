...
//  Interactive: false
//  Test Type: single
//  Batch ID: a8010fc1-4930-401b-afdd-b2c8d767a7bc
//
// Algorithm:
// Complexity: O()
// Note:
...
static int f[5000005], g[5000005], v[5000005];

void go(int *f, int a, int n) {
    f[0] = 1;
    ll x = a ? 0 : 1;
    for (int i = 1; i <= n; i++) {
        f[i] = (2LL * f[i - 1] - x + P) % P;
...
inline void solve(int Task_Id) {
    ...
    ll y = c < n ? x * (n - c) % P * v[n] % P : 0;
    ll q = 1, ans = 0;
    const ll iv2 = (P + 1LL) / 2;

    for (int k = 0, j = n; k <= n; k++, j--) {
        ll z = (k & 1) ? P - q : q;
        ans = (ans + z * f[j] % P * g[j] % P * h) % P;
