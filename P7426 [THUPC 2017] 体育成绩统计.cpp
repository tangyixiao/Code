...
namespace TANGYIXIAO {
int parse_min_sec(const string &s) {
    int a, b;
    sscanf(s.c_str(), "%d'%d\"", &a, &b);
    return a * 60 + b;
}

int parse_hms(const string &s) {
    int h, m, sec;
    sscanf(s.c_str(), "%d:%d:%d", &h, &m, &sec);
    return h * 3600 + m * 60 + sec;
}

long long parse_datetime(const string &date, const string &time_str) {
    ...
    for (int i = 0; i < month - 1; ++i)
        days += month_days[i];
    days += (day - 1);
    return (long long)days * 86400 + h * 3600 + m * 60 + s;
}

int get_sunshine_score(int cnt) {
    if (cnt >= 21)
        return 10;
    if (cnt >= 19)
        return 9;
...
int get_attendance_score(int cnt) {
    if (cnt >= 18)
        return 5;
    if (cnt >= 15)
        return 4;
...
struct Student {
    ...
    char gender;
    int special_score;    
    string run_score_str; 
    char physical_test;
    int plan_test_score; 
    int camp_times;     

    int legal_run_cnt = 0;        
    long long last_legal_end = -1e18;
};
...
inline void solve(int Task_Id) {
    ...
    for (int i = 0; i < n; ++i) {
        ...
        cin >> st.id >> st.gender >> st.special_score >> st.run_score_str >> st.physical_test >> st.plan_test_score >> st.camp_times;
        id_to_idx[st.id] = i;
    }
    ...
    cin >> m;
    while (m--) {
        string date, stu_id, start_time, end_time, pause_str;
        double l;
        int steps;
        cin >> date >> stu_id >> start_time >> end_time >> l >> pause_str >> steps;

        auto it = id_to_idx.find(stu_id);
        if (it == id_to_idx.end())
            continue; 
        Student &st = students[it->second];

        double min_dist = (st.gender == 'M') ? 3.0 : 1.5;
        if (l < min_dist - 1e-9)
            continue;

        long long start_sec = parse_datetime(date, start_time);
        long long end_sec = parse_datetime(date, end_time);
        long long elapsed = end_sec - start_sec;
        if (elapsed <= 0)
            continue;

        int pause_sec = parse_min_sec(pause_str);
        if (pause_sec > 270)
            continue;

        double dist_m = l * 1000.0;
        double speed = dist_m / elapsed;
        if (speed < 2.0 - 1e-9 || speed > 5.0 + 1e-9)
            continue;

        if (steps == 0)
            continue;
        double stride = dist_m / steps;
        if (stride > 1.5 + 1e-9)
            continue;

        if (start_sec - st.last_legal_end < 21600)
            continue;

        ++st.legal_run_cnt;
        st.last_legal_end = end_sec;
    }

    for (const Student st : students) {
        int total = st.special_score;

        int run_sec = parse_min_sec(st.run_score_str);
        int run_score = 0;
        if (st.gender == 'M') {
            vector<pair<int, int>> male_std = {
                {750, 20}, {780, 18}, {810, 16}, {840, 14}, {870, 12}, {910, 10}, {950, 8}, {990, 6}, {1030, 4}, {1080, 2}};
            for (auto &p : male_std) {
                if (run_sec <= p.first) {
                    run_score = p.second;
                    break;
            ...
            vector<pair<int, int>> female_std = {
                {400, 20}, {417, 18}, {434, 16}, {451, 14}, {470, 12}, {485, 10}, {500, 8}, {515, 6}, {530, 4}, {540, 2}};
            for (auto &p : female_std) {
                if (run_sec <= p.first) {
                    run_score = p.second;
                    break;
                }
        ...
        total += get_sunshine_score(st.legal_run_cnt);

        total += (st.physical_test == 'P') ? 10 : 0;

        int attendance_cnt = st.camp_times + st.legal_run_cnt;
        int attendance_score = get_attendance_score(attendance_cnt);
        total += st.plan_test_score + attendance_score;

        cout << st.id << " " << total << " " << get_grade(total) << "\n";
    }
