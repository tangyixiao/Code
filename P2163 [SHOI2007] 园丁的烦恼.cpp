...
//  Interactive: false
//  Test Type: single
//  Batch ID: e71c6043-9f38-455b-af99-b6500a8080e8
//
// Algorithm:
// Complexity: O()
// Note:
...
namespace TANGYIXIAO {
const int Z = 500005, V = 2000005;
static unsigned long long P[Z], Q[V];
static int Y[Z], F[Z], A[Z], K, U;
static inline void E(int x, int y, int i, int s) { Q[K++] = ((unsigned long long)(x + 1) << 40) | ((unsigned long long)(upper_bound(Y, Y + U, y) - Y) << 20) | ((unsigned long long)i << 1) | s; }
inline void solve(int Task_Id) {
    int N, M;
    cin >> N >> M;
    for (int I = 0; I < N; ++I) {
        int x, y;
        cin >> x >> y;
        P[I] = ((unsigned long long)x << 24) | y;
        Y[I] = y;
    }
    sort(Y, Y + N);
    U = unique(Y, Y + N) - Y;
    for (int I = 0; I < N; ++I) {
        int x = P[I] >> 24, y = P[I] & 16777215;
        P[I] = ((unsigned long long)x << 20) | (unsigned)(lower_bound(Y, Y + U, y) - Y + 1);
    }
    sort(P, P + N);
    for (int I = 0; I < M; ++I) {
        int a, b, c, d;
    ...
    for (int I = 0; I < K; ++I) {
        int X = (int)(Q[I] >> 40) - 1;
        while (J < N && (int)(P[J] >> 20) <= X) {
            for (int p = P[J] & 1048575; p <= U; p += p & -p)
                ++F[p];
            ++J;
        }
        int y = (Q[I] >> 20) & 1048575, s = 0;
