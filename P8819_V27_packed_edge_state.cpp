...
namespace TANGYIXIAO {
    ...
    constexpr uint64_t PM=EM<<19;
    constexpr uint64_t UM=EM<<38;
    constexpr uint64_t EB=1ULL<<57;

    inline void add_ex(int e,int v) {
        int ne=head[v];
        rs[e]=(rs[e]&UM)|EB|(uint64_t)ne;

        if (ne) {
            rs[ne]=(rs[ne]&~PM)|((uint64_t)e<<19);
        }
    ...
    inline void del_ex(int e,int v) {
        ...
        if (pe) {
            rs[pe]=(rs[pe]&~EM)|(uint64_t)ne;
        } else {
            head[v]=ne;
        }

        if (ne) {
            rs[ne]=(rs[ne]&~PM)|((uint64_t)pe<<19);
        }
