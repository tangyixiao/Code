...
//  Interactive: false
//  Test Type: single
//  Batch ID: f90d07d0-a147-4efc-824b-2481633a2a1c
//
// Algorithm:
// Complexity: O()
// Note:
...
namespace TANGYIXIAO {
const int N = 1505, M = 10005, INF = 1e9;
int m, n, S, T, ec, h[N], to[M], nx[M], w[M], c[M];
int d[N], pre[N], q[N], inq[N], a[25][45], id[25][45], cnt;
...
inline int calc(int op) {
    ...
    for (int i = 1; i <= n; i++) {
        for (int j = 1; j <= m + i - 1; j++) {
            int x = id[i][j];
            add(x, x + cnt, op == 1 ? 1 : INF, a[i][j]);

            if (i < n) {
                int v = op == 2 ? 1 : INF;
                add(x + cnt, id[i + 1][j], v, 0);
                add(x + cnt, id[i + 1][j + 1], v, 0);
            } else {
                add(x + cnt, T, INF, 0);
            }
        }
