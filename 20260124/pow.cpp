...
public:
	...
	BigInteger pow(const BigInteger&, const BigInteger&) const;
...
BigInteger& BigInteger::operator= (const std::vector<bool>& a) {
	if (a == std::vector<bool>{0}) return *this = 0;
	std::vector<bool> res;
	if (a[0] == 0) return build_binary(a);
	res.resize(a.size());
	for (int i = 0; i < (int) a.size(); ++i) res[i] = !a[i];
	BigInteger x; x.build_binary(res);
	return *this = -x - 1;
}
...
BigInteger BigInteger::pow(const BigInteger& x, const BigInteger& p) const {
	BigInteger res = 1, a = *this % p;
	for (BigInteger t = x; t != 0; t >>= 1) {
		if (t %2==1) res = res * a % p;
		a = a * a % p;
	}
	return res;
}
