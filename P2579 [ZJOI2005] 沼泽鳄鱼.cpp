...
namespace TANGYIXIAO {
    inline void solve(int Task_Id) {
        ...
        for (int i = 0; i < NFish; ++i) {
            ...
            scanf("%d", &T);
            int pos[4];
            for (int j = 0; j < T; ++j) scanf("%d", &pos[j]);
            for (int t = 0; t < 12; ++t) {
                int p = pos[t % T];
                fish[t][p] = 1;
            }
        ...
        for (int t = 0; t < 12; ++t) {
            ...
            for (int i = 0; i < N; ++i) {
                for (int j = 0; j < N; ++j) {
                    if (adj[i][j] && !fish[forbid][j]) {
                        M_t[t][i][j] = 1;
                    }
                }
            }
        }
        auto matmul = [&](int a[50][50], int b[50][50], int c[50][50]) {
            int tmp[50][50] = {0};
            for (int i = 0; i < N; ++i) {
                for (int k = 0; k < N; ++k) {
                    if (a[i][k]) {
        ...
        int period[50][50];
        memcpy(period, M_t[0], sizeof(period));
        for (int t = 1; t < 12; ++t) {
            int tmp[50][50];
            matmul(period, M_t[t], tmp);
        ...
        for (int i = 0; i < N; ++i)
            for (int j = 0; j < N; ++j)
                res[i][j] = (i == j);
        int base[50][50];
        memcpy(base, period, sizeof(base));
        while (cnt) {
            if (cnt & 1) {
                int tmp[50][50];
                matmul(res, base, tmp);
