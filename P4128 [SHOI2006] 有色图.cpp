...
//  Interactive: false
//  Test Type: single
//  Batch ID: b24649b3-c369-4f81-bc3f-f68847d9282a
//
// Algorithm:
// Complexity: O()
// Note:
...
int parts[55];
ll ans = 0;
void dfs(int rem, int max_val, int len) {
    if (rem == 0) {
        int freq[55] = {0};
        for (int i = 0; i < len; ++i)
            freq[parts[i]]++;
        ...
        for (int i = 1; i <= n; ++i) {
            ...
            for (int j = i + 1; j <= n; ++j) {
                if (!freq[j])
                    continue;
                c += freq[i] * freq[j] * gcd_tab[i][j];
            }
            if (freq[i] > 1)
                c += (ll)freq[i] * (freq[i] - 1) / 2 * i;
        }
        ll denom = 1;
        for (int i = 1; i <= n; ++i) {
            if (freq[i]) {
                denom = denom * modpow(i, freq[i]) % p;
                denom = denom * fact[freq[i]] % p;
            }
        }
        ll term = modpow(m, c) * modpow(denom, p - 2) % p;
        ans = (ans + term) % p;
        return;
    }
    for (int v = min(max_val, rem); v >= 1; --v) {
        parts[len] = v;
        dfs(rem - v, v, len + 1);
    }
}
