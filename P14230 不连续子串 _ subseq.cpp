...
int H[MAXN][MAXN]; 
int HM_M[MAXN]; int HM_C[MAXN]; int a[MAXN];
int last_pos[MAXN];
int T_dp[MAXN];
inline void solve(int Task_Id) {
    int n;
    ...
    long long current_TM = 1;
    long long current_TC = 1;
    HM_M[0] = 1;
    HM_C[0] = 1;
    ...
    for (int i = 1; i <= n; i++) {
        int x = a[i];
        int L = last_pos[x];
        long long r_m = (current_TM - (L > 0 ? HM_M[L - 1] : 0) + MOD) % MOD;
        long long r_c = (current_TC - (L > 0 ? HM_C[L - 1] : 0) + MOD) % MOD;

        long long s_m_i = 0;
        long long s_dp_x = 0;

        long long r_dp_x = (T_dp[x] - (L > 0 ? H[L - 1][x] : 0) + MOD) % MOD;

        s_m_i = (2 * r_m - r_dp_x + MOD) % MOD;
        s_dp_x = r_m;

        for (int v = 1; v <= n; v++) {
            if (v == x)
                continue;
            int delta = (T_dp[v] - (L > 0 ? H[L - 1][v] : 0) + MOD) % MOD;
            T_dp[v] = (T_dp[v] + delta) % MOD;
        }

        T_dp[x] = (T_dp[x] + s_dp_x) % MOD;
        current_TM = (current_TM + s_m_i) % MOD;
        current_TC = (current_TC + r_c) % MOD;
