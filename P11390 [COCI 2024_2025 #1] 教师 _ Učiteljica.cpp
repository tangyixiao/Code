...
struct SegTree {
    int minv[4 * MAXN], cnt[4 * MAXN], lazy[4 * MAXN];
    ...
    pair<int, int> query(int p, int l, int r, int ql, int qr) {
        ...
        if (qr > mid) {
            ...
            if (res.first == INT_MAX)
                ...
                if (right.first < res.first)
                    res = right;
                else if (right.first == res.first)
        ...
        if (mask == 0) {
            ...
            for (int r = 1; r <= n; ++r) {
                ...
                for (int c = 0; c <= maxc; ++c) {
                    ...
                    bool c_in = inS(c, mask);
                    bool cp1_in = inS(c + 1, mask);
                    int delta = 0;
                    if (c_in && !cp1_in)
                        delta = -1;
                    else if (!c_in && cp1_in)
