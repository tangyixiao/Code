...
double len(Pt a) { return sqrt(a.x * a.x + a.y * a.y); }
...
inline void solve(int Task_Id) {
    ...
    int m = 0;
    for (int i = 0; i < n; ++i) {
        while (m >= 2 && cross(h[m - 1] - h[m - 2], p[i] - h[m - 2]) <= 0)
            --m;
        h[m++] = p[i];
    }
    int k = m;
    for (int i = n - 2; i >= 0; --i) {
        while (m > k && cross(h[m - 1] - h[m - 2], p[i] - h[m - 2]) <= 0)
            --m;
        h[m++] = p[i];
    }
    if (m > 1)
    ...
    Pt v0 = h[1] - h[0];
    for (int i = 1; i < m; ++i) {
        if (cross(v0, h[i] - h[0]) > cross(v0, h[pidx] - h[0]))
            pidx = i;
        if (dot(v0, h[i] - h[0]) > dot(v0, h[qidx] - h[0]))
            qidx = i;
        if (dot(v0, h[i] - h[0]) < dot(v0, h[ridx] - h[0]))
            ridx = i;
    }
    ...
    for (int i = 0; i < m; ++i) {
        int j = (i + 1) % m;
        Pt v = h[j] - h[i];
        while (cross(v, h[(pidx + 1) % m] - h[i]) > cross(v, h[pidx] - h[i]))
            pidx = (pidx + 1) % m;
        while (dot(v, h[(qidx + 1) % m] - h[i]) > dot(v, h[qidx] - h[i]))
            qidx = (qidx + 1) % m;
        while (dot(v, h[(ridx + 1) % m] - h[i]) < dot(v, h[ridx] - h[i]))
            ridx = (ridx + 1) % m;

        double L = len(v);
        double cp = cross(v, h[pidx] - h[i]);
        double dq = dot(v, h[qidx] - h[i]);
        double dr = dot(v, h[ridx] - h[i]);
        double area = cp * (dq - dr) / (L * L);
        if (area < min_area) {
            min_area = area;
            double invL = 1.0 / L;
            Pt e = Pt(v.x * invL, v.y * invL);
            Pt n_unit = Pt(-e.y, e.x);
            double tr = dr * invL;
            double tq = dq * invL;
            double d = cp * invL;
            ans[0] = h[i] + e * tr;
            ans[1] = h[i] + e * tq;
            ans[2] = ans[1] + n_unit * d;
            ans[3] = ans[0] + n_unit * d;
        }
    }
