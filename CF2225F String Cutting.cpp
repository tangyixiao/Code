...
struct SuffixArray {
    ...
    int cmp(int l1, int len1, int l2, int len2) {
        int l = get_lcp(l1, l2);
        if (l >= min(len1, len2)) {
            if (len1 == len2)
                return 0;
...
bool can(const string &s, int n, int l, int k, int pos, int len, SuffixArray &sa) {
    int i = 0, less = 0, total = 0;
    bool has = false;
    while (i < n) {
        int lo = l, hi = n - i, best = -1;
        while (lo <= hi) {
            int mid = (lo + hi) / 2;
            int c = sa.cmp(i, mid, pos, len);
            if (c <= 0) {
                best = mid;
                lo = mid + 1;
            } else {
...
inline void solve(int Task_Id) {
    ...
    string ans;
    bool found = false;
    for (int idx = n - 1; idx >= 0 && !found; --idx) {
        int start = sa.sa[idx];
        int maxlen = n - start;
        for (int len = maxlen; len >= l; --len) {
            if (can(s, n, l, k, start, len, sa)) {
