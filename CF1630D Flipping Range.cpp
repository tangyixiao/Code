...
//  Interactive: false
//  Test Type: single
//  Batch ID: 80bf89d4-12d2-4b18-8c3c-e3d972b195aa
//
// Algorithm:
// Complexity: O()
// Note:
...
namespace TANGYIXIAO {
const int N = 1e6 + 5, inf = 4e18;
int T, n, m, g, a[N], f[N], r[N], s, c[2];
inline void solve(int Task_Id) {
    cin >> n >> m;
    for (int i = 1; i <= n; i++) {
        cin >> a[i];
    ...
    for (int i = 1; i <= n; i++) {
        s += abs(a[i]), r[(i - 1) % g + 1] = min(r[(i - 1) % g + 1], abs(a[i]));
        if (a[i] < 0) {
            f[(i - 1) % g + 1] ^= 1;
        }
    }
