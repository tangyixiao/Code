...
inline void solve(int Task_Id) {
    ...
    long long prev_raw = q1;
    int prev_a = q1 % C + 1;
    int first_a = prev_a;

    double ans = 0.0;
    for (int i = 2; i <= n; ++i) {
        long long cur_raw = (prev_raw * A + B) % 100000001;
        int cur_a = cur_raw % C + 1;
        ans += 1.0 / max(prev_a, cur_a);
        prev_raw = cur_raw;
        prev_a = cur_a;
    }
    ans += 1.0 / max(prev_a, first_a);
