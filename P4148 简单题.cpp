...
//  Interactive: false
//  Test Type: single
//  Batch ID: 22a00bc9-d120-4d5a-86db-f025d02f7732
//
// Algorithm:
// Complexity: O()
// Note:
...
namespace TANGYIXIAO {
const int Z = 200005;
static int X[Z], Y[Z], A[Z], B[Z], C[Z], D[Z], S[Z], V[Z], L[Z], R[Z], W[Z], T[Z];
int M, E, K, O, P;
void U(int u) {
    S[u] = V[u] + S[L[u]] + S[R[u]];
    W[u] = 1 + W[L[u]] + W[R[u]];
...
int cmp(int a, int b) { return K ? (X[a] == X[b] ? Y[a] < Y[b] : X[a] < X[b]) : (Y[a] == Y[b] ? X[a] < X[b] : Y[a] < Y[b]); }
int G(int l, int r, int d) {
    if (l > r)
        return 0;
    int m = (l + r) / 2;
    K = d;
    nth_element(T + l, T + m, T + r + 1, cmp);
    int u = T[m];
    L[u] = G(l, m - 1, d ^ 1);
    R[u] = G(m + 1, r, d ^ 1);
    U(u);
...
void I(int &u, int x, int y, int v, int d) {
    if (!u) {
        u = ++M;
        X[u] = x;
        Y[u] = y;
    ...
    S[u] += v;
    W[u]++;
    if (d ? x < X[u] || (x == X[u] && y < Y[u]) : y < Y[u] || (y == Y[u] && x < X[u]))
        I(L[u], x, y, v, d ^ 1);
    else
        I(R[u], x, y, v, d ^ 1);
    U(u);
    if (W[u] * 3 < max(W[L[u]], W[R[u]]) * 4) {
        E = 0;
        F(u);
        u = G(1, E, d);
    }
}
int Q(int u, int x1, int y1, int x2, int y2) {
    if (!u || A[u] > x2 || B[u] < x1 || C[u] > y2 || D[u] < y1)
        return 0;
    if (A[u] >= x1 && B[u] <= x2 && C[u] >= y1 && D[u] <= y2)
        return S[u];
    int r = 0;
    if (X[u] >= x1 && X[u] <= x2 && Y[u] >= y1 && Y[u] <= y2)
        r += V[u];
    return r + Q(L[u], x1, y1, x2, y2) + Q(R[u], x1, y1, x2, y2);
}
inline void solve(int Task_Id) {
    int N;
    cin >> N;
