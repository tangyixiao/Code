...
inline void solve(int Task_Id) {
    ...
    while (t--) {
        ...
        int w = n, s[2] = {p, q};
        for (int k = 0; k < 2; ++k) {
            int st = s[k], cmn = a[st], cmx = a[st];
            int c = 0, fmn = 0, fmx = 0, lmn, lmx;
            int i = st % n + 1, v = 1;
            while (v < n) {
                int val = a[i];
                int nmn = cmn < val ? cmn : val;
                int nmx = cmx > val ? cmx : val;
                if (nmx - nmn > m) {
                    if (!c)
                        fmn = cmn, fmx = cmx;
                    lmn = cmn, lmx = cmx;
            ...
            if (c == 1)
                fmn = lmn, fmx = lmx;
            int gmn = fmn < lmn ? fmn : lmn;
            int gmx = fmx > lmx ? fmx : lmx;
            if (gmx - gmn <= m)
                --c;
            if (c < w)
                w = c;
