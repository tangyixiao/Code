//  Author: Tangyixiao
//  Time: 2026-08-24 09:03:12
//  Problem: U105029 优雅地盛放吧，墨染樱
//  Contest: Luogu - SXYZ 训练赛①
//  URL: https://www.luogu.com.cn/problem/U105029?contestId=352491
//  Memory Limit: 125 MB
//  Time Limit: 1000 ms
//  Interactive: false
//  Test Type: single
//  Batch ID: 06ffb033-4365-42fe-9185-0c6089d6b68c
//
// Algorithm:
// Complexity: O()
// Note:
...
namespace TANGYIXIAO {
const int inf = 0x3f3f3f3f, INF = 0x7fffffff, N = 2e2 + 5;
struct node {
    int t, f, h;
...
inline void dfs(int nw, int l, int r) {
    if (!vis[nw][l][r]) {
        vis[nw][l][r] = true;
        if (nw <= g + 1) {
            if (l >= a[nw].t - a[nw - 1].t) {
                l -= (a[nw].t - a[nw - 1].t);
            } else {
                mx = max(mx, a[nw - 1].t + l);
                return;
            }
            if (r >= d - a[nw].h) {
                flag = true, mn = min(mn, a[nw].t);
                return;
            }
            dfs(nw + 1, l + a[nw].f, r), dfs(nw + 1, l, r + a[nw].h);
        }
    }
    return;
}
