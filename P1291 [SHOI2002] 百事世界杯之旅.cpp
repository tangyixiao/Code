...
inline void solve(int Task_Id) {
    ...
    if (cin >> n) {
        ...
        for (int i = 1; i <= n; ++i) {
            long long g = G(b, i);
            long long na = a * (i / g) + (long long)n * (b / g);
            long long nb = b / g * i;
            g = G(na, nb);
            a = na / g;
            b = nb / g;
        ...
        if (f == 0) {
            ...
            string s1 = to_string(z), s2 = to_string(f), s3 = to_string(b);
            cout << string(s1.length(), ' ') << s2 << "\n";
            cout << s1 << string(s3.length(), '-') << "\n";
            cout << string(s1.length(), ' ') << s3 << "\n";
        }
    }
    return;
}
