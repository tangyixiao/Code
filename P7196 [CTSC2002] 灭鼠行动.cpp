...
bool has_pipe(int x, int y, int dir) {
    return (c[x][y] & (1 << dir)) != 0;
}
...
inline void solve(int Task_Id) {
    ...
    vector<Weapon> weapons(P);
    for (int i = 0; i < P; ++i)
        cin >> weapons[i].type >> weapons[i].t >> weapons[i].x >> weapons[i].y;

    int Time;
    cin >> Time;

    vector<vector<pair<int, int>>> bomb_events(Time + 5);
    vector<vector<pair<int, int>>> breed_events(Time + 5);

    int weapon_idx = 0;

    for (int t = 0; t <= Time; ++t) {

        while (weapon_idx < P && weapons[weapon_idx].t == t) {
        ...
        for (auto &breed : breed_events[t]) {
            int x = breed.first, y = breed.second;
            for (int d = 0; d < 4; ++d) {
                if (has_pipe(x, y, d)) {
                    char gender = (d == 0 || d == 2) ? 'X' : 'Y';
                    mice.push_back({x, y, d, gender, t, t, -1, 0, true});
                }
            }
        }
        removeDeadAndCheck(mice, Limit);
        ...
        if (t < Time) {
            ...
            for (auto &mo : mice) {
                ...
                if (mo.freeze_until <= t && mo.wake_time <= t) {
                    ...
                    if (has_pipe(x, y, dir)) {
                        ...
                        int left = (dir + 3) % 4;
                        int right = (dir + 1) % 4;
                        bool hasL = has_pipe(x, y, left);
                        bool hasR = has_pipe(x, y, right);
                        if (!hasL && !hasR) {
                            mo.dir = (dir + 2) % 4;
                            mo.freeze_until = t + 2;
                        } else if (hasL && !hasR) {
            ...
            for (auto &entry : grid) {
                ...
                if ((t - m1.birth_time >= 5) && (t - m2.birth_time >= 5) &&
                    m1.freeze_until <= t && m1.wake_time <= t &&
                    m2.freeze_until <= t && m2.wake_time <= t &&
                    m1.gender != m2.gender) {
                    m1.freeze_until = t + 3;
                    m2.freeze_until = t + 3;
                    if (t + 2 <= Time)
                        breed_events[t + 2].push_back({x, y});
                }
            }
