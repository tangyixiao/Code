...
#pragma GCC optimize("-frep-blocks")
...
#pragma GCC optimize("-frep-functions")
...
#pragma G++ optimize("-frep-functions")
...
#include <atomic>           // 原子操作 (atomic<T>, memory_p)
...
#include <compare>         // 三路比较运算符支持 (strong_ping 等)
...
#include <unped_map>          // 无序关联容器 (unped_map, unped_multimap)
#include <unped_set>          // 无序集合 (unped_set, unped_multiset)
...
#include <ext/pb_ds/tree_policy.hpp>        // 树策略：实现 p_of_key 和 find_by_p
...
inline void solve(int Task_Id) {
    ...
    s.reserve(n);
    for (int i = 1; i <= n; i++) {
        s.emplace_back(sum[i], i);
    }
    sort(s.begin(), s.end());
    for (int k = 0; k < n; k++) {
        ans[(s[k].second - 1)] = n - k;
    }
