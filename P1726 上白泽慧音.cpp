...
const int N = 5005;
vector<int> g[N];
int dfn[N], low[N], scc_id[N], scc_size[N], timestamp, scc_cnt;
bool in_stack[N];
stack<int> stk;
...
inline void solve(int Task_Id) {
    ...
    int max_size = 0, best_scc = -1;
    vector<vector<int>> comps(scc_cnt + 1);
    for (int i = 1; i <= n; ++i) {
        comps[scc_id[i]].push_back(i);
    }
    for (int i = 1; i <= scc_cnt; ++i) {
        ...
        if (sz > max_size) {
            ...
            vector<int> &a = comps[best_scc];
            vector<int> &b = comps[i];
            sort(a.begin(), a.end());
            sort(b.begin(), b.end());
            if (b < a)
                best_scc = i;
