...
namespace TANGYIXIAO {
const int N = 205, M = 300005, MOD = 998244353;
int n, T, a[N], t[N], pre[N], p2[N], f[M], g[M];
inline void solve(int Task_Id) {
    int c;
    scanf("%d%d%d", &c, &n, &T);
    for (int i = 1; i <= n; ++i) {
    ...
    for (int i = n; i >= 1; --i) {
        int limit = T - t[i] - pre[i - 1];
        if (limit >= 0) {
            long long cnt = 1LL * p2[i - 1] * g[limit] % MOD;
    ...
    for (int i = 1; i <= n; ++i) {
        int limit = T - t[i];
        if (limit >= 0) {
            long long cnt = 1LL * p2[n - i] * f[limit] % MOD;
            ans = (ans + 1LL * a[i] * cnt) % MOD;
        }
        for (int j = T; j >= t[i]; --j) {
            f[j] += f[j - t[i]];
