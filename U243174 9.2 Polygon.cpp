//  Author: Tangyixiao
//  Time: 2026-08-24 11:13:44
//  Problem: U243174 9.2 Polygon
//  Contest: Luogu - SXYZ 训练赛①
//  URL: https://www.luogu.com.cn/problem/U243174?contestId=352491
//  Memory Limit: 512 MB
//  Time Limit: 1000 ms
//  Interactive: false
//  Test Type: single
//  Batch ID: 00aee926-8e68-4ba3-a3c3-944320d7cdf1
//
// Algorithm:
// Complexity: O()
// Note:
...
inline void solve(int Task_Id) {
    ...
    int maxn = 2 * N + 5;
    vector<vector<int64>> C(maxn + 1, vector<int64>(maxn + 1, 0));
    ...
    for (int j = 0; j <= N - 2; j++) {
        int64 term = mul_mod(C[N - 1][j], C[2 * N - 4 - j][N - 2 - j], MOD);
        term = mul_mod(term, C[N - 2 - j][K], MOD);
