...
vector<string> extractWordsLower(const string &s) {
    vector<string> res;
    int n = s.size();
    for (int i = 0; i < n; ++i) {
        if (isalpha(s[i])) {
...
vector<WordInfo> extractWordsInfo(const string &s) {
    vector<WordInfo> res;
    int n = s.size();
    for (int i = 0; i < n; ++i) {
        if (isalpha(s[i])) {
...
inline void solve(int Task_Id) {
    ...
    vector<string> words1 = extractWordsLower(S1);
    ...
    vector<WordInfo> words2 = extractWordsInfo(S2);
    int m = words2.size();
    vector<bool> boldable(m, false);

    for (int i = 0; i < m; ++i) {
        string cur = words2[i].lower;
        for (int j = i + 1; j < m; ++j) {
            cur += " " + words2[j].lower;
            if (keywords.count(cur)) {
                for (int k = i; k <= j; ++k)
                    boldable[k] = true;
            } else {
