//  Author: Tangyixiao
//  Time: 2026-08-24 14:58:47
//  Problem: U105028 偷心魔理沙
//  Contest: Luogu - SXYZ 训练赛①
//  URL: https://www.luogu.com.cn/problem/U105028?contestId=352491
//  Memory Limit: 125 MB
//  Time Limit: 1000 ms
//  Interactive: false
//  Test Type: single
//  Batch ID: 58fd35d1-073f-455e-bad7-4cf4a8aeef4d
//
// Algorithm:
// Complexity: O()
// Note:
...
inline void solve(int Task_Id) {
    ...
    for (int i = 0; i <= n; i++) {
        ...
        for (int j = 1; j <= m && i + (j << 1) <= n; j++) {
            f[i + (j << 1)] = max(f[i + (j << 1)], f[i] + s[i + j] - s[i]);
        }
    }
    cout << f[n];
    return;
