...
inline void bit_add(int *tree, int idx, int val) {
    while (idx <= N) {
        tree[idx] += val;
        idx += idx & -idx;
    }
...
inline int bit_sum(int *tree, int idx) {
    int s = 0;
    while (idx > 0) {
        s += tree[idx];
        idx -= idx & -idx;
...
inline int bit_range(int *tree, int l, int r) {
    if (l > r)
        return 0;
    return bit_sum(tree, r) - bit_sum(tree, l - 1);
}
inline void solve(int Task_Id) {

    scanf("%d", &M);
    for (int i = 0; i < M; ++i) {
        scanf("%d%llu%llu", &ops[i].op, &ops[i].x, &ops[i].y);
        if (ops[i].op == 1 || ops[i].op == 2) {
            ull d = ops[i].x ^ ops[i].y;
            dlist.push_back(d);
        }
    }
    ...
    ds = dlist;
    N = ds.size();
    int total_size = (N + 2) * 61;
    int *pool = new int[total_size]();
    memset(pool, 0, total_size * sizeof(int));
    for (int h = 0; h < 60; ++h) {
        bit[h] = pool + (N + 2) * h;
    }
    total_bit = pool + (N + 2) * 60;

    for (int i = 0; i < M; ++i) {
        int op = ops[i].op;
        ull x = ops[i].x, y = ops[i].y;
        if (op == 1) {
            ull d = x ^ y;
            int idx = lower_bound(ds.begin(), ds.end(), d) - ds.begin() + 1;
            bit_add(total_bit, idx, 1);
            ...
            for (int h = 0; h < MAXB; ++h)
                xbit[h] = (x >> h) & 1;
            for (int h = 0; h < MAXB; ++h) {
                ull high_mask = ~((1ULL << (h + 1)) - 1);
                ull high_part = t & high_mask;
                ull bit_h = (1ULL << h) & ((1 ^ ((t >> h) & 1)) ? (1ULL << h) : 0);
                ull L = high_part | bit_h;
                ull R = L | ((1ULL << h) - 1);
                int l = lower_bound(ds.begin(), ds.end(), L) - ds.begin();
                int r = upper_bound(ds.begin(), ds.end(), R) - ds.begin() - 1;
                if (l > r)
                    continue;
                int l_idx = l + 1, r_idx = r + 1;
                int total = bit_range(total_bit, l_idx, r_idx);
                if (total == 0)
                    continue;
                int cnt1 = bit_range(bit[h], l_idx, r_idx);
                if (xbit[h] == 0) {
                    ans += cnt1;
                } else {
                    ans += total - cnt1;
