...
struct BIT {
    ...
    void add(int pos, i128 val) {
        i128 v1 = val, v2 = val * pos;
...
void add_rectangle(int x1, int x2, int y1, int y2, i128 val) {
    if (x1 > x2 || y1 > y2)
        return;
    events.push_back({x1, y1, y2, val});
    events.push_back({x2 + 1, y1, y2, -val});
}
...
inline void solve(int Task_Id) {
    ...
    while (T--) {
        ...
        for (int i = n; i >= 0; --i) {
            while (top && p[stk[top]] >= p[i])
                --top;
            Rmin[i] = (top ? stk[top] - 1 : n);
            stk[++top] = i;
        }
        events.clear();
        for (int i = 0; i <= n; ++i) {
            add_rectangle(Lmax[i], i, i, Rmax[i], p[i]);
            add_rectangle(Lmin[i], i, i, Rmin[i], -p[i]);
        }
        queries.clear();
        for (int i = 1; i <= q; ++i) {
            int l1, r1, l2, r2;
            cin >> l1 >> r1 >> l2 >> r2;
            int A1 = l1 - 1, A2 = r1 - 1, B1 = l2, B2 = r2;

            queries.push_back({A2, B2, i, 1});
            if (A1 - 1 >= 0)
                queries.push_back({A1 - 1, B2, i, -1});
            if (B1 - 1 >= 0)
                queries.push_back({A2, B1 - 1, i, -1});
            if (A1 - 1 >= 0 && B1 - 1 >= 0)
                queries.push_back({A1 - 1, B1 - 1, i, 1});
            ans[i] = 0;
        }
        ...
        for (auto &qry : queries) {
            while (epos < events.size() && events[epos].x <= qry.x) {
                auto &e = events[epos];
                bit.range_add(e.l + 1, e.r + 1, e.v);
                ++epos;
            }
            i128 cur = bit.prefix_sum(qry.y + 1);
            ans[qry.id] += qry.sign * cur;
        }
        for (int i = 1; i <= q; ++i) {
            print_i128(ans[i]);
            cout << '\n';
