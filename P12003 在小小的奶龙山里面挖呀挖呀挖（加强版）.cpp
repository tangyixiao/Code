...
vector<int> allp;
vector<int> node_primes_pool;
int primes_start[MAXN];
int primes_len[MAXN];
...
void factorize(int x, vector<int> &fac) {
    for (int p : primes) {
        if (p * p > x)
            break;
        if (x % p == 0) {
...
inline void solve(int Task_Id) {
    ...
    vector<vector<int>> fac_val(n + 1);
    for (int i = 1; i <= n; ++i) {
        factorize(a[i], fac_val[i]);
        for (int p : fac_val[i])
            allp.push_back(p);
    }
    ...
    node_primes_pool.reserve(8 * n);
    for (int i = 1; i <= n; ++i) {
        primes_start[i] = node_primes_pool.size();
        for (int val : fac_val[i]) {
            int id = lower_bound(allp.begin(), allp.end(), val) - allp.begin();
            node_primes_pool.push_back(id);
        }
        primes_len[i] = node_primes_pool.size() - primes_start[i];
    }
    vector<vector<int>>().swap(fac_val);
    ...
    for (int i = 0; i < q; ++i) {
        ...
        if (in[u] > in[v])
            swap(u, v);
        if (l == u)
            queries[i] = {in[u], in[v], i, 0};
        else
            queries[i] = {out[u], in[v], i, l};
    }

    int block = max(1, (int)(2 * n / sqrt(q)));
    ...
    auto toggle = [&](int node) {
        int s = primes_start[node];
        int len = primes_len[node];
        if (vis[node]) {
            for (int i = s; i < s + len; ++i) {
                int p = node_primes_pool[i];
                if (--cnt[p] == 0)
                    --cur;
            }
            vis[node] = false;
