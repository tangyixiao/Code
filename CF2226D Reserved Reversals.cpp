...
inline void solve(int Task_Id) {
    ...
    int first_O_large = INT_MAX, last_O_small = -1;
    int first_E_large = INT_MAX, last_E_small = -1;

    for (int i = 0; i < n; ++i) {
        if (a[i] % 2 == 1) {
            if (a[i] > max_even)
                first_O_large = min(first_O_large, i);
            if (a[i] < min_even)
                last_O_small = max(last_O_small, i);
        } else {
            if (a[i] > max_odd)
                first_E_large = min(first_E_large, i);
            if (a[i] < min_odd)
                last_E_small = max(last_E_small, i);
        }
    }

    bool bad = false;
    if (first_O_large < last_O_small)
        bad = true;
    if (first_E_large < last_E_small)
        bad = true;
