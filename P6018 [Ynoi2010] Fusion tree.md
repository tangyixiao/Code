...
$((100...0)_2 - 0) \operatorname{and} \ (1 << (r ^ c) - 1) = (1 << (r ^ c) - 1)$

$((100...0)_2 - y) \operatorname{and} \ (1 << (r ^ c) - 1) = 0$，这里 $y > 0$

先考虑对每个块去掉首位，块内是否有 $1$。

我们用 $A \operatorname{and} \ (\sim C)$ 可以去掉每一块的首位。

然后用 $C - (A \operatorname{and} \ (\sim C)$ 可以使得每一块中除首位外如果有 $1$，则其在该块首位为 $0$，否则为 $1$。

然后用 $(C - (A \operatorname{and} \ (\sim C))) \operatorname{and} C$ 去掉了 $C - (A \operatorname{and} \ (\sim C))$ 中每一块中除首位外的部分。

然后用 $(C - ((C - (A \operatorname{and} \ (\sim C))) \operatorname{and} C))$ 可以得到：如果一块中除首位外有 $1$，则块首位为 $1$，否则为 $0$，且块首位外所有位置都是 $0$ 的一个数
再考虑对每个块只保留首位，块内是否有 $1$。

这个用 $A \operatorname{and} C$ 即可。

最后 $(A \operatorname{and} C) \operatorname{or} \ (C - ((C - (A \operatorname{and} \ (\sim C))) \operatorname{and} C))$ 可以得到：如果一块中有 $1$，则块首位为 $1$，否则为 $0$，且块首位外所有位置都是 $0$ 的一个数。

令 $D = \sum \limits_{k = 0} ^ {r - 1} 2 ^ {k (r ^ c - 1)}$，

通过 $(((A \operatorname{and} C) \operatorname{or} \ (C - ((C - (A \operatorname{and} \ (\sim C))) \operatorname{and} C)) \times D)) >> (w - r)$ 可以将每块首位的数字拼到一个长 $r$ 的二进制数中。
