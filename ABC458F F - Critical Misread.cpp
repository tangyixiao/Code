...
inline void mat_mul(int M, long long A[][N], long long B[][N], long long C[][N]) {
    for (int i = 0; i < M; ++i) {
        for (int j = 0; j < M; ++j) {
            long long sum = 0;
            for (int k = 0; k < M; ++k) {
...
inline void mat_pow(int M, long long base[][N], int p, long long res[][N]) {
    ...
    while (p > 0) {
        if (p & 1) {
            mat_mul(M, res, cur, tmp);
            memcpy(res, tmp, sizeof(tmp));
        }
        mat_mul(M, cur, cur, tmp);
...
inline void solve(int Task_Id) {
    ...
    long long pr[N][N];
    mat_pow(gcnt, tr, n, pr);

    int sta = stg[0];
    long long ans = 0;
    for (int j = 0; j < gcnt; ++j) {
        ans = (ans + pr[sta][j]) % mod;
    }
    cout << ans << '\n';
