...
inline void solve(int Task_Id) {
    ...
    long long sum_i_ai = 0;
    for (int i = 0; i < n; ++i) {
        cin >> a[i];
        max_a = max(max_a, a[i]);
        total_cubes += a[i];
        sum_i_ai += (long long)(i + 1) * a[i];
    }
    ...
    vector<int> sc(max_a + 2, 0);
    for (int v = max_a; v >= 1; --v) {
        sc[v] = sc[v + 1] + freq[v];
    }

    long long sum_c = 0;
    long long sum_c_choose2 = 0;
    for (int h = 1; h <= max_a; ++h) {
        long long c = sc[h];
        sum_c += c;
        sum_c_choose2 += c * (c - 1) / 2;
    }

    long long D = (long long)n * sum_c - sum_c_choose2 - sum_i_ai;
    long long ans = D;

    for (int i = 0; i < n; ++i) {
        int v = a[i];
        long long cv = sc[v];
        long long delta = (i + 1) - n + cv - 1;
        ans = max(ans, D + delta);
    }
