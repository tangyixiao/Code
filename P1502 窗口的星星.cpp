//  Author: Tangyixiao
//  Time: 2026-08-16 10:14:02
//  Problem: P1502 窗口的星星
//  Contest: Luogu
//  URL: https://www.luogu.com.cn/problem/P1502
//  Memory Limit: 512 MB
//  Time Limit: 1000 ms
//  Interactive: false
//  Test Type: single
//  Batch ID: 0987ff34-0d60-40c2-91e9-ef98ff3f517a
//
// Algorithm:
// Complexity: O()
// Note:
...
void U(int u, int l, int r, int ql, int qr, int v) {
    if (ql <= l && r <= qr) {
        M[u] += v;
        L[u] += v;
        return;
...
inline void solve(int Task_Id) {
    ...
    while (T--) {
        ...
        for (int I = 0; I < N; ++I) {
            ...
            int l;
            cin >> x >> y >> l;
            ev[I * 2] = {x - W + 1, y - H + 1, y, l};
            ev[I * 2 + 1] = {x + 1, y - H + 1, y, -l};
            Y[K++] = y - H + 1;
            Y[K++] = y + 1;
        }
        sort(Y, Y + K);
        ...
        for (int i = 0; i < 2 * N;) {
            int j = i;
            while (j < 2 * N && ev[j].x == ev[i].x) {
                int ql = lower_bound(Y, Y + K, ev[j].y1) - Y;
                int qr = lower_bound(Y, Y + K, ev[j].y2 + 1) - Y;
                if (ql < qr)
                    U(1, 0, K - 2, ql, qr - 1, ev[j].l);
                j++;
            }
            A = max(A, (long long)M[1]);
            i = j;
        }
        cout << A << '\n';
    }
