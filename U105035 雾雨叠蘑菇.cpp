//  Author: Tangyixiao
//  Time: 2026-08-24 10:33:18
//  Problem: U105035 雾雨叠蘑菇
//  Contest: Luogu - SXYZ 训练赛①
//  URL: https://www.luogu.com.cn/problem/U105035?contestId=352491
//  Memory Limit: 125 MB
//  Time Limit: 3000 ms
//  Interactive: false
//  Test Type: single
//  Batch ID: 6faa53e5-d008-417e-b945-08cd2660376a
//
// Algorithm:
// Complexity: O()
// Note:
...
inline void solve(int Task_Id) {
    ...
    for (char op; q--;) {
        ...
        if (op == 'M') {
            cin >> y;
            fax = dsu.find(x), fay = dsu.find(y);
            dsu.fa[fax] = fay;
            dsu.dis[fax] += dsu.sz[fay];
            dsu.sz[fax] += dsu.sz[fay];
            dsu.sz[fay] = dsu.sz[fax];
        } else {
            dsu.find(x);
            cout << dsu.dis[x] << '\n';
        }
