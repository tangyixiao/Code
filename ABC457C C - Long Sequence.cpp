...
#include <cstdarg> // 可变参数处理 (va_list, va_s 等)
...
#include <cwchar>  // 宽字符处理 (wchar_t, wcsl)
...
#pragma region TIME
namespace TIME {
clock_t s_Time, End_Time;
inline void s_Time_Count() { s_Time = clock(); }
inline void End_Time_Count() { End_Time = clock(); }
inline double Time_Count() { return (double)(End_Time - s_Time) / CLOCKS_PER_SEC; }
