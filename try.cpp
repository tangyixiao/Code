...
vector<tuple<int, int, int, int>> scc_edges[N];
vector<tuple<int, int, int, int>> cross_edges[N];
...
inline void solve(int Task_Id) {
    ...
    for (int i = 0; i < m; ++i) {
        ...
        cin >> a >> b >> r >> p;
        g[a].push_back({b, r, p});
        rg[b].push_back({a, r, p});
    }
    for (int i = 1; i <= n; ++i) {
        if (!dfn[i])
            tarjan(i);
    }
    vector<int> scc_outdeg(scc_cnt + 1, 0);
    for (int u = 1; u <= n; ++u) {
        for (auto e : g[u]) {
            int v = e.to, r = e.r, p = e.p;
            if (scc_id[u] == scc_id[v]) {
                scc_edges[scc_id[u]].emplace_back(u, v, r, p);
            } else {
                cross_edges[scc_id[u]].emplace_back(u, v, r, p);
                scc_outdeg[scc_id[u]]++;
            }
        }
    }
    vector<int> indeg_scc(scc_cnt + 1, 0);
    for (int i = 1; i <= scc_cnt; ++i) {
        for (auto [u, v, r, p] : cross_edges[i]) {
            indeg_scc[scc_id[v]]++;
        }
    }
    queue<int> q;
    ...
    while (!q.empty()) {
        ...
        q.pop();
        topo.push_back(u);
        for (auto [uu, vv, rr, pp] : cross_edges[u]) {
            int v = scc_id[vv];
            if (--indeg_scc[v] == 0)
                q.push(v);
        }
    ...
    vector<bool> scc_has_internal(scc_cnt + 1, false);
    for (int i = 1; i <= scc_cnt; ++i) {
        scc_has_internal[i] = !scc_edges[i].empty();
    }
    for (int idx = topo.size() - 1; idx >= 0; --idx) {
        int sc = topo[idx];
        vector<ll> base(scc_nodes[sc].size(), 0);
        unordered_map<int, int> pos;
        int sz = scc_nodes[sc].size();
        for (int i = 0; i < sz; ++i) {
            pos[scc_nodes[sc][i]] = i;
        }
        using InternalEdge = tuple<int, int, int>;
        vector<vector<InternalEdge>> rev(sz);
        for (auto [u, v, r, p] : scc_edges[sc]) {
            int uidx = pos[u], vidx = pos[v];
            rev[vidx].emplace_back(uidx, r, p);
        }
        if (scc_has_internal[sc]) {
            queue<int> qu;
            ...
            while (!qu.empty()) {
                ...
                qu.pop();
                inq[v] = false;
                for (auto [u, r, p] : rev[v]) {
                    ll cand = max((ll)r, base[v] - p);
                    if (cand > base[u]) {
                        base[u] = cand;
                        if (!inq[u]) {
                            qu.push(u);
        ...
        for (int i = 0; i < sz; ++i) {
            int node = scc_nodes[sc][i];
            ans[node] = base[i];
        }
        queue<int> qu;
        vector<bool> inq(sz, false);
        for (auto [u, v, r, p] : cross_edges[sc]) {
            int uidx = pos[u];
            ll cand = max((ll)r, ans[v] - p);
            if (cand < ans[u]) {
                ans[u] = cand;
                if (!inq[uidx]) {
                    qu.push(uidx);
        ...
        vector<vector<InternalEdge>> fwd(sz);
        for (auto [u, v, r, p] : scc_edges[sc]) {
            int uidx = pos[u], vidx = pos[v];
            fwd[uidx].emplace_back(vidx, r, p);
        }
        while (!qu.empty()) {
            int u = qu.front();
            qu.pop();
            inq[u] = false;
            for (auto [v, r, p] : fwd[u]) {
                ll cand = max((ll)r, ans[scc_nodes[sc][v]] - p);
                if (cand < ans[scc_nodes[sc][u]]) {
                    ans[scc_nodes[sc][u]] = cand;
                    if (!inq[u]) {
                        qu.push(u);
                        inq[u] = true;
                    }
