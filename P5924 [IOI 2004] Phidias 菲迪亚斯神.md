...
If Phidias cuts out a slab of size $A\times B$, then it cannot be used as a $B\times A$ slab unless $A = B$. For each required slab size, Phidias may cut out zero or more slabs. After all cuts are finished, any produced slab whose size is not among the required sizes becomes waste.

Phidias wants to know how to cut the original slab so that the total waste area is minimized.

For example, in the figure below, the original slab has width $21$ and height $11$, and the required sizes are $10\times4$, $6\times2$, $7\times5$, and $15\times10$. Then the minimum total waste area is $10$. The figure also shows a cutting plan that achieves the minimum waste area of $10$:
...
The first integer $W$ is the width of the original slab, and the second integer $H$ is the height of the original slab.

The second line contains an integer $N$, the number of required slab types. The next $N$ lines give the sizes of the required slabs.

Each line contains two integers: the first is the required slab width $W_i$, and the second is the required slab height $H_i$.
...
Constraints: for $100\%$ of the testdata, $1\le W, H\le 600$, $0\le N\le 200$, $1 \le W_i \le W$, $1 \le H_i \le H$.
