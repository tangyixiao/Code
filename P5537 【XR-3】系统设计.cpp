...
//  Interactive: false
//  Test Type: single
//  Batch ID: a7598cf3-5f7c-402a-8910-d3dd7435f80c
//
// Algorithm:
// Complexity: O()
// Note:
...
int n, m, q, f[N], d[N], o[N], c[N], w[N], s[N], hs[N], dp[N], tp[N], df[N], rv[N], bt[N], tmr, A[N], rt;
unsigned long long H[N], P[N], st[N * 4];
void d1(int u, int dep) {
    dp[u] = dep;
    s[u] = 1;
    int ms = 0;
    for (int i = 0; i < d[u]; i++) {
...
void bd(int nd, int L, int R) {
    ...
    bd(nd * 2, L, M);
    bd(nd * 2 + 1, M + 1, R);
    st[nd] = st[nd * 2] * P[R - M] + st[nd * 2 + 1];
}
void up(int nd, int L, int R, int x, int v) {
    if (L == R) {
        st[nd] = v;
        return;
    }
...
int gl(int nd, int L, int R, int ql, int qr, int &x) {
    if (ql <= L && R <= qr) {
        int ln = R - L + 1, rm = dp[bt[x]] - dp[x];
        if (ln <= rm) {
            int nx = rv[df[x] + ln];
            if (st[nd] == H[nx] - H[x] * P[ln]) {
                x = nx;
                return ln;
            }
        }
    ...
    int M = (L + R) / 2, mt = 0;
    if (ql <= M) {
        mt = gl(nd * 2, L, M, ql, qr, x);
        if (mt < min(M, qr) - max(L, ql) + 1)
            return mt;
    }
    if (qr > M)
        mt += gl(nd * 2 + 1, M + 1, R, ql, qr, x);
    return mt;
}
inline void solve(int Task_Id) {
    cin >> n >> m >> q;
