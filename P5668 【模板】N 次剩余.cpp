...
ll bs(ll g, ll y, ll m, ll o) {
    ...
    int z = sqrt((long double)o) + 1;
    hc();

    ll v = y;
    for (int j = 0; j < z; j++, v = v * g % m)
        hp(v, j);
...
int ur(ll n, ll k, ll p, int q, ll P, ll *out) {
    if (p != 2 || q <= 2) {
        ll f = ph(p, q);
        ll g = rt(p, q, P);
        ll e = bs(g, k, P, f);
...
bool ck(ll n, ll k, ll p, int q, ll P) {
    k %= P;
    ...
    return ur(n, k, p, q, P, c) > 0;
}

int sv(ll n, ll k, ll p, int q, ll P, ll *out) {
    k %= P;
