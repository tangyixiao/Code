//  Author: Tangyixiao
// Time: 2026-08-29 10:26:18
//  Problem: U105740 隐形的翅膀
//  Contest: Luogu - SXYZ 训练赛②
//  URL: https://www.luogu.com.cn/problem/U105740?contestId=353385
//  Memory Limit: 125 MB
//  Time Limit: 1000 ms
//  Interactive: false
//  Test Type: single
//  Batch ID: 37e6ab5f-6174-4f58-8205-1390914c2e37
//
// Algorithm:
// Complexity: O()
// Note:
...
namespace TANGYIXIAO {
const int N = 3e4 + 5;
const double G = 0.6180339887498949, eps = 1e-15;
int n, a[N], x, y;
double ans = 1e9;
...
inline void solve(int Task_Id) {
    ...
    sort(a + 1, a + n + 1);
    for (int i = 1; i < n; i++) {
        int p = lower_bound(a + i + 1, a + n + 1, a[i] / G) - a;
        for (int j = max(i + 1, p - 1); j <= min(n, p); j++) {
            if (abs(1.0 * a[i] / a[j] - G) < ans - eps || abs(abs(1.0 * a[i] / a[j] - G) - ans) <= eps && make_pair(a[i], a[j]) < make_pair(x, y)) {
                ans = abs(1.0 * a[i] / a[j] - G), x = a[i], y = a[j];
            }
        }
    }
    cout << x << '\n'
