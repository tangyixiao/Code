...
//  Interactive: false
//  Test Type: single
//  Batch ID: a45e5914-49a9-4670-969a-a2415d7d94e9
//
// Algorithm:
// Complexity: O()
// Note:
...
namespace TANGYIXIAO {
const int N = 2e5 + 5;
int n, k, a[N], b[N], l, r, d, c, s, la;
inline void solve(int Task_Id) {
    cin >> n >> k, c = s = 0, la = 1;
    for (int i = 1; i <= n; i++) {
        cin >> a[i], b[i] = a[i];
    }
    sort(b + 1, b + 1 + n), l = b[1], r = b[((n + k + 1) >> 1)], d = r - l;
    for (int i = 1; i + ((n + k + 1) >> 1) - 1 <= n; i++) {
        if (b[i + ((n + k + 1) >> 1) - 1] < d + b[i]) {
            l = b[i], r = b[i + ((n + k + 1) >> 1) - 1], d = r - l;
        }
    }
    cout << l << " " << r << "\n";
    for (int i = 1; i <= n; i++) {
