...
inline void solve(int Task_Id) {
    ...
    for (int i = 0; i < T; ++i) {
        ...
        mat[uu][vv] = std::min(mat[uu][vv], (long long)w);
        mat[vv][uu] = std::min(mat[vv][uu], (long long)w);
    }
    int sz = cnt;
    ...
    for (int i = 0; i < sz; ++i)
        for (int j = 0; j < sz; ++j)
            res[i][j] = (i == j) ? 0 : INF;
    long long base[200][200];
    memcpy(base, mat, sizeof(mat));
    int exp = N;
    while (exp) {
        if (exp & 1) {
            // res = res * base
            long long tmp[200][200];
            for (int i = 0; i < sz; ++i)
                for (int j = 0; j < sz; ++j) {
                    tmp[i][j] = INF;
                    for (int k = 0; k < sz; ++k) {
                        if (res[i][k] < INF && base[k][j] < INF) {
                            long long val = res[i][k] + base[k][j];
                            if (val < tmp[i][j])
                                tmp[i][j] = val;
                        }
                    }
        ...
        for (int i = 0; i < sz; ++i)
            for (int j = 0; j < sz; ++j) {
                tmp[i][j] = INF;
                for (int k = 0; k < sz; ++k) {
                    if (base[i][k] < INF && base[k][j] < INF) {
                        long long val = base[i][k] + base[k][j];
                        if (val < tmp[i][j])
                            tmp[i][j] = val;
                    }
                }
