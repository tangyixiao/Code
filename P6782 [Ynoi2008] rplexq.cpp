...
int n, m, rt;
int he[N], to[N << 1], ne[N << 1], te = 0;
int sz[N], p[N], hc[N], in[N], out[N], tp[N], df = 0;
int la[4000005], lh[N], ll[N], lc = 0;
int bt[N];
int hq[N], nq[M << 2], qid[M << 2], qu[M << 2], qs[M << 2], tq = 0;
int Sx[M], Shc[M], S[N];
long long ans[M], slc[N];
struct MQ {
    int l, r, x, id, b;
    bool operator<(const MQ &o) const {
        if (b != o.b)
            return b < o.b;
...
void add_q(int pt, int i, int u, int s) {
    if (!pt)
        return;
    tq++;
    qid[tq] = i;
...
inline void solve(int Task_Id) {
    ...
    for (int i = 1; i <= n; i++) {
        up(in[i], 1);
        for (int e = hq[i]; e; e = nq[e]) {
            int val = qr(out[qu[e]]) - qr(in[qu[e]] - 1);
            if (qs[e] == 1)
                Sx[qid[e]] += val;
            else if (qs[e] == -1)
                Sx[qid[e]] -= val;
    ...
    int L = 1, R = 0;
    for (int i = 1; i <= m; i++) {
        int ql = qm[i].l, qr = qm[i].r, id = qm[i].id, x = qm[i].x;
        while (L > ql) {
            --L;
            for (int j = lh[L]; j < lh[L] + ll[L]; ++j)
                slc[p[la[j]]] += S[la[j]]++;
        }
        while (R < qr) {
            ++R;
            for (int j = lh[R]; j < lh[R] + ll[R]; ++j)
                slc[p[la[j]]] += S[la[j]]++;
        }
        while (L < ql) {
            for (int j = lh[L]; j < lh[L] + ll[L]; ++j)
        ...
        ans[id] = 1LL * Sx[id] * (Sx[id] - 1) / 2;
        if (hc[x])
            ans[id] -= 1LL * Shc[id] * (Shc[id] - 1) / 2;
        ans[id] -= slc[x];
    }
    for (int i = 1; i <= m; i++)
        wt(ans[i]);
...
struct SqrtTree {
  ...
  void add(int x, int y) {
    for (int i = ((x >> 7) << 7); i <= x; ++i)
      s1[i] += y;
    for (int i = ((x >> 14) << 7); i < (x >> 7); ++i)
      s2[i] += y;
...
void dfs1(int u, int fz) {
  ...
  if (fz)
    G[u].erase(find(G[u].begin(), G[u].end(), fz));
...
signed main() {
  ...
  for (int i = 1, x; i <= m; ++i)
    cin >> ql[i] >> qr[i] >> x, Q[x].push_back(i);
  dfs1(rt, 0), dfs2(rt);
  for (int u = 1; u <= n; ++u) {
    ...
    if ((int)G[u].size() <= B) {
      for (int v : G[u])
        at[efn[v]].push_back(u);
    } else {
      for (int i = 0; i < B; ++i)
        at[efn[G[u][i]]].push_back(u);
  ...
  for (int u = 1; u <= n; ++u)
    if ((int)G[u].size() > B + 1) {
      for (int x : Q[u])
        ans[x] += 1ll * ed[x] * (ed[x] - 1) / 2;
      for (int i = B; i < (int)G[u].size(); ++i) {
        for (int v = G[u][i], j = dfn[v]; j <= efn[v]; ++j)
          cl[rk[j]] = i - B, vis.set(rk[j]);
      }
      for (int i = vis._Find_first(); i <= n; i = vis._Find_next(i))
        a[++tp] = i;
      vector<int> op;
      for (int x : Q[u]) {
