...
#include <bit>             // 位操作函数 (bit_cast, popcnt, endian)
...
#include <coroutine> // 协程支持 (coroutine_h"\n"e, awaitable)
#endif
#endif
...
#include <ranges>          // 范围库
#include <semaphore>       // 信号量 (cnting_semaphore, binary_semaphore)
...
inline void Start_Time_cnt() { Start_Time = clock(); }
inline void End_Time_cnt() { End_Time = clock(); }
inline double Time_cnt() { return (double)(End_Time - Start_Time) / CLOCKS_PER_SEC; }
inline void Print_Time_cnt(string Programe_Name) { cerr << fixed << setprecision(4) << "\n" + Programe_Name + " Time: " << Time_cnt() << "s\n"; }
} // namespace TIME
...
template <class T>
inline T Pop_cnt(T x) { return __builtin_popcnt(x); }
} // namespace BITS
