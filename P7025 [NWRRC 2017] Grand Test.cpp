...
namespace TANGYIXIAO {
const int N = 2e5 + 5;
int h[N], to[N], nx[N], ec;
int dep[N], p[N], vis[N], tag[N];
struct E {
    int u, v;
} e[N];
int cnt, p1[N], p2[N], p3[N], t[N], tmp[N], k1, k2, k3;
...
void g_dn(int x, int y, int a[], int &k) {
    int tk = 0;
    for (int c = y;; c = p[c]) {
        tmp[tk++] = c;
        if (c == x)
...
void g_up(int x, int y, int a[], int &k) {
    k = 0;
    for (int c = x;; c = p[c]) {
        a[k++] = c;
        if (c == y)
...
inline void solve(int Task_Id) {
    ...
    int u1 = e[id1].u, v1 = e[id1].v, u2 = e[id2].u, v2 = e[id2].v;
    int A, B, uA, uB;
    if (dep[v1] <= dep[v2]) {
        A = v1;
        uA = u1;
