...
inline void solve(int Task_Id) {
    ...
    vector<int> odd_rad(len);
    for (int i = 0; i < len; ++i)
        odd_rad[i] = rad[2 * i + 1] / 2;

    vector<int> even_rad(len - 1);
    for (int i = 0; i < len - 1; ++i)
        even_rad[i] = rad[2 * i + 2] / 2;

    vector<vector<pair<int, int>>> odd_centers(len);
    vector<vector<pair<int, int>>> even_centers(len - 1);
    ...
    for (int x = 0; x < n; ++x) {
        ...
        if (sum % 2 == 0) {
            int C = sum / 2;
            int d = (r_ - l) / 2;
            odd_centers[C].push_back({x, d});
        } else {
            int i = (sum - 1) / 2;
            int d = (r_ - l + 1) / 2;
            even_centers[i].push_back({x, d});
        }
    }
    ...
    for (int C = 0; C < len; ++C) {
        vector<pair<int, int>> list = odd_centers[C];
        list.push_back({a[C], 0});
        sort(list.begin(), list.end());
        int max_d = 0, expected = 0;
        for (auto &p : list) {
            if (p.first == expected) {
                max_d = max(max_d, p.second);
                if (max_d > odd_rad[C])
                    break;
                ++expected;
            } else if (p.first > expected) {
                break;
            }
        }
    ...
    for (int i = 0; i < len - 1; ++i) {
        vector<pair<int, int>> list = even_centers[i];
        sort(list.begin(), list.end());
        int max_d = 0, expected = 0;
        for (auto &p : list) {
            if (p.first == expected) {
